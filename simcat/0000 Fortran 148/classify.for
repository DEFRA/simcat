...
*     --------------------------------------------------------------------------
      ...
      if ( ical13 .eq. 1 ) return ! Edward II
...
*     store the mean and standard deviation ------------------------------------
      COMPM = C(idet,1)
      COMPS = C(idet,2)
      if ( DETYPE (idet) .eq. 909 ) then ! dissolved metal
...
*     Kstat is 6 for the 99-percentile -----------------------------------------
	...
	kstat = 0
*     set 95-percentile target if not set with determinand data ----------------
...
*     set up the effective sampling rate ---------------------------------------
	qualnuse = qualn(idet)

*     standards expressed as a mean --------------------------------------------
      if ( kstat .gt. 0 .and. kstat .lt. 7 ) then
      if ( kstat .eq. 1 ) then
      call write the mean standard (01, idet, targit)
      call write the mean standard (21, idet, targit)
...
*     95-percentile target -----------------------------------------------------
      if (kstat .eq. 2 ) then
	xpoint=95.0
      call write the percentile standard (01, idet, targit, xpoint)
      call write the percentile standard (21, idet, targit, xpoint)
      call change to log domain (test,xpoint,COMPM,COMPS,testm,tests)
      call calculate compliance with percentile standard (testm,
     &tests,qualnuse,targit,xpoint,EX,testq,testql,testqu,KU)
	endif

*     99-percentile target -----------------------------------------------------
      if ( kstat .eq. 6 ) then
	xpoint=99.0
      call write the percentile standard (01, idet, targit, xpoint)
      call write the percentile standard (21, idet, targit, xpoint)
...
*     5-percentile target ------------------------------------------------------
	...
	xpoint=5.0
      call write the percentile standard (01, idet, targit, xpoint)
      call write the percentile standard (21, idet, targit, xpoint)
      call write the percentile standard (30, idet, targit, xpoint)
...
*     estimate mean and standard deviation for percentile ----------------------
	...
	call ASSEM (testql,testqu,SET1)
      call sort format 1 (testq,qualn(idet))
      write(01,70)valchars10,units(idet),SET1,EX
      write(21,70)valchars10,units(idet),SET1,EX
   70 format(46x,'Test statistic =',a10,1x,a4,4x,a13/
     &21x,'Confidence that the standard was failed =',F10.3,' %'/
     &110('-'))
...
*     if ( testq .gt. targit ) ifailtest50 (idet) = 1
      if ( ex .gt. 95.0 ) ifailtest95 (idet) = 1
	ee = ex
	cc = 100.0 - conf of class(2)
	if ( ee .gt. 1. .and. cc .gt. 1.0 ) then
      endif
	endif
	endif
...
*     list compliance for each determinand -------------------------------------
 ...
 3266 format('End of reach: ',a16,2x,a16,2f6.1,f7.2,f7.2,i2,3f9.3,
     &f11.3,' %',i6)
      ...
      if ( kplace .eq. 1 ) then ! not the start of the reach
      write(30,3166) Rname(IREACH),Dname(idet),
     &Total river length,Total RQS length 1,test,targit,MRQS(idet),
     &Total length dets 00(idet),Total length dets 50(idet),
     &Total length dets 95(idet),EX,kplace
 3166 format('Start reach: ',a16,3x,a16,2f6.1,f7.2,f7.2,i2,3f9.3,
     &f11.3,' %',i6)
      endif ! if ( kplace .ne. 3 ) ... not the start of the reach
      endif ! if ( nobigout .le. 0 )
      exall = amax1 ( exall,  ex )
 	endif !  if ( ical13 .eq. 0  )
   10 continue ! loop on determinands 
...
*     exclude Determinads from the overall classification ----------------------
...
*     exclude Determinads from the overall classification ----------------------
   ...
   49 continue
      ...
      if ( ical13 .eq. 0 ) then
      if ( nobigout .le. 0 ) write(30,26) Total river length,
     &Total length 2, 
...
*     set up the heading -------------------------------------------------------
	...
	write(01,1010)UNAME(feeture),feeture,IREACH,
     &DIST(feeture),RNAME(IREACH)
	write(21,1010)UNAME(feeture),feeture,IREACH,
     &DIST(feeture),RNAME(IREACH)
 1010 format(/110('=')/'Classification at: ',A40,'Feature:',I6,
     ...
     &'of the Reach called ',A16)
      ...
      if ( kplace .eq. 4 ) then
	write(48,1820)UNAME(feeture),feeture,IREACH,
     &DIST(feeture),RNAME(IREACH)
	write(01,1820)UNAME(feeture),feeture,IREACH,
     &DIST(feeture),RNAME(IREACH)
	write(21,1820)UNAME(feeture),feeture,IREACH,
     &DIST(feeture),RNAME(IREACH)
 1820 format(/110('=')/'Classification d/s of: ',A40,'Feature:',I6,
     ...
     &'of the Reach called ',A16)
      ...
      if ( kplace .eq. 1 ) then
	write(48,1910)RNAME(IREACH)
	write(01,1910)RNAME(IREACH)
	write(21,1910)RNAME(IREACH)
 1910 format(/110('=')/'Classification at the head of the Reach ',
     &'called ',A16)
	endif
      if ( kplace .eq. 3 ) then
	write(48,1919)RNAME(IREACH)
	write(01,1919)RNAME(IREACH)
	write(21,1919)RNAME(IREACH)
 1919 format(/110('=')/'Classification at the end of the the Reach ',
     &'called ',A16/110('-'))
	write(48,1939)RLENGTH(IREACH)
	write(01,1939)RLENGTH(IREACH)
	write(21,1939)RLENGTH(IREACH)
...
*     99999999999999999999999999999999999999999999999999999999999999999999999999
      IQSreach = 0
      class limmits (1,idet) = 1.0e10
      do ic = 2, nclass
      class limmits (ic,idet) = 0.0
      enddo
      if ( background class .eq. 1 ) then ! set up background class limits -----
...
*     store the mean and standard deviation ------------------------------------
      ...
      COMPM = cpart1 (idet,1) ! dissolved metal
	COMPS = cpart1 (idet,2) ! dissolved metal
      endif ! if ( DETYPE (idet) .eq. 909 ) dissolved metal --------------------

      if ( limits in place .eq. 0 ) goto 10
*     ignore the excluded determinands -----------------------------------------
*     those not part of the Water Framework Directive --------------------------
...
*     exclude determinands from overall classification -------------------------
      if ( exclude BOD .eq. 1 .and. Detype (idet) .eq. 101 ) goto 2900
      if ( exclude NO3 .eq. 1 .and. Detype (idet) .eq. 106 ) goto 2900
      if ( exclude PO4 .eq. 1 .and. Detype (idet) .eq. 105 ) goto 2900
      if ( exclude DOX .eq. 1 .and. Detype (idet) .eq. 104 ) goto 2900
      if ( exclude AMM .eq. 1 .and. Detype (idet) .eq. 103 ) goto 2900
...
*     10-percentile class limits ----------------------------------------------
	...
	call calculate compliance with percentile standard (testm,
     &tests,qualn(idet),targit,xpoint,EX,testq,testql,testqu,KU)
      ...
      if ( nocon .eq. 99 ) then
      if ( nobigout .le. 0 ) then
	write(21,77)COMPM,COMPS    
	write(48,77)COMPM,COMPS    
   77 format(
     &24x,'                                   Mean =',F12.3/
     &24x,'                     Standard deviation =',F12.3)
      write(21,70)testq,testql,testqu,qualn(idet),EX
      write(48,70)testq,testql,testqu,qualn(idet),EX
   70 format(
     &24x,'                         Test statistic =',F12.3/
     &24x,'                 Lower Confidence limit =',F12.3/
     &24x,'                 Upper Confidence limit =',F12.3/
     &24x,'                         Sample numbers =',F12.3/
     &24x,'Confidence that the standard was failed =',F12.3,' %'/
     &77('-'))
 	endif ! if ( nobigout .le. 0 )
  	endif ! if ( nocon .eq. 99 )
      endif ! if ( targit .gt. 1.0e-20 ) 
...
*     initialise the face value class -----------------------------------------
	...
	write(01,2221)Dname(idet),(in class(iclass,idet),
     &iclass = 1,nclass),(Class limmits (ii,idet), ii = 1,nclass - 1),
     &testq
	write(21,2221)Dname(idet),(in class(iclass,idet),
     &iclass = 1,nclass),(Class limmits (ii,idet), ii = 1,nclass - 1),
     &testq
 2221 format(A11,5f9.2,4x,4f10.2,f10.3)
      endif ! if ( limits in place .eq. 1 ) then -------------------------------
	if ( limits in place .eq. 0 ) then ! -------------------------------------
	write(48,2241)Dname(idet),(in class(iclass,idet),
     &iclass = 1,nclass),testq
	write(01,2241)Dname(idet),(in class(iclass,idet),
     &iclass = 1,nclass),testq
	write(21,2241)Dname(idet),(in class(iclass,idet),
     &iclass = 1,nclass),testq
 2241 format(A11,5f9.2,4x,4('         -'),f10.3)
      endif ! if ( limits in place .eq. 0 ) ------------------------------------
      endif ! if ( nobigout .le. 0 ) ===========================================
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      if ( suppress4 .lt. 2 .and. kerror .eq. 1 ) then
 ...
 8411 format('* No determinands for classification',15x,'...',7x,
     &'at ',a40,1x,25('.'))
      ...
      if ( suppress4 .eq. 2 .and. kerror .eq. 1 ) then
      suppress4 = suppress4 + 1
	call change colour of text (21) ! dull red
      write( *,8491)uname(KU)
 8491 format('* No determinands for classification',15x,'...',7x,
     &'at ',a40,' and elsewhere ...........')
      call set screen text colour
      endif
      Face determinand = 1
      endif
...
*     this will be labelled JDET -----------------------------------------------
      ...
      if ( Detype (idet) .eq. 100 )  goto 2000
      if ( Detype (idet) .eq. 102 )  goto 2000
...
*     sum the per cent of lengths in each class for each determinand -----------
	...
	if ( qtype (idet) .ne. 4 ) then
      if ( kplace .ne. 4 ) then
      total length in classes (iclass, idet) = 
     &total length in classes (iclass, idet) + distp  
...
*     sum the per cent of lengths in each class over all determinands ... 
      if ( kplace .ne. 4 ) then
      total length over all (iclass) = 
     &total length over all (iclass) + distp
 ...
 2621 format(110('=')/'Accumulated lengths in classes (kilometres) ...'/
	...
	if ( qtype (idet) .eq. 4 ) goto 2312
      if ( Detype (idet) .eq. 100 ) goto 2312
      if ( Detype (idet) .eq. 102 ) goto 2312
      if ( Detype (idet) .eq. 106 ) goto 2312
      if ( Detype (idet) .eq. 200 ) goto 2312
      if ( Detype (idet) .eq. 201 ) goto 2312
      write(48,1488)(total length in classes (iclass, idet), 
     &iclass = 1, nclass), Dname(idet)
 1488 format('           ',5f9.2,23x,' (',a11,')')
 2312 continue
 	write(48,2587)
 	write(01,2587)
 	write(21,2587)
...
*     ==========================================================================
    ...
    1 format(a11,24x,i5,'-percentile standard =',a10,1x,a4)
      ...
      if ( nobigout .le. 0 ) then
      if ( kplace .ne. 2 .and. kplace .ne. 4 ) then
      write(01,7009)
      write(21,7009)
 7009 format(110('-')/'Assessment of compliance with standards ',
     &70('.')/110('-'))
      ...
      if ( kplace .eq. 4 ) then
      write(01,7809)UNAME(feeture)
      write(21,7809)UNAME(feeture)
 7809 format(110('-')/'Assessment of compliance with standards d/s of ',
     &a40,/110('-'))
      endif
      if ( kplace .eq. 2 ) then
      write(01,7509)UNAME(feeture)
      write(21,7509)UNAME(feeture)
 7509 format(110('-')/'Assessment of compliance with standards at ',
     &a40,/110('-'))
      endif
      endif
