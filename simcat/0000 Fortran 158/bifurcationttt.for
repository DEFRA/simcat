...
*     ==========================================================================
*     File BIFURCATION.FOR ... 1107 lines (76 lines of comments) ---------------
*     --------------------------------------------------------------------------
...
*     check whether arrays can be cleared --------------------------------------
      ...
      if ( JT(JJ) .eq. 11 .and. JF(JJ) .eq. IDENTIFY ) goto 2297 ! second arm
      endif
 2296 continue
...
*     77777777777777777777777777777777777777777777777777777777777777777777777777
      ...
      TBLODE1 (JPL,J1) = TBLODE4 (ISTOR,JPL,J1)   
      enddo ! do J1 = 1, nx ----------------------------------------------------
...
*     77777777777777777777777777777777777777777777777777777777777777777777777777
      ...
      TELODEshots(iworks,JPL,is) = TELOADrchshots(iworks,ISTOR,is) ! 33333333333
      endif ! if ( JPL .eq. ndshot ) 33333333333333 apportionment of percentiles
...
*     rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr
      ...
      !call generate log normal river flows ! log-normal          
...
*     eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
      ...
      if ( BM(3) .gt. 1.0E-08 ) BM(3)=EFM/BM(3) ! bias in mean
      if ( BS(3) .gt. 1.0E-08 ) BS(3)=EFS/BS(3) ! bias in Q95
      call generate log normal bifurcation (GEFM,GEFS,EFM) ! log normal effluent        
      endif ! effluent type ====================================================
*     eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
...
*     set the starting values of the loads at the head of the new reach --------
      ...
      do is = 1, NS ! 33333333333333333333333333333 apportionment of percentiles
      TELODEshots(iworks,JPL,is) = frat1 * TELODEshots(iworks,JPL,is) ! 33333333
      enddo ! do is = 1, NS 33333333333333333333333 apportionment of percentiles
      ...
      frat1 = AXLODE (JPL,i13) / TGLODE2 (JPL,i13) ! scale the loads @@@@@@@@@@@
...
*     77777777777777777777777777777777777777777777777777777777777777777777777777
      ...
      TILOADUP2(JPL,J1) = xxlode(22,J1) ! Gap filling for river flows
...
*     77777777777777777777777777777777777777777777777777777777777777777777777777
      ...
      frat = AXLODE (JPL,J1) / TGLODE2 (JPL,J1) ! scale the loads @@@@@@@@@@@@@@ 

      TBLODE2 (JPL,J1) = frat * TBLODE2 (JPL,J1) ! removed by abstractions  
      TBLODE1 (JPL,J1) = frat * TBLODE1 (JPL,J1) ! scale the loads @@@@@@@@@@@@@ 
      
      if ( kount bodies .gt. 0 ) then
      do ibodies = 1, kount bodies
*     monthly loads from upstream sub-catchments -------------------------------
      TWLOADS(ibodies,JPL,J1) = frat * TWLOADS(ibodies,JPL, J1) ! scale loads @@
      do ip = 1, n2prop
*     breakdown of monthly loads from upstream sub-catchments ------------------
      TWLOADSapp(ibodies,JPL,J1,ip) = frat * 
     &TWLOADSapp(ibodies,JPL,J1,ip)
      enddo
...
*     check whether arrays can be cleared --------------------------------------
 ...
 2296 continue
      ...
      IRHOLD (Ireech) = 0 !  used to store data for a reach that is needed later
 2297 continue
...
*     77777777777777777777777777777777777777777777777777777777777777777777777777     
      ...
      if ( n251 .eq. 1 ) then ! 3333333333333333333 apportionment of percentiles
      do is = 1, NS ! 33333333333333333333333333333 apportionment of percentiles
      TELODEshots(iworks,JPL,is) = TELODEshots(iworks,JPL,is) * split ! 33333333
      enddo ! do is = 1, NS 33333333333333333333333 apportionment of percentiles
      endif ! if ( n251 .eq. 1 ) ! 3333333333333333 apportionment of percentiles
...
*     net loads from discharges (3 12 5 and 39 and 60 and 61) -----------TELODE2
      ...
      TALOADUP1 (JPL,J1) = TALOADUP1(JPL,J1) * Split ! scale the loads @@@@@@@@@
      TILOADUP1 (JPL,J1) = TILOADUP1(JPL,J1) * Split ! scale the loads @@@@@@@@@ 
