...
*     read the name of the data file -------------------------------------------
 ...
 9999 write( *,9998)
 9998 format('*** Error in reading RUN157.TMP '/
     &'*** Calculation stopped ...')
	call stop
      end
...
*     ..21   shorter version out .out:                   filename.OWT
...
*      121-130                  Files for output on apportionment.ADL
*     ==========================================================================
...
*     open summary output file -----------------------------------------------03
      write (fname,99) datname
      call trunkout ('S','U','M')
      open(03,FILE=FNAME) ! open summary output file .SUM --------------------03
	write(03,*)' '
...
*     file for outputs to .GIS ... output file .GIS1.CSV ----- SAGIS -------- 42
      write (fname,99) datname
      call trunk2
 ...
 8255 format('SIMCAT',',','Version 15.7 ',',', ! ---------------------------- 42
...
*     check for illegal values -------------------------------------------------
      if ( xx .gt. 1.0 .or. xx .lt. -1.0 ) then
      pollution data (le,idet,MO) = -9.9 ! initialise data 
      goto 22
      endif
...
*     add stem for output file for apportionment ... .ADL etc
    ...
    1 continue
    8 continue
      write (fname,5) (a(i),i=1,j),(a(i),i=j+1,129)
      read(fname,5) (a(i),i=1,136)
...
*     add stem for output file for apportionment ... .ACC
      subroutine trunk13 (kay)
      include 'COMMON DATA.FOR'
      character*1 a(137)
	character*1 b(10)
...
*     add the name of the folder to the name of the file -----------------------  
      subroutine add folder for output
      include 'COMMON DATA.FOR'
      if ( folder .eq. '    ' ) then
      write (FNAME,2) 'OUTPUT'
    2 format(A30)
      call blank ! eliminate blanks in filenames
      else
      write (FNAME,1) folder, 'OUTPUT'
    1 format(a100,A30)
      call blank ! eliminate blanks in filenames
      endif
      output_folder = fname
