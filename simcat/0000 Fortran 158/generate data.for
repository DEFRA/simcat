...
*     ==========================================================================
*     File generate data.for ... 7034 lines ------------------------------------
*     --------------------------------------------------------------------------
...
*     generate shots for river flow --------------------------------------------
      ...
      if (  F(IF,2)/F(IF,1) .lt. 0.6 .and. munthly structure .eq. 1 ) ! --------
     &then !  impose monthly structure on log normal flows ---------------------
      
      call generate monthly structure for stream flows 2 ! headwaters etc ------
      ...
      !write(33,1477)uname(feeture),Flow(1),Flow(2),IF
      !1477 format(
      !&'Summary statistics after adjustments at: ',a30/
      !&'After SR Generate Monthly Structure for Stream Flows 2 ... '/
      !&'That was called from SR Generate Log Normal River Flows ... '/
      !&77('-')/
      !&'              Mean = ',f10.4/
      !&'      5-percentile = ',f10.4,8x,'Flow data set:',i5/77('='))

      return
      
      else ! use annual mean data ...
*     use the the annual data ==================================================
...
*     calculate the Monte-Carlo sampling errors for log-normal river flows -----
 ...
 6422 format(
     ...
     &'                    5-percentile =',f12.4/77('='))
      ...
      !if ( MONF .gt. 1 ) call write shots for river flow ! after corrections
      ...
      return
      endif ! if (  F(IF,2)/F(IF,1) .lt. 0.6) then
*     use of the annual mean data ==============================================
...
*     impose monthly structure on log normal flow removals =============================
      call impose monthly structure for river flows (1,idirx) ! log normal river flow removals
      return
      endif
...
*     call impose monthly structure for river flows (1,idirx) ! normal river flow removals
*     return
*     endif
...
*     call impose monthly structure for river flows (1,idirx) ! log normal effluent removals
*     return
*     endif
...
*     if ( munthly structure .eq. 1 ) then ! impose monthly structure on log normal flow removals
*     call impose monthly structure for river flows (1,idirx) ! normal effluent flow removals
*     return
*     endif
...
*     create the  corrected shots -------------------------------------------------
      ...
      subroutine generate log normal bifurcation (GM,GS,EM)! wowowowowowowow
      include 'COMMON DATA.FOR'
...
*     create the  corrected shots -------------------------------------------------
      ...
      imonth = qmonth (is) ! set the month for this shot
      call get the correlated random numbers (IS,R1,R2,R3,R4) ! removed logn eff
      FTMS (is) = Vlogn ( R3, GS, GM, RF3, BM(3), BS(3), EM )
      FTMS (IS) = amax1 ( 0.0, FTMS(IS) )
      enddo
      if ( MONF .gt. 1 ) call shots for removed flows
      return

      !if ( munthly structure .eq. 1 ) then ! monthly structure on bifurcation
      !call impose monthly structure for river flows (0,idirx) ! log normal bifurcation
      !return
      !endif
...
*     call impose monthly structure for river flows (0,idirx) ! normal effluent flows
*     return
*     endif
...
*     impose correlation between added flow and the river flow -----------------
      R3 = CO2 * R1 + R3 * SQRMB(202, 1.0 - CO2 * CO2 ) ! bifurcation ----------
...
*     correct for Monte carlo sampling errors ----------------------------------
   ...
   29 format(55x,'Calculated annual mean quality =',f8.3,12x,
     &'...  Standard deviation =',f8.3)       
      ...
      subroutine write shots for river quality two
      include 'COMMON DATA.FOR'
      if ( QTYPE (JP) .eq. 4 ) return
   ...
   13 format('Downstream shots for river quality for: ',a11)
      write(31,19)
   19 format(140('-'))
      write(31,14) (CMS(JP,IS),IS=1,NS)
   14 format(14F10.4)
      write(31,19)
	write(31,29) C(JP,1), C(JP,2)
   29 format(55x,'Calculated annual mean quality =',f8.3,12x,
     &'...  Standard deviation =',f8.3)       
      write(31,19)
      endif
 ...
 8813 format(144('-')/'CURRENT shots for the flow in the main river',
   ...
   29 Format('Calculated mean flow   =',f8.2/
     ...
     &144('-')/)       
      ...
      subroutine write shots for river flow three
      include 'COMMON DATA.FOR'

      if ( MONF .gt. 1 ) then ! ================================================
      if ( nobigout .le. 0 ) then
      write(31,8813)uname(feeture),feeture
 8813 format(144('-')/'DOWNSTREAM shots for the flow in the main river',
     &' ... ',a40,i3/
     &144('-'))
      write(31,14)(FMS(IS),IS=1,NS)
   14 format(12f12.2)
   ...
   29 Format('Calculated mean flow   =',f8.2/
     ...
     &32('-')/)       
      ...
      subroutine write shots of discharge flow
      include 'COMMON DATA.FOR'
...
*     calculate the annual mean and standard deviation -------------------------
      ...
      do is = 1 , NS
      xM1 = xM1 + EFshots(is)
      xM2 = xM2 + EFshots(is) * EFshots(is)
      enddo
      xM2=(xM2-xM1*xM1/FLOAT(NS))/FLOAT(NS-1)
      if ( xM2 .gt. 1.0e-10 ) then
      xM2=SQRoot(2024,xM2)
      else
      xM2 = 0.0
      endif
...
*     --------------------------------------------------------------------------
      ...
      write(01,29)xm1,xm2
      write(33,29)xm1,xm2
   29 Format('Calculated mean flow   =',f9.3/
     &       'Standard deviation     =',f9.3/150('-'))       
   39 format('Calculated mean flow   =',f12.6,
     &' ... SR write shots of discharge flow'/
     &       'Standard deviation     =',f12.6,
     &' ... Calculated from the shots (EFshots)'/150('-'))       
      endif
...
*     generate monthly structure on data for river flow ------------------------
      subroutine impose monthly structure for river flows (jdir,idirx) ! type 2 or 3
      include 'COMMON DATA.FOR'
      ...
      !write(33,7430)IF,uname(feeture),F(IF,1),F(IF,2),F(IF,2)/F(IF,1)
 7430 format('Impose monthly structure on river flow ... Set Number',i5/
     &'Feature: ',a40/60('-')/
     &'                Annual mean = ',f10.4/
     &'     95-percentile low flow = ',f10.4,
     &' ... ratio',f8.4/60('-'))
      !write(33,7530)BM(1),BS(1)
 7530 format(
     &'     Annual correction (BM) = ',f10.4/
     &'     Annual correction (BS) = ',f10.4)
...
*     ==========================================================================      
      ...
      xxsum = 0.0
      do i = 1,12 ! loop on months
      xxsum = xxsum +  struct1(i)
      enddo
      xxsum = xxsum / 12.0


*     ==========================================================================      
*     compare the ratio with the smallest average monthly flow (July) ========== 
      tratio = t95 / tmean ! ratio of input annual 95-percentile to the mean ---
*     compare ratio of Q95 to mean with ratio of July mean to annual mean ------
      if ( tratio + 0.5 .gt. struct1(7) ) then ! it is bigger than July --------
      tadd = tratio - struct1(7) + 0.08 ! adjustment to monthly means ----------
...
*     set the monthly means ... and set the monthly 95-percentiles -------------
      tmonthly = 2.25
      do i = 1, 12
      struct1 (i) = struct1 (i) * tmean ! monthly mean
      struct2 (i) = struct1 (i) * tratio * tmonthly ! monthly 95-percentile 
      enddo
...
*     correct the monthly data for the number of days in a month ---------------
     ...
     &                days in months (imon)
      struct2(imon) = (365.0/12.0) * struct2(imon) / 
     &                days in months (imon)
...
*     write(01,2398)
 2398 format('Month',15x,'Mean',8x,'95%','      Ratio','   Correlation'/
     &26x,'exeedence',11x,'      (F on f)'/60('-'))
      !write(01,8599)(struct1(i),struct2(i),struct4(i),i=1 ,12)
      !&struct2(i)/struct1(i),struct4(i),i=1 ,12)
 8599 format(
     &'January ...  ',3f11.4,f14.4/
     &'February ... ',3f11.4,f14.4/
     &'March ...    ',3f11.4,f14.4/
     &'April ...    ',3f11.4,f14.4/
     &'May ...      ',3f11.4,f14.4/
     &'June ...     ',3f11.4,f14.4/
     &'July ...     ',3f11.4,f14.4/
     &'August ...   ',3f11.4,f14.4/
     &'September ...',3f11.4,f14.4/
     &'October ...  ',3f11.4,f14.4/
     &'November ... ',3f11.4,f14.4/
     &'December ... ',3f11.4,f14.4)
      !endif ! if ( MONF .gt. 1 ) ===============================================
...
*     sample the distributions of data on river flow using annual data =========
      do 2224 is = 1, NS ! =====================================================
      fdms2 = FRAN function (is) ! get the random normal deviate ---------------
*     compute the value of the shot --------------------------------------------
      FMS(is) = VALFL ( fdms2, GRFS, GRFM,RF3, BM(1), BS(1), RF5 )
      FMS(IS) = amax1 ( 0.0, FMS(IS) ) ! daily flows ---------------------------
      YY(IS) = FMS(IS) ! daily flows -------------------------------------------
 2224 continue ! ===============================================================
      
      call statistics for monthly river flows (1,CM1,CM2)
      
      tmeanan = CM1 ! the calculated mean using annual data --------------------
      t95an = CM2 ! the calculated 95-percentile using annual data -------------
...
*     sample the distributions of river flow using monthly data ================
      ...
      FMS(IS) = amax1 ( 0.0, FMS(IS) ) ! daily flows ---------------------------
      YY(IS) = FMS(IS) ! daily flows -------------------------------------------
      endif ! if ( RFM .gt. 1.0e-9 ) -------------------------------------------
   24 continue ! ===============================================================

      call statistics for monthly river flows (1,CM1,CM2) ! calculate FLOW(1) 

*     set up factors for the next iteration ------------------------------------
      do imon = 1, 12
      BSM(imon) = tmeanan/FLOW(1) ! + 1.0)/2.0 ! for monthly means --------------
      BSS(imon) = t95an/FLOW(2) !+ 1.0)/2.0 ! for monthly 5-percentile ---------
      enddo

      !write(33,3326)FLOW(1),tmeanan,BSM(1),100.0*FLOW(1)/tmeanan,
      !&FLOW(2),t95an,BSS(1),100.0*FLOW(2)/t95an
 3326 format(77('=')/
     &'Starting results for monthly data ...'/77('-')/
     &17x,'    monthly     annual        BSM',17x,'% fit'/
     &17x,'       data       data        BSS'/77('-')/
     &'      Mean flow =',3f11.4,10x,f12.4/
     &'   5-percentile =',3f11.4,10x,f12.4/77('-'))  
...
*     adjust the monthly data on river flow ------------------------------------
      do imonth = 1,12
      struct1 (imonth) = struct1 (imonth) * BSM(1)
      struct2 (imonth) = struct2 (imonth) * BSS(1)
      enddo
      do 22 is = 1, NS
      YY(is) = 0.0
      imonth = qmonth (is) ! set the month for this shot
*     mean and 95-percentile low flow ------------------------------------------
      ...
      fdms2 = FRAN function (is) ! get the random normal deviate ---------------
*     compute the value of the shot --------------------------------------------
      YY(is) = VALFL ( fdms2, GRFS, GRFM,RF3, BM(1), BS(1), RF5 )
   ...
   22 continue
      ...
      oldBSM = BSM(1) ! the last used scaling factor for the mean
      oldBSS = BSS(1) ! the last used scaling factor for the 95-percentile
      do i = 1, 12
      BSM(i) = tmeanan/CM1 ! next values of the scaling factors for the mean
      BSS(i) = t95an/CM2 ! next values of the scaling factors for q95
      enddo
      
      rat1 = CM1 / tmeanan ! ratio of calculated and target mean
      rat2 = CM2 / t95an ! ratio of calculated and target 95-percentile

*     test whether the targets have been met ===================================
      if ( rat1 .lt. 1.002 .and. rat1 .gt. 0.998) then ! target met for mean ===
      if ( rat2 .lt. 1.004 .and. rat2 .gt. 0.996) goto 6668 ! and met for Q95 ==
*     continue interations =====================================================
      if ( iterate .gt. 40 ) then ! end of iterations --------------------------
      if ( rat2 .lt. 1.010 .and. rat2 .gt. 0.990) then
      goto 6668 ! and met for Q95 --
      else
      goto 5558 ! iteration failed   
      endif ! if ( rat2 .lt. 1.010 etc 
      else
      goto 5555 ! proceed to the next iteration --------------------------------
      endif ! if ( iterate .gt. 40 ) -------------------------------------------
      goto 5555 ! proceed to the next iteration --------------------------------
      else ! target for mean not yet met =======================================
      if ( iterate .gt. 40 ) goto 5558 ! iteration failed
      goto 5555 ! proceed to the next iteration --------------------------------
      endif ! if ( rat1 .lt. 1.002 etc =========================================
*     the have been met or all attempts completed ==============================


 5558 continue ! iterations failed =============================================
      idirx = 99
      suppress12 = suppress12 + 1
      write(33,6266)iterate,IF,uname(feeture)
 6266 format(//77('=')/'Unable to place monthly structure on ',
     &'river flow by iteration',i4/
     &'ANNUAL DATA WILL BE IMPOSED ...'/77('-')/
     &'Date set:',i6,'  used at ',a35/77('='))
      if ( ifbatch .ne. 1 .and. kerror .eq. 1 ) then ! -------------------------
      call change colour of text (14) ! bright yellow
      write( *,5466)IF,JT(feeture)
 5466 format(
     &'*** Unable to impose monthly structure ',
     &'on river flow set:',i7,33x,'Data-type:',i4,' ...')
      call set screen text colour
      endif ! if ( ifbatch .ne. 1 .and. kerror .eq. 1 ) ------------------------
      return ! revert to the use of annual data ================================
 
     
 6668 continue ! convergence reached ===========================================
      write(33,6211)iterate,IF,uname(feeture)
 6211 format(//77('=')/'Convergence achieved for river flow by',
     &' iteration',i4/
     &'Monthly data are imposed ...'/77('-')/
     &'Date set:',i6,'  at ',a35/77('='))
...
*     initialise the concentrations --------------------------------------------
      ...
      do IP = 1, n2prop ! -------- initialise shots for various types of sources
      CTMS(IP,JP,IS) = 0.0 ! concentrations from various types of feature ------
      UCTMS(IP,JP,IS) = 0.0 ! u/s concentrations from various types of feature -
      enddo ! do IP = 1, n2prop ------------------------------------- initialise
      enddo
      ...
      do idet = 1, mp10
      do ii = 1,5    
      CMcontrib(ip,idet,ii) = 0.0 ! contribution to concentration
      enddo
      do ii = 1,2    
      LMcontrib(ip,idet,ii) = 0.0 ! contribution to load
      enddo
      enddo
      enddo
...
*     special correlation ... river quality on river flow ----------------------
      ...
      QUALNB(JP,17) = QNUM(IQ,JP) ! ------------------- at the head of the reach
...
*     accumulate the river derived load --------------- at the head of the reach
      ...
      do is = 1, NS
      CTMS(17,JP,IS) = CMS(JP,IS) ! --------------- concentrations from boundary 
      CTMS(NTD,JP,IS) = CMS(JP,IS) ! -------------- concentrations from boundary
      enddo
...
*     ==========================================================================
      idnum = 17
      do IS = 1,NS
      Y(IS) = CTMS(idnum,JP,IS)
      enddo
      call calculate summaries of contribution (idnum,JP,Y,0) ! --- still needed
      idnum = 30
      do IS = 1,NS
      Y(IS) = CTMS(idnum,JP,IS)
      enddo
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      fupmx = 0.0
      xnum = qnum(IQ,JP) ! for headwaters
      call get sampling rates for river quality ! headwaters (10) --------------
     &(fupmx,CMcontrib(idnum,JP,1),
     &QUALNB(JP,idnum),F(IF,1),F(IF,1),xnum,IQDIST,1)
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
*     add in any specified diffuse sources ===================================== 
      ...
      xdiffnum31 = 0.0 ! net sampling for total diffuse ========================
...
*     DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
      ...
      xnum = qnum(idqual,JP) ! sampling for added diffuse to headwaters ========
      QUALNB(JP,idnum) = xnum ! sampling for added diffuse to headwaters =======
...
*     calculate the load added to the headwaters DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
      ...
      call calculate summaries of river quality (0,JP,Y)
 ...
 4290 format(77('+')/
     ...
     &38x,    '            Set number =',i10)
      write(200+JP,4290)nameprop(idnum),ECM,LUNITS(JP),ECS,
     &LUNITS(JP),dname(JP),PDRC(idqual,JP),idqual
 ...
 4200 format(77('+')/
     ...
     &a11,27x,'          Type of data =',i10/
     &38x,    '            Set number =',i10)
      write(200+JP,4200)nameprop(idnum),ECM,UNITS(JP),ECS,
     &UNITS(JP),dname(JP),PDRC(idqual,JP),idqual
...
*     xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 6231 format(77('=')/'Data-type:',i3,' is requested for ',a37/
     ...
     &'the reach headwaters ...'/77('='))
      write(200+JP,6231)IQDIST,nameprop(idnum) 
...
*     ==========================================================================
     ...
     &.and. IMDIST5 .ne. 7 ) then ! for concentrations -------------------------
      ...
      CTMS(idnum,JP,IS) = EC ! --------- set the added the diffuse concentration
      CTMS(NTD,JP,IS) = CTMS(NTD,JP,IS) + EC ! --- add the diffuse concentration
      CTMS(NTF,JP,IS) = CTMS(NTF,JP,IS) + EC ! --- add the diffuse concentration 
      ...
      CTMS(idnum,JP,IS) = EC/FMS(IS) ! - set the added the diffuse concentration
      CTMS(NTD,JP,IS) = CTMS(NTD,JP,IS) + EC/FMS(IS) ! add diffuse concentration
      CTMS(NTF,JP,IS) = CTMS(NTF,JP,IS) + EC/FMS(IS) ! add diffuse concentration
      endif ! if ( FMS(IS) .gt. 0.00001 ) --------------------------------------
      DLX = EC / float(NS) ! load added to the headwaters ======================
      diffuse load (JP,I13) = diffuse load (JP,I13) + DLX ! added as loads =====
      endif ! if ( IQDIST .ne. 7, 6 etc ----------------------------------------
...
*     ==========================================================================
      idnum2 = NTF ! total from all types of diffuse ---------------------------
      xCMcont = CMcontrib(idnum2,JP,1)
      xxCMcont = 0.0
      xxCScont = 0.0
      do IS = 1,NS
      ...
      xdiffload = xdiffload / float (NS) ! mean added load
      
      xnum = ((QUALNB(JP,idnum2)*xCMcont) 
     &     + (xnum*(CMcontrib(idnum2,JP,1)-xCMcont)))
     &     / CMcontrib(idnum2,JP,1)
      
      QUALNB(JP,idnum2) = xnum
*     ==========================================================================
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 6316 format(77('-')/45x,'Mean load added =',f10.1,1x,a4/77('='))
      write(200+JP,6316)xdiffload,LUNITS(JP)
      ...
      xxxload = xxxload + CMS(JP,IS) * FMS(IS)
      enddo
      xxxload = xxxload / float(NS)
      call calculate summaries of river quality (0,JP,Y)
      
      write(01,8230)dname(JP),
     &C(JP,1),UNITS(JP),C(JP,2),UNITS(JP),xxxload,LUNITS(JP)
 8230 format(
     &a11,26x,   ' Resulting mean quality =',f10.2,1x,a4/
     &37x,       '     Standard deviation =',f10.2,1x,a4/!77('-')/
     &37x,       '   Resulting total load =',f10.1,1x,a4/77('='))
      write(200+JP,8230)dname(JP),
     &C(JP,1),UNITS(JP),C(JP,2),UNITS(JP),xxxload,LUNITS(JP)
...
*     calculate loads and the summary statistics of load ! --- head of the reach
      ...
      call update summaries of contribution ! --------- at the head of the reach
...
*     initialise the concentrations ------------------------ for input to a lake
      do IS = 1, NS
      CMS(JP,IS) = 0.0
      do IP = 1, n2prop ! ---------------------------------- for input to a lake
      CTMS(IP,JP,IS) = 0.0 ! concentrations from various types of feature ------
      UCTMS(IP,JP,IS) = 0.0 ! u/s concentrations from various types of feature -
      enddo ! do IP = 1, n2prop ! -------------------------- for input to a lake
      enddo
...
*     accumulate the river derived load ... for input to a lake ============ 158
      do jdet = 1, NDET
...
*     generate river quality data for lake outflow into a river ---------------
   ...
   14 format(77('-')/'*** Illegal quality data for flow',
     ...
     &'*** Value entered for the set of data is',i5/77('-'))
      ...
      do IP = 1, n2prop ! ============================ lake outflow into a river
      CTMS(IP,JP,IS) = 0.0 ! concentrations from various types of feature ------
      UCTMS(IP,JP,IS) = 0.0 ! u/s concentrations from various types of feature -
      enddo ! do IP = 1, n2prop ! ============================= for lake outflow
      enddo ! do IS = 1, NS
...
*     generate river quality data for lake outflow------------------------------
      ...
      do IS = 1, NS
      CTMS(17,JP,IS) = CMS(JP,IS) ! ------- concentrations from the lake outflow
      call add up all the loads !  ! for all determinands -- for input to a lake
      enddo
...
*     accumulate the river derived load ... lake outflow into a river ====== 158
      do jdet = 1, NDET
      if ( QTYPE (jdet) .ne. 4 ) then
      nx = n13
      if ( munthly structure .eq. 0 ) nx = 1 ! no requirement for monthly loads
