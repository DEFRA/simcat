...
*     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 161
*     write out comma-separated concentrations ------------------------- DAi.CSV
      write(180+jper,1300)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &Length of main river,C(jper,1),C(jper,2), ! ---------------------- DAi.CSV
     &XL(13),XU(13),unamex ! ------------------------------------------- DAi.CSV
 1300 format(' ',a40,',',i4,',',a40,',',a20,',', ! --------------------- DAi.CSV
     &'River concentration:',',','Mean etc.', ! ------------------------ DAi.CSV
     &',',0pf12.2,4(',',1pe12.4),11(','),'99999,,,','At: ',a40) ! ------ DAi.CSV
*     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 161
...
*     write out comma-separated tables of exceedence ----------------------- 161
      write(180+jper,4401)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),Length of main river,exceedences50(jper,13),
     &(exceedences50(jper,im), im = 1,12),unamex
 4401 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'% days exceeding annual mean concentration',',',a11, ! ---------- DAi.CSV
     &','0pf12.2,(',',0pf12.2,'%'),12(',',0pf12.2,'%'),',,', ! --------- DAi.CSV
     &'99999,',',,At: ',a40) ! ----------------------------------------- DAi.CSV
     
      write(180+jper,4402)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),Length of main river,exceedences90(jper,13),
     &(exceedences90(jper,im), im = 1,12),unamex
 4402 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'% days exceeding annual 90-percentile',',',a11, ! --------------- DAi.CSV
     &','0pf12.2,(',',0pf12.2,'%'),12(',',0pf12.2,'%'),',,', ! --------- DAi.CSV
     &'99999,',',,At: ',a40) ! ----------------------------------------- DAi.CSV
      write(180+jper,4403)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),Length of main river,exceedences95(jper,13),
     &(exceedences95 (jper,im), im = 1,12),unamex
 4403 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'% days exceeding annual 95-percentile',',',a11, ! --------------- DAi.CSV
     &','0pf12.2,(',',0pf12.2,'%'),12(',',0pf12.2,'%'),',,', ! --------- DAi.CSV
     &'99999,',',,At: ',a40) ! ----------------------------------------- DAi.CSV
*    ----------------------------------------------------------------------- 161
...
*     write out comma-separated monthly loads ------------------------------ 161
      write(180+jper,2404)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &Length of main river,lmon(jper,1,13),
     &lmon(jper,2,13),XL(13),XU(13),unamex
 2404 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Load in the river:',',','Mean etc.', ! -------------------------- DAi.CSV
     &(',',0pf12.2),4(',',1pe12.4),11(','),'99999,,,','At: ',a40)  ! --- DAi.CSV
*     ---------------------------------------------------------------------- 161
...
*     endif ! if ( cpart1(jper,1) .gt. 1.0e-12 )
 ...
 1206 format('Plus or minus ...   ',13(a7,'%')) ! ----------------------- Di.MON
      ...
      if ( iplace .ne. 3 ) then ! -------------------------------------- DAi.CSV
      write(180+jper,1300)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(p1mon(jper,1,i),i=1,12),cpart1(jper,1),jt(feeture) !  DAi.CSV
 1300 format(' ',a40,',',i4,',',a40,',',a20,',','PARTS for ',',',a11, ! -DAi.CSV
     &13(',',1pe12.4),',',i4) ! ---------------------------------------- DAi.CSV
      write(180+jper,1301)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(p1mon(jper,2,i),i=1,12),cpart1(jper,2),jt(feeture) !  DAi.CSV
 1301 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Standard deviation for ',',',a11,13(',',1pe12.4),',',i4) ! ------ DAi.CSV
      write(180+jper,1305)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(XL(i),i=1,13),jt(feeture)
 1305 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Lower limit on dissolved ',',',a11,13(',',1pe12.4),',',i4) ! ---- DAi.CSV
      write(180+jper,1306)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(XU(i),i=1,13),jt(feeture)
 1306 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Upper limit on dissolved ',',',a11,13(',',1pe12.4),',',i4) ! ---- DAi.CSV

      if ( iplace .eq. 3 ) then
      unamex = 'End of Reach'
      write(180+jper,1400)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(p1mon(jper,1,i),i=1,12),cpart1(jper,1),jt(feeture)
 1400 format(' ',a40,',',i4,',',a40,',',a20,',','PARTS for ',',',a11, !  DAi.CSV
     &13(',',1pe12.4),',',i4) ! ---------------------------------------- DAi.CSV
      write(180+jper,1401)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(p1mon(jper,2,i),i=1,12),cpart1(jper,2),jt(feeture)
 1401 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Standard deviation for ',',',a11,13(',',1pe12.4),',',i4) ! ------ DAi.CSV
      write(180+jper,1405)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(XL(i),i=1,13),jt(feeture)
 1405 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Lower limit on dissolved ',',',a11,13(',',1pe12.4),',',i4) ! ---- DAi.CSV
      write(180+jper,1406)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(XU(i),i=1,13),jt(feeture)
...
*     --------------------------------------------------------------------------
 ...
 1301 format(' ',a40,',',i4,',',a40,',',a20,',',
     ...
     &dname(jper),(XL(i),i=1,13),jt(feeture)
 1305 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Lower limit on solid ',',',a11,13(',',1pe12.4),',',i4) ! -------- DAi.CSV
      write(180+jper,1306)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(XU(i),i=1,13),jt(feeture)
 1306 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Upper limit on solid ',',',a11,13(',',1pe12.4),',',i4) ! -------- DAi.CSV
      endif
      ...
      write(180+jper,1400)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(P2mon(jper,1,i),i=1,12),cpart2(jper,1),jt(feeture)
 1400 format(' ',a40,',',i4,',',a40,',',a20,',','PARTS for ', ! -------- DAi.CSV
     &',',a11,13(',',1pe12.4),',',i4) ! -------------------------------- DAi.CSV
      write(180+jper,1401)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &dname(jper),(P2mon(jper,2,i),i=1,12),cpart2(jper,2),jt(feeture)
 1401 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Standard deviation for ',',',a11,13(',',1pe12.4),',',i4) ! ------ DAi.CSV
...
*     set the month for this shot ----------------------------------------------
   ...
   16 CM = CM/NS
     ...
     &(jper,ipt)
      include 'COMMON DATA.FOR'
      real lcontot(13)
      dimension CS(12),CML(13),CMU(13) ! ----------------------------------- 161
...
*     net loads from discharges (3 12 5 39 60 61 62) ************* LMcontrib(01)
      call munthly split (01,jper,lcontot,CS,CML,CMU)
      write(180+jper,9188)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(1),Length of main river,LMcontrib(01,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9188 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','3125396061,',',,', ! --------- DAi.CSV
     &'At: ',a37)
   
*     net load from sewage works (3) ***************************** LMcontrib(02)
      call munthly split (02,jper,lcontot,CS,CML,CMU)
      write(180+jper,9928)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(2),Length of main river,LMcontrib(02,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9928 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','3,',',,','At: ',a37) ! ------- DAi.CSV
    
*     net load from intermittent discharges (12) ***************** LMcontrib(03)
      call munthly split (03,jper,lcontot,CS,CML,CMU)
      write(180+jper,9929)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(3),Length of main river,LMcontrib(03,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9929 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','12,',',,','At: ',a37) ! ------ DAi.CSV

*     industrial discharges (5) ********************************** LMcontrib(04)
      call munthly split (04,jper,lcontot,CS,CML,CMU)
      write(180+jper,9951)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(4),Length of main river,LMcontrib(04,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9951 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','5,',',,','At: ',a37) ! ------- DAi.CSV
     
*     mine waters (39) ******************************************* LMcontrib(05)
      call munthly split (05,jper,lcontot,CS,CML,CMU)
      write(180+jper,9730)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(5),Length of main river,LMcontrib(05,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9730 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','39,',',,','At: ',a37) ! ------ DAi.CSV
     
*     Other Point Sources (60) *********************************** LMcontrib(28)
      call munthly split (28,jper,lcontot,CS,CML,CMU)
      write(180+jper,9630)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(28),Length of main river,LMcontrib(28,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9630 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','60,',',,','At: ',a37) ! ------ DAi.CSV
     
*     private wastewaters (61) *********************************** LMcontrib(29)
      call munthly split (29,jper,lcontot,CS,CML,CMU)
      write(180+jper,9230)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(29),Length of main river,LMcontrib(29,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9230 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','61,',',,','At: ',a37) ! ------ DAi.CSV
     
*     fish farms (62) ******************************************** LMcontrib(30)
      call munthly split (30,jper,lcontot,CS,CML,CMU) ! fish farms
      write(180+jper,9262)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(30),Length of main river,LMcontrib(30,jper,1),
     &(lcontot(J13),J13=1,12),unamex
 9262 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','62,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (18,jper,lcontot,CS,CML,CMU)
      write(180+jper,9909)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(18),Length of main river,LMcontrib(18,JDET,1),
     &(lcontot(J13),J13=1,12),unamex
 9909 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','13,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (06,jper,lcontot,CS,CML,CMU)
      write(180+jper,9910)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(6),Length of main river,LMcontrib(06,jper,1), ! -- DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9910 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','25,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (07,jper,lcontot,CS,CML,CMU)
      write(180+jper,9911)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(7),Length of main river,LMcontrib(07,jper,1), ! -- DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9911 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','27,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (08,jper,lcontot,CS,CML,CMU)
      write(180+jper,9912)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(8),Length of main river,LMcontrib(08,jper,1), ! -- DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9912 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','29,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (09,jper,lcontot,CS,CML,CMU)
      write(180+jper,9913)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(9),Length of main river,LMcontrib(09,jper,1), ! -- DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9913 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','31,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (10,jper,lcontot,CS,CML,CMU)
      write(180+jper,9914)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(10),Length of main river,LMcontrib(10,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9914 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','33,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (11,jper,lcontot,CS,CML,CMU)
      write(180+jper,9915)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(11),Length of main river,LMcontrib(11,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9915 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','35,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (12,jper,lcontot,CS,CML,CMU)
      write(180+jper,9916)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(12),Length of main river,LMcontrib(12,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9916 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','37,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (13,jper,lcontot,CS,CML,CMU)
      write(180+jper,9116)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(13),Length of main river,LMcontrib(13,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9116 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','40,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (14,jper,lcontot,CS,CML,CMU)
      write(180+jper,9922)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(14),Length of main river,LMcontrib(14,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9922 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','42,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (15,jper,lcontot,CS,CML,CMU)
      write(180+jper,9925)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(15),Length of main river,LMcontrib(15,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9925 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','46,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (16,jper,lcontot,CS,CML,CMU)
      write(180+jper,9225)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(16),Length of main river,LMcontrib(16,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9225 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','48,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (23,jper,lcontot,CS,CML,CMU)
      write(180+jper,9505)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(23),Length of main river,LMcontrib(23,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9505 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','50,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (24,jper,lcontot,CS,CML,CMU)
      write(180+jper,9525)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(24),Length of main river,LMcontrib(24,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9525 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','52,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (25,jper,lcontot,CS,CML,CMU)
      write(180+jper,9545)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(25),Length of main river,LMcontrib(25,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9545 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','54,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (26,jper,lcontot,CS,CML,CMU)
      write(180+jper,9565)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(26),Length of main river,LMcontrib(26,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9565 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','56,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (27,jper,lcontot,CS,CML,CMU)
      write(180+jper,9585)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(27),Length of main river,LMcontrib(27,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9585 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ',',',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','58,',',,','At: ',a37) ! ------ DAi.CSV

      call munthly split (19,jper,lcontot,CS,CML,CMU)
      write(180+jper,9917)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(19),Length of main river,LMcontrib(19,JDET,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9917 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ,',a37, ! diffuse input (feature type 15)
     &(',',0pf12.2),13(',',1pe12.4),',,','19,',',,','At: ',a37) ! ------ DAi.CSV
     
      call munthly split (17,jper,lcontot,CS,CML,CMU)
      write(180+jper,9180)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(17),Length of main river,LMcontrib(17,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9180 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ,',a37, ! boundaries and tributaries
     &(',',0pf12.2),13(',',1pe12.4),',,','2101315,',',,','At: ',a37) ! - DAi.CSV
*     --------------------------------------------------------------------------
      call munthly split (20,jper,lcontot,CS,CML,CMU)
      write(180+jper,9181)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(20),Length of main river,LMcontrib(20,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9181 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load from: ,',a37, ! reach diffuse input
     &(',',0pf12.2),13(',',1pe12.4),',,','500,',',,','At: ',a37) ! ----- DAi.CSV
*     --------------------------------------------------------------------------
      write(180+jper,9182)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,Length of main river,TNLOADUP2(jper,1), ! ----------------- DAi.CSV
     &(TNLOADUP2(jper,J13),J13=2,N13),unamex
 9182 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load added from: ,','Natural purification',
     &(',',0pf12.2),13(',',1pe12.4),',,','600,',',,','At: ',a37) ! ----- DAi.CSV
     
      call munthly split (21,jper,lcontot,CS,CML,CMU)
      write(180+jper,9186)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(21),Length of main river,LMcontrib(21,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9186 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load added by: ,',a37, ! gap filling for flows
     &(',',0pf12.2),13(',',1pe12.4),',,','700,',',,','At: ',a37) ! ----- DAi.CSV
     
      call munthly split (22,jper,lcontot,CS,CML,CMU)
      write(180+jper,9184)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(22),Length of main river,LMcontrib(22,jper,1), ! - DAi.CSV
     &(lcontot(J13),J13=1,12),unamex
 9184 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Net load added by: ,',a37, ! gap filling for quality
     &(',',0pf12.2),13(',',1pe12.4),',,','800,',',,','At: ',a37) ! ----- DAi.CSV
     
*     load lost by abstractions **************************************** DAi.CSV
      write(180+jper,9918)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,Length of main river,TBLODE2(jper,1), ! ------------------- DAi.CSV
     &(TBLODE2(jper,J13),J13=2,N13),unamex
 9918 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Load lost to: ,','Abstractions',
     &(',',0pf12.2),13(',',1pe12.4),',,','71819,',',,','At: ',a37) ! --- DAi.CSV
*     load lost by abstractions **************************************** DAi.CSV

*     load lost by natural purification ******************************** DAi.CSV
      write(180+jper,9183)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,Length of main river,TNLOADDN2(jper,1), ! ----------------- DAi.CSV
     &(TNLOADDN2(jper,J13),J13=2,N13),unamex
 9183 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Load lost to: ,','Natural purification',
     &(',',0pf12.2),13(',',1pe12.4),',,','601,',',,','At: ',a37) ! ----- DAi.CSV
*     load lost by natural purification ******************************** DAi.CSV
     
*     load lost by gap-filling of flows ******************************** DAi.CSV
      write(180+jper,9187)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(21),Length of main river,TILOADDN2(jper,1), ! ---- DAi.CSV
     &(TILOADDN2(jper,J13),J13=2,N13),unamex
 9187 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Load lost by: ,',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','701,',',,','At: ',a37) ! ----- DAi.CSV
*     load lost by gap-filling of flows ******************************** DAi.CSV
     
*     load lost by gap-filling of quality ****************************** DAi.CSV
      write(180+jper,9185)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,nameprop(22),Length of main river,TALOADDN2(jper,1), ! ---- DAi.CSV
     &(TALOADDN2(jper,J13),J13=2,N13),unamex
 9185 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'Load lost by: ,',a37,
     &(',',0pf12.2),13(',',1pe12.4),',,','801,',',,','At: ',a37) ! ----- DAi.CSV
*     load lost by gap-filling of quality ****************************** DAi.CSV
     
*     total load lost ************************************************** DAi.CSV
      write(180+jper,9195)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,dname(jper),Length of main river,TLOSSES2(jper,1), ! ------ DAi.CSV
     &(TLOSSES2(jper,J13),J13=2,N13),unamex
 9195 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'TOTAL LOAD REMOVED',',',a11,
     &(',',0pf12.2),13(',',1pe12.4),',,','802,',',,','At: ',a37) ! ----- DAi.CSV
*     **************************************************************************

*     **************************************************************************
      call munthly split (NTD,jper,lcontot,CS,CML,CMU) ! total loads
      write(180+jper,9919)GIScode(feeture),jxtype,unamex, ! ------------ DAi.CSV
     &rnamex,dname(jper),Length of main river,LMcontrib(NTD,jper,1), ! - DAi.CSV  
     &(lcontot(J13),J13=1,12),unamex
 9919 format(' ',a40,',',i4,',',a40,',',a20,',',
     &'NET TOTAL OF ALL LOADS',',',a11,
     &(',',0pf12.2),13(',',1pe12.4),',,','999,',',,','At: ',a37) ! ----- DAi.CSV
*     **************************************************************************
...
*     running total load introduced by upstream boundaries and tributaries -----
      if ( abs (LMcontrib(17,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars (13),LMcontrib(17,jper,1))
      write(100+jper,3000)(Tenchars (i),i=1,13)
 3000 format('Boundaries and streams (2,10) ',13a10)
      endif
*     total load introduced by clean diffuse sources ------------- LMcontrib(20)
      if ( abs (LMcontrib(20,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars (13),LMcontrib(20,jper,1))
...
*     added by flow gap filling ---------------------------------- LMcontrib(21)
      if ( ical .gt. 3 .or. ical .eq. 2 ) then
      call set format for printout(Tenchars(13),LMcontrib(21,jper,1))
      write(100+jper,3006)Tenchars(13)
 3006 format('Added by flow gap filling     ',13a10)
      endif

*     added by quality gap filling for --------------------------- LMcontrib(22)
      if ( ical .gt. 3 ) then
      call set format for printout(Tenchars (13),LMcontrib(22,jper,1))
      write(100+jper,3004)Tenchars(13)
 3004 format('Added by quality gap filling  ',13a10)
      endif

*     net loads from all discharges (3 12 5, etc) ---------------- LMcontrib(01)
      call set format for printout(Tenchars(13),LMcontrib(01,jper,1))
      write(100+jper,3208)Tenchars(13)
 3208 format('Net discharges (3 12 5 and 39)',13a10)
*     net load from sewage works (3) ----------------------------- LMcontrib(02)
      if ( abs (LMcontrib(02,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(02,jper,1))
      write(100+jper,3028)Tenchars(13)
 3028 format('Sewage works (3)   ',11x,13a10)
      endif
*     intermittent discharges (12) ------------------------------- LMcontrib(03)
      if ( abs (LMcontrib(03,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(03,jper,1))
      write(100+jper,3029)Tenchars(13)
 3029 format('Intermittent discharges (12)',2x,13a10)
      endif
*     industrial discharges (5) ---------------------------------- LMcontrib(04)
      if ( abs (LMcontrib(04,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(04,idet,1))
      write(100+jper,3051)Tenchars(13)
 3051 format('Industrial discharges (5)   ',2x,13a10)
      endif
*     mine waters (39) ------------------------------------------- LMcontrib(05)
      if ( abs (LMcontrib(05,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(05,jper,1))
      write(100+jper,3030)Tenchars(13)
 3030 format('Mine waters (39)            ',2x,13a10)
      endif
*     Other Point Sources (60) ----------------------------------- LMcontrib(28)
      if ( abs (LMcontrib(28,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(28,jper,1))
      write(100+jper,3930)Tenchars(13)
 3930 format('"Other" Point Sources (60)    ',2x,13a10)
      endif
*     private wastwaters (61) ------------------------------------ LMcontrib(29)
      if ( abs (LMcontrib(29,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(29,jper,1))
      write(100+jper,3830)Tenchars(13)
 3830 format('Private wastewaters (61)    ',2x,13a10)
      endif
*     fish farms (62) -------------------------------------------- LMcontrib(30)
      if ( abs (LMcontrib(30,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(30,jper,1))
      write(100+jper,3862)Tenchars(13)
 3862 format('Fish farms (62)             ',2x,13a10)
      endif
*     diffuse (river-type) (13) ---------------------------------- LMcontrib(18)
      if ( abs (LMcontrib(18,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(18,jper,1))
      write(100+jper,3009)Tenchars(13)
 3009 format('Diffuse (river-type) (13)     ',13a10)
      endif
*     livestock (25) --------------------------------------------- LMcontrib(06)
      if ( abs (LMcontrib(06,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(06,jper,1))
      write(100+jper,3010)Tenchars(13)
 3010 format('Livestock (25)                ',13a10)
      endif
*     arable (27) ------------------------------------------------ LMcontrib(07)
      if ( abs (LMcontrib(07,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(07,jper,1))
      write(100+jper,3011)Tenchars(13)
 3011 format('Arable (27)                   ',13a10)
      endif
*     highway (29) ----------------------------------------------- LMcontrib(08)
      if ( abs (LMcontrib(08,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(08,jper,1))
      write(100+jper,3012)Tenchars(13)
 3012 format('Highway (29)                  ',13a10)
      endif
      
*     urban (31) ------------------------------------------------- LMcontrib(09)
      if ( abs (LMcontrib(09,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(09,jper,1))
      write(100+jper,3013)Tenchars(13)
 3013 format('Urban (31)                    ',13a10)
      endif
      
*     atmosphere (33) -------------------------------------------- LMcontrib(10)
      if ( abs (LMcontrib(10,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(10,jper,1))
      write(100+jper,3014)Tenchars(13)
 3014 format('Atmosphere (33)               ',13a10)
      endif
      
*     natural background (35) --------------------------------------------------
      if ( abs (LMcontrib(11,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(11,jper,1))
      write(100+jper,3015)Tenchars(13)
 3015 format('Natural background (35)       ',13a10)
      endif
      
*     septic tanks (37) --------------------------------------------------------
      if ( abs (LMcontrib(12,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(12,jper,1))
      write(100+jper,3016)Tenchars(13) ! -------------------------------- Di.MON
 3016 format('Septic tanks (37)             ',13a10)
      endif
      
*     aggregate CSOs (40) ------------------------------------------------------
      if ( abs (LMcontrib(13,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(13,jper,1))
      write(100+jper,3216)Tenchars(13)
 3216 format('Aggregate CSOs (40)           ',13a10)
      endif
*     Diffuse (effluent-type) (15) --------------------------------------------
      if ( abs (LMcontrib(19,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(19,jper,1))
      write(100+jper,3017)Tenchars(13)
 3017 format('Diffuse (effluent-type) (15)  ',13a10)
      endif
      
*     aggregate STWs (42) ------------------------------------------------------
      if ( abs (LMcontrib(14,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(14,jper,1))
      write(100+jper,3817)Tenchars(13)
 3817 format('Aggregate STWs (42)           ',13a10)
      endif
 
*     Diffuse mines (46) -------------------------------------------------------
      if ( abs (LMcontrib(15,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(15,jper,1))
      write(100+jper,3815)Tenchars(13)
 3815 format('Diffuse mines (46)            ',13a10)
      endif
 
*     Birds, boats and angling (48) --------------------------------------------
      if ( abs (LMcontrib(16,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(16,jper,1))
      write(100+jper,3885)Tenchars(13)
 3885 format('Birds, boats and angling (48) ',13a10)
      endif
      
*     ------------------------ (50) --------------------------------------------
      if ( abs (LMcontrib(23,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(23,jper,1))
      write(100+jper,3505)Tenchars(13)
 3505 format('---------------------- (50) ',13a10)
      endif
      
*     ------------------------ (52) --------------------------------------------
      if ( abs (LMcontrib(24,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(24,jper,1))
      write(100+jper,3525)Tenchars(13)
 3525 format('---------------------- (52) ',13a10)
      endif
*     ------------------------ (54) --------------------------------------------
      if ( abs (LMcontrib(25,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(25,jper,1))
      write(100+jper,3545)(Tenchars (i),i=1,13)
 3545 format('---------------------- (54) ',13a10)
      endif
      
*     ------------------------ (56) --------------------------------------------
      if ( abs (LMcontrib(26,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(26,jper,1))
      write(100+jper,3565)Tenchars(13)
 3565 format('---------------------- (56) ',13a10)
      endif
      
*     ------------------------ (58) --------------------------------------------
      if ( abs (LMcontrib(27,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars(13),LMcontrib(27,jper,1))
      write(100+jper,3585)Tenchars(13)
...
*     Headwaters and tributaries (10) -------------------------------------------
      if ( abs (LMcontrib(20,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars (13),LMcontrib(20,jper,1))
      write(100+jper,3865)(Tenchars (i),i=1,13) ! -------------------------- MON
 3865 format('Headwater and streams (10)    ',13a10)
      endif

*     total-------------------------------------------------------- LMcontrib(30)
      if ( abs(LMcontrib(NTD,jper,1)) .gt. 0.000001) then
      call set format for printout(Tenchars (13),LMcontrib(NTD,jper,1))
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 1277 format('End of reach: ',a16)
      ...
      write(180+jper,8100)GIScode(feeture),jxtype,unamex,rnamex, ! ----- DAi.CSV
     &Length of main river,FLOW(1),Flow(2),unamex ! -------------------- DAi.CSV
 8100 format(' ',a40,',',i4,',',a40,',',a20,',', ! --------------------- DAi.CSV
     &'River flow:',',','Mean and 95-percentile', ! -------------------- DAi.CSV    
     &(',',0pf12.2),2(',',1pe12.4),13(','),'99999,,,','At: ',a40) ! ---- DAi.CSV
