...
*     ==========================================================================
*     --------------------------------------------------------------------------
*     File: mass balance.for ... 2321 lines ------------------------------------
*     --------------------------------------------------------------------------
...
*     apply to all discharges (3 5 12 39 60 61) ------------ point source inputs
     ...
     &     JT(feeture) .eq. 60 .or. JT(feeture) .eq. 61) then ! add to total ---
      LMS(1,JP,IS) = ((RF * RLC(01)) + !! ------- add to total for all effluents
     &               (EFMB * EC) ) / (RF + EF)
      else ! for other inputs --------------------------------------------------
      LMS(1,JP,IS) = ( (RF * RLC(1)) ) / (RF + EF) !  dilute dilute other inputs
      endif ! apply to discharges (3 5 12 39 60 61 ) ------- point source inputs
*     add to the totals ------------------------------------ point source inputs
*     TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
*     ==========================================================================
      if ( JT(feeture) .eq. 03 ) idnum = 2 ! ------------------ sewage works (3)
      if ( JT(feeture) .eq. 12 ) idnum = 3 ! ------ intermittent discharges (12)
      if ( JT(feeture) .eq. 05 ) idnum = 4 !  -------- industrial discharges (5)
      if ( JT(feeture) .eq. 39 ) idnum = 5 ! --------- apply to mine waters (39)
      if ( JT(feeture) .eq. 02 ) idnum = 17  ! ------------ apply to streams (2)
      if ( JT(feeture) .eq. 60 ) idnum = 28  ! -------- Other Point Sources (60)
      if ( JT(feeture) .eq. 61 ) idnum = 29  ! -------- private wastewaters (61)
      LMS(idnum,JP,IS) = ( (RF * RLC(idnum)) + (EFMB * EC) ) / (RF + EF)
      do ip = 1, nprop 
      if ( ip .ne. idnum ) then 
      if ( ip .ne. 1 ) then
...
*     ==========================================================================
      ...
      LMS(idnum,JP,IS) = ( (RF * RLC(idnum)) + (EFMB * EC) ) / (RF) ! ++++++++++     
      endif ! if ( add conc .eq. ++++++++++++++++++++++++++++++++ diffuse inputs
...
*     dilute the concentrations from the other sources of pollution ------------      
      ...
      if ( add conc .eq. 0 ) then ! ------------ diffuse inputs with added flows   
      LMS(IP,JP,IS) = (RF * RLC(IP)) / (RF + EF) ! ------- dilute this pollution
...
*     assess loads contributing to percentiles 3333 apportionment of percentiles 
      if ( n251 .eq. 1 .and. JP .eq. ndshot ) then ! 333333333333333333333333333
      call identify the shots for percentiles (JP) ! of river quality 3333333333
            
      if ( JT(feeture) .eq. 03 .or. JT(feeture) .eq. 05 .or. ! 33333333333333333
     &     JT(feeture) .eq. 12 .or. JT(feeture) .eq. 39 .or. ! 33333333333333333
     &     JT(feeture) .eq. 60 .or. JT(feeture) .eq. 61) then ! 3333333333333333
      ...
      TELOADshots(kountworks,JP,IS) = Creg(3,is) * Creg(4,is) ! store loads 333
      enddo ! 3333333333333333333333333333333333333 apportionment of percentiles
      endif ! saved the loads for back-tracking 333 apportionment of percentiles
      endif ! if ( JT(feeture) .eq. 3 etc ! 3333333 apportionment of percentiles
...
*     monthly distribution -----------------------------------------------------
 ...
 5513 format(33x,77('-')/33x,A11,8x,
    ...
    7 format(/77('-')/'Correlation coefficients set for ',
     &a11,' at ',a37/77('-'))
      write(01,1)CO1
