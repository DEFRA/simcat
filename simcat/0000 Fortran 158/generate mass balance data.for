...
*     ==========================================================================
*     File: generate mass balance data.for ... 6589 lines ----------------------
*     --------------------------------------------------------------------------
...
*     for a discharge ----------------------------------------------------------
      ...
      if ( JTYPE .eq. 62) call non parametric effluent flows
...
*     effluents ----------------------------------------------------------------
      ...
      if ( JTYPE .eq. 62) call non parametric discharge quality
...
*     called from mass balance ---------------------------------------------------
      ...
      if ( JTYPE .eq. 62 ) call monthly discharge flow
...
*     called from mass balance -------------------------------------------------
      ...
      if ( JTYPE .eq. 62 ) return ! fish farm
...
*     for a stream -------------------------------------------------------------
 ...
 3725 format(77('s')/'Generate monthly structure for ',
     &'stream flows at: ',a35)
      if (JP.eq.ndetfirst.and.numpfeet(JTYPE).ne.0) 
     &write(33,3726)nameprop(numpfeet(JTYPE))
 3726 format('Diffuse input: ',a35/77('-'))
      ...
      if (JP.eq.ndetfirst) write(33,3725)uname(KFEAT)
      if (JP.eq.ndetfirst.and.numpfeet(JTYPE).ne.0) 
     &write(33,3726)nameprop(numpfeet(JTYPE))!,numpfeet(JTYPE)!,distp
...
*     or for effluents ---------------------------------------------------------
 ...
 3272 format(77('e')/'Generate monthly discharge flows for: ',
     &a35/77('e'))
      ...
      if ( JTYPE .eq. 62 ) then
      if (JP.eq.ndetfirst) write(33,3272)uname(feeture)
      call generate monthly structure for discharge flows 2 ! type 62
      endif
...
*     effluents ----------------------------------------------------------------
      ...
      if ( JTYPE .eq. 62 ) call monthly discharge quality
...
*     count the percentage of time the flow is greater than zero ---------------
      ...
      do idet = 1,ndet
      if ( qtype(idet) .ne. 4 .and. ical13 .eq. 0 ) then 
      write(150+idet,1664)zeroEF
...
*     set up a monthly structure ===============================================
      if ( JP .eq. ndetfirst ) then
      !write(01,51)
      !write(170+JP,51) ! ------------------------------------------------ Ci.GAP
   51 format(
     &'Calculated discharge flows obtained from the annual data ... '/
     &77('-'))
      call sort format 2 (EEFM,EEFS)
      !write(01,52) valchars10,FUNIT,valchars11,FUNIT
      !write(120+JP,52) valchars10,FUNIT,valchars11,FUNIT ! -------------- Di.ADL
      !write(170+JP,52) valchars10,FUNIT,valchars11,FUNIT ! -------------- Ci.GAP
   52 format(31x,'              Mean =',a10,1x,a4/
     &       31x,'Standard deviation =',a10,1x,a4)
      call sort format 2 (CM1,CM2)
      endif ! if ( JP .eq. ndetfirst ) ! ----------------------------------------
...
*     --------------------------------------------------------------------------
 ...
 6688 format(
     ...
     &7x,a37,7x,'Set number',i7,1x,7('.'),2i6)
      ...
      if ( BMMM .lt. 0.0001) then
      if ( BSSS .lt .0.0001 ) goto 5559 ! setting succeeded ~
      endif

      if ( iterate .lt. 40 ) goto 5555
...
*     setting up monthly data has failed ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 5266 format(77('-'),28x,25('-')/
     &'*** Unable to set up monthly structure for:',6x,'...',
     &7x,a37,9x,'Set',i7,' ..........',i4/77('-'),28x,25('-'))
      if (JP.eq.ndetfirst) write(33,5206)uname(kfeat),IQ
 5206 format(77('-')/
     &'*** Unable to set up monthly flow structure for: ',a37/
     &'*** Retained annual data. Set number:',i6/77('-'))
      ...
      if ( abs(rat1) .lt. 0.01 .and. (rat1) .gt. -0.01) then ! ---------------------
      call change colour of text (14) ! bright yellow
...
*     ==========================================================================
 5559 continue ! successful iterations ========================================= 
     
      if ( JP .eq. ndetfirst ) then
      call summaries for discharge flows (1,CM1,CM2)
      write(33,5399)BSMM,BSSS,iterate
 5399 format(77('=')/'% Convergence  ',2f11.6,27x,
     &'Iteration',i4/77('d')/)
      endif
      Eflow(1) = CM1
      Eflow(2) = CM2
      !if (JP.eq.ndetfirst) call write shots of discharge flow
...
*     if(JP.eq.ndetfirst)call summaries for discharge flows (1,CM1,CM2)
*     if ( MONF .gt. 1 ) call write shots of discharge flow
      endif
...
*     get the normal deviate for monthly stream quality ------------------------
      ...
      Z1(is) = cdmm
*     compute the value of the shot --------------------------------------------
      ECshots(is) = Vlogn (cdmm,GRCS,GRCM,RC3,BSM(imonth),
     &BSS(imonth),RCM)
   22 continue ! loop on shots
      
      !if ( nobigout .le. 0 ) write(150+JP,95)
      !if ( nobigout .le. 0 ) write(150+JP,99) dname(jp)
   99 format('River random numbers: ',a11)
      !if ( nobigout .le. 0 ) write(150+JP,95)
   95 format(77('s'))
      !if ( nobigout .le. 0 ) write(150+JP,94) (Z1(IS),IS=1,50)
   94 format(7x,10F7.4,'  ')
      !if ( nobigout .le. 0 ) write(150+JP,95)
...
*     ... a monthly distribution of discharge quality --------------------------
      ...
      EECM = pollution data (IQ,JP,1) ! mean effluent quality ------------------
      EECS = pollution data (IQ,JP,2) ! standard deviation ---------------------
      spcorrfa = pollution data(IQ,JP,4) ! correlation for flow and quality ----

      !if ( MONQ .gt. 1 ) then ! write out details =============================
      if ( ical13 .eq. 0 ) then ! gap filling is not being produced ------------
      write(01,51)Dname(Jp)
      write(150+JP,51)Dname(Jp)
   51 format(/66('X')/'In subroutine monthly discharge quality',
     &' - pollution data and EECM'/
     &66('X')/
     ...
     &66('X'))
      ...
      if ( ical13 .eq. 0 ) write(01,52)valchars10,UNITs(JP),valchars11,
     &UNITs(JP)
      write(150+JP,52)valchars10,UNITs(JP),valchars11,UNITs(JP)
   52 format('Input data:',24x,'          Mean =',a10,1x,a4/
     &26x,'     Standard deviation =',a10,1x,a4/66('X'))
      endif ! if ( ical13 .eq. 0 )  --------------------------------------------
      !endif ! if ( MONQ .gt. 1 ) write out details ============================
      
      call sort format 2 (ECM,ECS)
      write(150+JP,88)ECM,UNITs(JP),ECS,UNITs(JP)
   88 format(/66('Y')/
     &28x,'                   Mean =',F8.2,1x,a4/
     &28x,'     Standard deviation =',F8.2,1x,a4/
     &66('Y')/'Entered subroutine monthly discharge quality ... ',
     &'ECM and ECS'/66('Y')/)
...
*     sample the distributions -------------------------------------------------
      ...
      ECM1 = seas1 (imonth) ! mean discharge quality ---------------------------
      ECS1 = seas2 (imonth) ! standard deviation --------------------------------
      ECF1 = seas3 (imonth) ! shift ---------------------------------------------
      EC31 = ( ECM + ECF ) * ( ECM + ECF )
      spcorrfa = seas4 (imonth) ! effluent flow and quality --------------- cRAN

*     mean and standard deviation of logged data -------------------------------
      GECM = 0.0
      GECS = 0.0
      if ( EC31 .gt. 1.0e-9) then
      GECM = ALOG ( EC31 / SQRoot(1258, EC31 + ECS1 * ECS1 ) )
      GECS = SQRoot(1259, ALOG ( 1.0 + ( ECS1 * ECS1 ) / EC31 ) )
      endif
...
*     get the normal deviate for monthly discharge quality ---------------------
      ...
      Z1(is) = cdmm
*     compute the value of the shot --------------------------------------------
      ECshots(is) = Vlogn (cdmm,GECS,GECM,ECF1,BSM(imonth),
     &BSS(imonth),ECM)
      Z2(is) = ECshots(is)
      endif
   22 continue

      write(150+JP,45)ECM,UNITs(JP),ECS,UNITs(JP)
   45 format(66('B')/
     &28x,'                   Mean =',F8.2,1x,a4/
     &28x,'     Standard deviation =',F8.2,1x,a4/
     &66('B')/'After do 22 imon  ... ',
     &'ECM and ECS'/66('B')/)

*     ==========================================================================
      if ( nobigout .le. 0 ) write(150+JP,95)
      if ( nobigout .le. 0 ) write(150+JP,92) dname(jp)
  ...
  339 Format('Calculated mean =',f8.5,8x,
     &       '           ...  Standard deviation =',f8.5/77('X'))       
      
      write(150+JP,48)CM1,UNITs(JP),CM2,UNITs(JP)
   48 format(/66('Z')/
     &28x,'                   Mean =',F8.2,1x,a4/
     &28x,'     Standard deviation =',F8.2,1x,a4/
     &66('Z')/'In subroutine monthly discharge quality ... ',
     &'CM1 and CM2'/66('Z')/)
      
      write(150+JP,47)ECM,UNITs(JP),ECS,UNITs(JP)
   47 format(66('Z')/
     &28x,'                   Mean =',F8.2,1x,a4/
     &28x,'     Standard deviation =',F8.2,1x,a4/
     &66('Z')/'In subroutine monthly discharge quality ... ',
...
*    &66('X'))
      ...
      write(150+JP,58)Dname(JP)
   58 format(66('X')/
     &'Calculated discharge quality: ',a11,' (from monthly shots)'/
     &66('X'))

      write(01,59)CM1,UNITs(JP),CM2,UNITs(JP)
      write(150+JP,59)CM1,UNITs(JP),CM2,UNITs(JP)
   59 format('From shots:',26x,'          Mean =',F8.2,1x,a4/
     &28x,'     Standard deviation =',F8.2,1x,a4/
     &66('X')/'Leaving subroutine monthly discharge quality ... ',
     &'CM1 and CM2'/66('X'))

      write(150+JP,89)ECM,UNITs(JP),ECS,UNITs(JP)
   89 format(/66('Y')/
     &'From shots:',26x,'          Mean =',F8.2,1x,a4/
     &28x,'     Standard deviation =',F8.2,1x,a4/
     &66('Y')/'Leaving subroutine monthly discharge quality ... ',
     &'ECM and ECS'/66('Y')/)
...
*     compute Monte Carlo sampling errors for monthly discharge quality data
     ...
     &(imon)
      ...
      jmonth = imonth
      if ( jmonth .ne. imon ) goto 2
      ECM1 = seas1 (jmonth) ! mean quality --------------------------------------
      ECS1 = seas2 (jmonth) ! mean quality --------------------------------------
      EC31 = seas3 (jmonth) ! shift flow ----------------------------------------
*     special correlation coefficient ... added flow on added quality ----------
      spcorrfa = seas4 (jmonth) ! effluent flow and quality --------------- cRAN

      GECM = 0.0
      GECS = 0.0
      EM31 = (ECM1 + EC31) * (ECM1 + EC31)

*     mean and standard deviation of logged data -------------------------------
      if ( EM31 .gt. 1.0e-9) then
      GECM = ALOG ( EM31 / SQRoot(1262, EM31 + ECS1 * ECS1 ) )
      GECS = SQRoot(1263, ALOG ( 1.0 + ( ECS1 * ECS1 ) / EM31 ) )
      endif
...
*     compute the value of the shot --------------------------------------------
 ...
 4288 format('Calculated flow   ',2f11.4)
      ...
      BSMM = abs (100.0 * (1.0 - CM1 / tmean ))
      BSSS = abs (100.0 * (1.0 - CM2 / t95 )) 

      iterate = 0 ! initialise number of iterations
      
      if (JP.eq.1.or.JP.eq.ndetfirst) write(33,6099)BSMM,BSSS,
     &iterate
 6099 format(77('-')/'% Differences     ',2f11.4,24x,'Iteration',i4/
     &77('='))
...
*     ==========================================================================
 ...
 5266 format(/78('X')/
     ...
     &78('X')/)
      ...
      if (JP.eq.1.or.JP.eq.ndetfirst) suppress12 = suppress12 + 1
      !if (JP.eq.1.or.JP.eq.ndetfirst) write(09,8466)IF
 8466 format(77('-')/
     &'*** Unable to set up the imposed monthly structure ',
     &'for river flow set:',i7/77('-'))
      if (JP.eq.1.or.JP.eq.ndetfirst) write(33,5266)IF,JT(feeture),
     &iterate
      else ! if ( iterate .eq. 30 ) ============================================
      call statistics for monthly river flows (1, CM1, CM2 )
      rat1 = CM1 / tmean
      rat2 = CM2 / t95
      
      BSMM = abs (100.0 * (CM1 / tmean ))
      BSSS = abs (100.0 * (CM2 / t95 )) 

      if (JP.eq.1.or.JP.eq.ndetfirst) write(33,5390)BSMM,BSSS,iterate
 5390 format('% Convergence     ',2f11.4,24x,'Iteration',i4)
      if (JP.eq.1.or.JP.eq.ndetfirst) write(33,5999)CM1,CM2
 5999 format(77('=')/'Calculated flow   ',2f11.4,24x/77('f')/)
...
*     non-standard correlation -------------------------------------------------
      ...
      if ( jt (feeture) .eq. 03 .or. jt (feeture) .eq. 12 .or.
     &     jt (feeture) .eq. 05 .or. jt (feeture) .eq. 39 .or.
...
*     write(150+JP,5399)IQ,JP,CMlast,CMwant,CMratio ! ------------------- Di EFF
...
*     write out the shots for discharge quality --------------------------------
     ...
     &     IQDIST .eq. 9 .or. IQDIST .eq. 11 ) then ! load LLLLLLLLLLLLLLLLLLLLL
      ...
      write(150+jp,13) dname(jp) ! -------------------------------------- Di EFF
   13 format(/147('-')/'Generated shots for added load: ',a11/147('-'))
      ...
      write(150+jp,12) dname(jp) ! -------------------------------------- Di EFF
   12 format(147('-')/'Generated shots for added discharge quality: ',
     &a11/147('-'))
      ...
      write(150+jp,14) (ECshots(IS),IS=1,NS) ! -------------------------- Di EFF
   14 format(f9.2,15F9.2)
      write(01,15)
      write(31,15) ! ------------------------------------------------------- EFF
      write(150+jp,15) ! ------------------------------------------------ Di EFF
   15 format(147('-'))
      endif ! if ( nobigout .le. 0 ) ===========================================
   ...
   29 Format(63x,'Calculated annual mean quality =',f8.2,
     &       '           ...  Standard deviation =',f8.2/147('-'))
      ...
      if ( IQDIST .eq. 6 .or. IQDIST .eq.  7 .or.   ! load LLLLLLLLLLLLLLLLLLLLL
     &     IQDIST .eq. 9 .or. IQDIST .eq. 11 ) then ! load LLLLLLLLLLLLLLLLLLLLL
      write(01,43) dname(jp)
      write(31,43) dname(jp) ! --------------------------------------------- EFF
      write(150+jp,43) dname(jp) ! -------------------------------------- Di EFF
   43 format(/147('~')/'Generated shots for added load: ',a11/147('~'))
      else ! if ( IQDIST .eq. 6 etc LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
      write(01,42) dname(jp)
      write(31,42) dname(jp) ! --------------------------------------------- EFF
      write(150+jp,42) dname(jp) ! -------------------------------------- Di EFF
   42 format(147('~')/'Generated shots for added discharge quality: ',
     &a11/147('~'))
      endif ! if ( IQDIST .eq. 6 etc LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
      write(01,44) (ECshots(IS),IS=1,NS)
      write(31,44) (ECshots(IS),IS=1,NS) ! --------------------------------- EFF
      write(150+jp,44) (ECshots(IS),IS=1,NS) ! -------------------------- Di EFF
   44 format(f9.2,15F9.2)
      write(01,45)
      write(31,45) ! ------------------------------------------------------- EFF
      write(150+jp,45) ! ------------------------------------------------ Di EFF
   45 format(147('~'))
      endif ! if ( nobigout .le. 0 ) ===========================================
 ...
 5399 format(2i4,3f12.4)
  ...
  186 CM = CM/float(KS)
      ...
      if ( IQDIST .eq. 6 .or. IQDIST .eq.  7 .or.   ! load LLLLLLLLLLLLLLLLLLLLL
     &     IQDIST .eq. 9 .or. IQDIST .eq. 11 ) then ! load LLLLLLLLLLLLLLLLLLLLL
      write(01,63) dname(jp)
      write(31,63) dname(jp) ! --------------------------------------------- EFF
      write(150+jp,63) dname(jp) ! -------------------------------------- Di EFF
   63 format(/147('=')/'Adjusted shots for added load: ',a11/147('='))
      else ! if ( IQDIST .eq. 6 etc LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
      write(01,62) dname(jp)
      write(31,62) dname(jp) ! --------------------------------------------- EFF
      write(150+jp,62) dname(jp) ! -------------------------------------- Di EFF
   62 format(147('=')/'Adjusted shots for added discharge quality: ',
     &a11/147('='))
      endif ! if ( IQDIST .eq. 6 etc LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
      write(01,64) (ECshots(IS),IS=1,NS)
      write(31,64) (ECshots(IS),IS=1,NS) ! --------------------------------- EFF
      write(150+jp,64) (ECshots(IS),IS=1,NS) ! -------------------------- Di EFF
   64 format(2x,16F9.2)
      write(01,65)
      write(31,65) ! ------------------------------------------------------- EFF
      write(150+jp,65) ! ------------------------------------------------ Di EFF
   65 format(147('='))
      endif ! if ( nobigout .le. 0 ) ===========================================
