...
*     ==========================================================================
*     File: process the selected reach.for ... 1491 lines ----------------------
*     --------------------------------------------------------------------------
...
*     set the distance from the Head of Reach to the first Feature -------------
     ...
     &lconmon(1,j,1,13),lconmon(2,j,1,13) ! ----------------------------- Gi.CSV
  305 format('Start of Reach',',',A16,',',I4,',',i4,
     &80(',',1PE11.4))

      if ( n257 .eq. 1 ) then ! ................................................
      write(210+j,395)GIScode(KFEAT+1),RNAME(JREACH(KFEAT+1)), ! ------- DDi.CSV
     &dname(J),adist(JREACH(KFEAT+1)),dist(KFEAT+1),RQO(J), ! ---------- DDi.CSV
     &confail(J),(in class(iclass,j),iclass=1,nclass) ! ---------------- DDi.CSV
  395 format(' ',a40,',','111,','Start of Reach',',',A16,',COMPLIANCE',
     &',',a11,',,,',f8.2,
     &',,,,,,,,,,,,,,,,,',f8.2,20(',',f8.2))
      endif ! if ( n257 .eq. 1 ) ...............................................
...
*     perform classification and assess compliance -----------------------------
      ...
      do j=1,MP10
      if ( QTYPE(j) .ne. 4 ) then
      write(210+j,1395)GIScode(KFEET),JT(KFEET),UNAME(KFEET), ! -------- DDi.CSV
     &RNAME(IREACH),dname(j), ! ---------------------------------------- DDi.CSV
     &distp,dist(KFEET),RQO(j),confail(j) ! ---------------------------- DDi.CSV
     &,(in class(iclass,j),iclass=1,nclass) ! -------------------------- DDi.CSV
     &,(totals in classes(iclass,j),iclass=1,nclass) ! ----------------- DDi.CSV
 1395 format(' ',a40,',',i4,',',a37,',',A16,',COMPLIANCE',
     &',',a11,',,,',f8.2,
     &',,,,,,,,,,,,,,,,,',f8.2,20(',',f8.2))
      endif
      enddo
      endif ! if ( n257 .eq. 1 ) 77777777777777777777777777777777777777777777777
...
*     gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg
*     do the gap filling -------------------------------------------------------
      if ( ical13 .eq. 1 ) then ! gap filling is being used --------------------
     ...
     &JT(IFEAT1) .ne. 23 ) then  ! exclude certain features --------------------
      ...
      endif !  if ( ical13 .eq. 1 ) gap filling is being used ------------------
*     gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg
...
*     mix the total concentrations ---------------------------------------------
     ...
     &           +  QD(ISTOR2,JP,IS) * FD(ISTOR2,IS)) / FMS(IS) ! -------------- 
      ...
      do ip = 1, nprop  ! mix the apportionment of the concentrations ==========

      LMS(ip,JP,IS) = (EQD(ip,ISTOR1,JP,IS) * FD(ISTOR1,IS) ! mix reaches ------
     &              +  EQD(ip,ISTOR2,JP,IS) * FD(ISTOR2,IS)) / FMS(IS) ! -------
      enddo ! ip = 1, nprop ! mix the apportionment of the concentrations ======
...
*     158 ################################################################## 157    
     ...
     &                 + T39LOAD3 (ISTOR2,JP,J1)
      T60LOAD2 (JP,J1) = T60LOAD3 (ISTOR1,JP,J1)    
     &                 + T60LOAD3 (ISTOR2,JP,J1)
      T61LOAD2 (JP,J1) = T61LOAD3 (ISTOR1,JP,J1)    
     &                 + T61LOAD3 (ISTOR2,JP,J1)
     ...
     &                 + T39LOAD4 (ISTOR2,JP,J1)
      T60LOAD1 (JP,J1) = T60LOAD4 (ISTOR1,JP,J1)    
     &                 + T60LOAD4 (ISTOR2,JP,J1)
      T61LOAD1 (JP,J1) = T61LOAD4 (ISTOR1,JP,J1)    
     &               + T61LOAD4 (ISTOR2,JP,J1)
     ...
     &                  + T48LOAD3  (ISTOR2,JP,J1)
      ...
      T50LOAD2  (JP,J1) = T50LOAD3  (ISTOR1,JP,J1)     
     &                  + T50LOAD3  (ISTOR2,JP,J1)
      T52LOAD2  (JP,J1) = T52LOAD3  (ISTOR1,JP,J1)     
     &                  + T52LOAD3  (ISTOR2,JP,J1)
      T54LOAD2  (JP,J1) = T54LOAD3  (ISTOR1,JP,J1)     
     &                  + T54LOAD3  (ISTOR2,JP,J1)
      T56LOAD2  (JP,J1) = T56LOAD3  (ISTOR1,JP,J1)     
     &                  + T56LOAD3  (ISTOR2,JP,J1)
      T58LOAD2  (JP,J1) = T58LOAD3  (ISTOR1,JP,J1)     
     &                  + T58LOAD3  (ISTOR2,JP,J1)
     ...
     &                  + T48LOAD4  (ISTOR2,JP,J1)
      T50LOAD1  (JP,J1) = T50LOAD4  (ISTOR1,JP,J1)     
     &                  + T50LOAD4  (ISTOR2,JP,J1)
      T52LOAD1  (JP,J1) = T52LOAD4  (ISTOR1,JP,J1)     
     &                  + T52LOAD4  (ISTOR2,JP,J1)
      T54LOAD1  (JP,J1) = T54LOAD4  (ISTOR1,JP,J1)     
     &                  + T54LOAD4  (ISTOR2,JP,J1)
      T56LOAD1  (JP,J1) = T56LOAD4  (ISTOR1,JP,J1)     
     &                  + T56LOAD4  (ISTOR2,JP,J1)
      T58LOAD1  (JP,J1) = T58LOAD4  (ISTOR1,JP,J1)     
     &                  + T58LOAD4  (ISTOR2,JP,J1)
...
*     store the annual loads discharged by upstream effluents ------------------
     ...
     &                    + TELOADAVrch(iworks,ISTOR2,JP)
      ...
      if ( JP .eq. ndshot ) then ! 3333333333333333 apportionment of percentiles
      TELOADshots(iworks,JP,is) = TELOADrchshots(iworks,ISTOR1,is) ! 333333333
     &                          + TELOADrchshots(iworks,ISTOR2,is) ! 333333333
      endif ! if ( JP .eq. ndshot ) ! 3333333333333 apportionment of percentiles
...
*     compute the confidence limits on river quality ---------------------------
     ...
     &(FEND(I),QE(JP,I),QDN(JP,I),FEND(J),QE(JP,J),QDN(JP,J),0)
	endif ! if ( QTYPE (JP) .ne. 4 )
      enddo ! JP = 1, ndet
...
*     check whether this next Feature is a river boundary ----------------------
     ...
     &     KTYPE .eq. 23 .or. KTYPE .eq. 45 ) then
      ...
      do J1 = 1, N13
      TGLODE2 (idet,J1) = 0.0
      TRLODE2 (idet,J1) = 0.0 ! =========================================== 157
      TELODE2   (idet,J1) = 0.0 ! net total loads from discharges (3 12 5 and 39)
      T03LOAD2  (idet,J1) = 0.0 ! net total loads from discharges (3)
      T05LOAD2  (idet,J1) = 0.0 ! net total loads from discharges (5)
      T12LOAD2  (idet,J1) = 0.0 ! net loads from discharges (12)
      T39LOAD2  (idet,J1) = 0.0 ! net total loads from discharges (39)
      
      T60LOAD2  (idet,J1) = 0.0 ! net total loads from discharges (60)
      T61LOAD2  (idet,J1) = 0.0 ! net total loads from discharges (61)
      ...
      T13LOAD2  (idet,J1) = 0.0 ! net total loads from diffuse (13)
      ...
      T50LOAD2  (idet,J1) = 0.0
      T52LOAD2  (idet,J1) = 0.0
      T54LOAD2  (idet,J1) = 0.0
      T56LOAD2  (idet,J1) = 0.0
      T58LOAD2  (idet,J1) = 0.0
      ...
      T60LOAD1  (idet,J1) = 0.0
      T61LOAD1  (idet,J1) = 0.0
      ...
      T50LOAD1  (idet,J1) = 0.0
      T52LOAD1  (idet,J1) = 0.0
      T54LOAD1  (idet,J1) = 0.0
      T56LOAD1  (idet,J1) = 0.0
      T58LOAD1  (idet,J1) = 0.0
...
*     breakdown of monthly loads from upstream sub-catchments ------------------
      ...
      if ( n251 .eq. 1 ) then ! 3333333333333333333 apportionment of percentiles
      do is = 1, NS ! 33333333333333333333333333333 apportionment of percentiles
      if ( idet .eq. ndshot ) TELOADshots(iworks,idet,is) = 0.0 ! percentiles
      enddo ! do is = 1, NS 33333333333333333333333 apportionment of percentiles
      endif ! if ( n251 .eq. 1 ) ! 3333333333333333 apportionment of percentiles
