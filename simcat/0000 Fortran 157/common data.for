...
*     ==========================================================================
*     File: common data.for ... 464 lines 
*     --------------------------------------------------------------------------
...
*     --------------------------------------------------------------------------
      ...
      parameter (NU = 19999 ) ! maximum number of features ---------------------
      ...
      parameter (NUED = 1700) ! number of discharges for back-tracking ---------
      ...
      parameter (NPROP = 29 ) ! number of types of feature tracked for inputs --
     ...
     integer reachstore, calspecial, diffreachstop
     ...
     &suppress21,suppress22,suppress23,suppress24,suppress25,
     ...
     &        exclude PO4, exclude DOX, exclude AMM 
      ...
      real lmon,in class,LMS,loadmon,lconmon
	...
	common /GR1/ virtualreach
     ...
     &IMM,MPAR,MONF,MONQ,MU,NDET,NEXC(MP10),NEXF,NGAUGE,
     ...
     &flowchars(2),valchars10,valchars11,valchars12,
     &B44(MP10)

      common /ATT/ISCREEN,LAPSE,IHR,IMIN,ISEC,IHUN,IYEAR,JMONTH,IDAY,
     &BIGLAPSE
     
      common /BTT/NMORQ,suppress (9,MP10),isupprezz,
     &suppress1,suppress3,suppress4,suppress5,
     &suppress6,suppress7,suppress8,suppress9, suppress10,
     &suppress11,suppress12,suppress13,suppress14,
     &suppress16,suppress17,suppress18,suppress19,suppress20,
     ...
     &DNA(MP10),DNAME(MP10)
      common /BDP/ CMX(MP10,MS),accum load(MS)
      common /BL/ LMS(nprop,MP10,MS),ulms(nprop,MP10,MS),jorder(nprop)
	...
	common /B2/EFMS(MS),ufms(MS),YY(MS),dfms(MS),
     ...
     &QDIFFGAP(MP10),QE(MP10,MR),quolity data(NV,MP10,MO+3),QNAT(MP10),
     ...
     &diffuse load(MP10,13),nameprop(nprop)
      ...
      common /B10/propsequence(nprop),numprop(nprop)

*     total loads                                               ... TGLODE1
*     total load introduced upstream boundaries and tributaries ... TRLODE1
*     total load from discharges (3 12 5 and 39) and (60, 61)   ... TELODE1
...
*     net loads                                                 ... TGLODE2
*     net load from discharges (3 12 5 and 39) and (60, 61)     ... TELODE2
...
*     net load removed by abstractions                          ... TBLOAD2
     ...
     &ELOAD60  (MP10,13), ELOAD61  (MP10,13)
     ...
     &T48LOAD1  (MP10,13), T50LOAD1  (MP10,13),
     &T52LOAD1  (MP10,13), T54LOAD1  (MP10,13),
     &T56LOAD1  (MP10,13), T58LOAD1  (MP10,13)
*     ====================================================================== 157
     ...
     &TBLOAD2   (MP10,13), TLOSSES2  (MP10,13), TLOSSES1(MP10,13),   
     ...
     &T48LOAD2  (MP10,13), T50LOAD2  (MP10,13),
     &T52LOAD2  (MP10,13), T54LOAD2  (MP10,13),
     &T56LOAD2  (MP10,13), T58LOAD2  (MP10,13)
      common /l3a3/ 
     ...
     &T48LOAD3  (MR,MP10,13), T50LOAD3  (MR,MP10,13),
     &T52LOAD3  (MR,MP10,13), T54LOAD3  (MR,MP10,13),
     &T56LOAD3  (MR,MP10,13), T58LOAD3  (MR,MP10,13)
*     ==========================================================================
      common /l3a4/ 
     ...
     &T48LOAD4  (MR,MP10,13), T50LOAD4  (MR,MP10,13),
     &T52LOAD4  (MR,MP10,13), T54LOAD4  (MR,MP10,13),
     &T56LOAD4  (MR,MP10,13), T58LOAD4  (MR,MP10,13)

      common /l3g/ i13, n13, NSM(MP10,13)
     
      common /l2e/ 
     &T03LOAD1 (MP10,13),    T03LOAD2 (MP10,13),  
     &T05LOAD1 (MP10,13),    T05LOAD2 (MP10,13),
     &T12LOAD1 (MP10,13),    T12LOAD2 (MP10,13),
     &T39LOAD1 (MP10,13),    T39LOAD2 (MP10,13),
     &T60LOAD1 (MP10,13),    T60LOAD2 (MP10,13),
     &T61LOAD1 (MP10,13),    T61LOAD2 (MP10,13),
     &T03LOAD3 (MR,MP10,13), T03LOAD4 (MR,MP10,13), 
     &T05LOAD3 (MR,MP10,13), T05LOAD4 (MR,MP10,13),
     &T12LOAD3 (MR,MP10,13), T12LOAD4 (MR,MP10,13),
     &T39LOAD3 (MR,MP10,13), T39LOAD4 (MR,MP10,13),
     &T60LOAD3 (MR,MP10,13), T60LOAD4 (MR,MP10,13),
     &T61LOAD3 (MR,MP10,13), T61LOAD4 (MR,MP10,13)
      
      common /l4a/ TELOADAV (NUED,MP10),TELOADAVrch (NUED,KR,MP10),
     &TDLOADAV (NUW,MP10)
      
*     apportionment of percentiles ---------------------------------------------
*     common /l4b/ TELOADshots (NUED,MP10,MS) ! --------------------------------
*     common /l4c/ TELOADrchshots (NUED,KR,MS) ! -------------------------------
      common /l4b/ TELOADshots (NUED,1,1) ! ------------------------------------
      common /l4c/ TELOADrchshots (NUED,KR,1) ! --------------------------------

      common /l4y/ mark works, kountworks, need works, kill works,
     &identify werks (NUED)
...
*     partitioning -------------------------------------------------------------
     ...
     &exceedences95 (MP10,13)
      ...
      common /app/ EQD(nprop,KR,MP10,MS),Lsummary(nprop,MP10,5),
     &loadmon(nprop,MP10,2,13), lconmon(nprop,MP10,2,13),
     ...
     &standards for reach (MR,NC)
      common /new1/ n251 ! 333333333333333333333333 apportionment of percentiles
      common /new2/ n253 ! GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG calibration
      common /new3/ n257 ! --------------------------------------------- DDi.CSV
      common /reg/ Creg(4,MS)

      common /ign/ add diffconc (NU), add conc
      
      common /ark/ arkdist(NU),crossmatch(MP10,NUED,NU)
