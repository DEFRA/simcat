...
*     ==========================================================================
*     File: meet the percentile river targets ... 2385 lines -------------------
*     --------------------------------------------------------------------------
...
*     --------------------------------------------------------------------------
      ...
      icp = 3 ! initialise target as a 95-percentiles --------------------------
...
*     switch for percentile for river quality targets PPPPPPPPPPPPPPPPPPPPPPPPPP
      ...
      if ( Mtargit .eq. 4 ) kptarg =  5 ! 5-percentile
      if ( Mtargit .eq. 5 ) kptarg = 10 ! 10-percentile
*     pppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 1705 format(/110('=')/'Assess the effluent quality needed ', ! --------- Di EFF
     ...
     &'The target is a',i3,'-percentile of: ',a10,1x,a4, ! -------------- Di EFF
     &' ... downstream of ',a37/110('-')) ! ----------------------------- Di EFF
      write(130+JP,1504)GIScode(feeture),uname(feeture), ! -------------- Ei.CSV
     &rname(ireach),kptarg,dname(JP),targit,kptarg,JT(KFEAT), !---------- EI.CSV
     &uname(feeture) ! -------------------------------------------------- Ei.CSV
 1504 format(' ',a40,',',a40,',',a16,',', ! ----------------------------- Ei,CSV
     &'River quality standard (',i2,'-percentile)', ! ------------------- Ei.CSV
     &',',a11,2(','),(',',1pe12.4),',',i2,'-percentile', ! -------------- Ei.CSV
     &',',i4,',',a35) ! ------------------------------------------------- Ei.CSV
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     
      
      
      call sort format 3 (pollution data(IQ,JP,1),ECM,TECM) ! mean quality
...
*     downstream ---------------------------------------------------------------
     ...
     &(IMPOZZ,TRQS,IBOUND)
      ...
      RCXXX = 1.0 ! initialise the scaling of u/s river quality for Mode 09 9999
      ...
      if ( MRQS(JP) .eq. 2 ) then ! 95-percentile ! -----------------------------
      ipl = 95 ! 95-percentile
      ipc = 3 ! value of C(JP,ipc) for 95-percentile
      endif ! if ( MRQS(JP) .eq. 2 ) --------------------------------------------
      if ( MRQS(JP) .ne. 4 .and. MRQS(JP) .ne. 5 ) then ! == not dissolved oxygen
      if ( MRQS(JP) .eq. 3 ) then ! 90-percentile ! -----------------------------
      ipl = 90 ! 90-percentile
      ipc = 4 ! value of C(JP,ipc) for 90-percentile
      endif ! if ( MRQS(JP) .eq. 3 ) --------------------------------------------
      if ( MRQS(JP) .eq. 6 ) then ! 99-percentile -------------------------------
      ipl = 99 ! 99-percentile
      ipc = 5 ! value of C(JP,ipc) for 99-percentile
      endif ! if ( MRQS(JP) .eq. 6 ) --------------------------------------------
      endif
      endif
...
*     set the mean, standard deviation and 95-percentile -----------------------
      ...
      if ( kptarg .eq. 95 ) then ! 95-percentile PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
      RCX = C(JP,3) ! the 95-percentile 
      icp =  3
      ipl = 95 
      endif ! 95-percentile PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
      if ( kptarg .eq. 90 ) then ! 90-percentile PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
      RCX = C(JP,4)
      icp =  4 
      ipl = 90 
      endif ! 90-percentile PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
      if ( kptarg .eq. 99 ) then ! 99-percentile PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
      RCX = C(JP,5)
      icp =  5
      ipl = 99 
      endif ! 99-percentile PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
*     --------------------------------------------------------------------------
...
*     upstream quality is set within the target upstream class 99999999999999999
      ...
      call sort format 3 (RCX,RCM,RCS)
      write(01,3000)kyy,SET1,ipoint,valchars10,units(JP),kyy, ! ------------ OUT
     &valchars11,units(JP),valchars12,units(JP) ! -------------------------- OUT
      write(31,3000)kyy,SET1,ipoint,valchars10,units(JP),kyy, ! ------------ EFF
     &valchars11,units(JP),valchars12,units(JP) ! -------------------------- EFF
      write(150+JP,3000)kyy,SET1,ipoint,valchars10,units(JP),kyy, ! ----- Di EFF
     &valchars11,units(JP),valchars12,units(JP) ! ----------------------- Di EFF
 3000 format(/110('=')/
     &'Upstream quality is imposed as',i3,'% of the target class ', ! 9999999999
     &'(Run Type 9) ... Limits: ',a13/110('=')/
     &6x,'Imposed upstream ',i3,'-percentile =',a10,1x,a4,3x,
     &'   (at',i3,'% of class limits)'/
     &'  Imposed mean upstream river quality =',a10,1x,a4/
...
*    NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN
     ...
     &rname(ireach),kyy,SET1,dname(JP),RCM,RCS,RCX,kptarg,
     &JT(KFEAT),unamex
...
*     create the altered upstream river quality data 999999999999999999999999999
      ...
      else ! if ( RCX .gt. RCXX ) ! RCX is not greater than RCXX 999999999999999
...
*     99999999999999999999999999999999999999999999999999999999999999999999999999
 ...
 2654 format('River quality provided:          Mean =',a10,1x,A4,
     ...
     &63x,'And current standard deviation =',a10,1x,A4)
      ...
      call sort format 3 (TRCX,ECX,ECS)
      write(01,3964)kptarg,valchars10,UNITS(JP),valchars11,UNITS(JP), ! ---- OUT
     &SET2 ! --------------------------------------------------------------- OUT
      write(31,3964)kptarg,valchars10,UNITS(JP),valchars11,UNITS(JP), ! -=-- EFF
     &SET2 ! --------------------------------------------------------------- EFF
      write(150+JP,3964)kptarg,valchars10,UNITS(JP),valchars11, ! ------- Di EFF
     &UNITS(JP),SET2 ! -------------------------------------------------- Di EFF
 3964 format(23x,i3,'-percentile =',a10,1x,A4,
     &'     By current discharge quality:  Q95 =',a10,1x,A4,4x,A13)
...
*     ==========================================================================
 ...
 8963 format(' ',a40,',',a40,',', ! ---------------- percentile targets - Ei.CSV
...
*     UPSTREAM RIVER QUALITY HAS NOT BEEN MODIFIED) 9999999999999999999999999999
 ...
 7964 format(23x,i3,'-percentile =',a10,1x,A4,
     ...
     &'(UPSTREAM RIVER QUALITY HAS NOT BEEN MODIFIED)'/110('-')) ! 9999999999999
      ...
      write(130+JP,7963)GIScode(feeture),unamex, ! -- current dischage -- Ei.CSV
     &dname(JP),TRCM,TRCS,TRCX,kptarg,JT(KFEAT),unamex ! ---------------- Ei.CSV
 7963 format(' ',a40,',',a40,',', ! ------------------------------------- Ei.CSV
     &'River quality d/s of current discharge',',', ! ------------------- Ei.CSV
     &'Upstream river quality has not been modified', ! ----------------- Ei.CSV
     &',',a11,3(',',1pe12.4),',',i2,'-percentile',',',i4,',',a35) ! ----- Ei.CSV
...
*     99999999999999999999999999999999999999999999999999999999999999999999999999
 ...
 7522 format(' ',a40,',',a40,',',
     ...
     &dname(JP),C(JP,1),C(JP,2),C(JP,icp),kptarg,JT(KFEAT),unamex
 7532 format(' ',a40,',',a40,',',
     &'????? quality d/s of current discharge',',',
...
*     ==========================================================================
 ...
 7592 format(' ',a40,',',a40,',',a16,',', ! ---- percentile targets ----- Ei.CSV
     &'DECISION: Retain current effuent quality', !  percentile targets - Ei.CSV
     &',',a11,2(',',1pe12.4),',,,',i4,',',a35) ! ------------------------ Ei.CSV
*     ==========================================================================
      endif ! if ( ifeffcsv .eq. 1 ) VVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV
...
*     ==========================================================================
     ...
     &rname(ireach), ! ------------- current dischage quality retained -- Ei.CSV
     &dname(JP),TECM,TECS,TECX,JT(KFEAT),unamex ! ----------------------- Ei.CSV
 7792 format(' ',a40,',',a40,',',a16,',', ! ---- percentile targets ----- Ei.CSV
     &'Current effuent quality retained', ! ---- percentile targets ----- Ei.CSV
...
*    &80x,'95-PERCENTILE =',a10,1x,A4/
     ...
     &rname(ireach),dname(JP),TECM,TECS,TECX,JT(KFEAT),unamex ! --------- Ei.CSV
...
*     is the river target stricter than the current upstream river quality ? ---
      ...
      if ( UPC(JP,ipc)*RCXXX .ge. TRQS ) then ! - u/s quality is worse than TRQS 
...
*     starting set of data for the iteration scheme ----------------------------
  ...
  996 ITER = ITER + 1 ! the number of the next iteration -----------------------
      ...
      write(007,3362)ITER,TECM!,TECS
 3362 format(i2,' TECM        ',2x,f8.4,8x,' ... after 996 ITER = 1')
      endif
      else
      if (JP.eq.2.and.feeture.eq.34) then
      write(007,2862)ITER,TECM
 2862 format(i2,' TECM ',9x,f8.4,8x,' ... after 996 ITER + 1')
      endif 
      endif

      call get the summary statistics of discharge quality (TECM,TECS)
      
      if ( JP.eq.5 .and. feeture.eq.55 ) then
      !write(007,8519)ITER,ECM,ECS,ECX
 8519 format(i2,' ECM, ECS, ECX',3f8.4,82x,' ... current?')          
      endif
...
*     the latest completed iteration ===========================================
  893 TECM1 = TECM ! store the discharge quality after iteration number 2
      ...
      if (JP.eq.2.and.feeture.eq.34) then
      write(007,7162)ITER,TECM1,TRCX1,TECM0,TECM1 
 7162 format(i2,' TECM 1  ',f14.4,13x,'TRCX 1',f8.4,
     &' replaced TECM1 0 & 1',2f8.4)
      endif
...
*     ==========================================================================     
 9095 continue ! iterations are complete for river quality target --------------
      ...
      call calculate summaries of river quality (33,JP,RQ)
      TRCM = C(JP,1)   ! mean downstream river quality 999
      TRCS = C(JP,2)   ! standard deviation
      TRCX = C(JP,ipc) ! percentile downstream river quality
...
*     for the percentile river quality standard -------------------------------- 
     ...
     &(95.0,iqdist,TECM,TECS,TECX,qualnuse,testql,testqu)
      ...
      if ( iter .eq. 61 ) then ! ===============================================
      write(01,8993)kptarg,valchars10,UNITS(JP),valchars11,UNITS(JP), ! ---- OUT
     &SET1
      write(31,8993)kptarg,valchars10,UNITS(JP),valchars11,UNITS(JP), ! ---- EFF
     &SET1
      write(150+JP,8993)kptarg,valchars10,UNITS(JP),valchars11, ! ------- Di EFF
     &UNITS(JP),SET1
 8993 format('TARGET river quality:   ',i2,'-percentile =',a10,1x,A4,2x,
 ...
 8501 format(' ',a40,',',a40,',',a16,',',
     ...
     &',',a11,3(',',1pe12.4),',','95-percentile',
     &(',',i4),',',a35) ! ------------------------------ u/s quality ---- Ei.CSV
      write(130+JP,8502)GIScode(feeture),unamex, ! ----- u/s quality ---- Ei.CSV
     &rname(ireach),dname(JP),TRCM,TRCS,TRCX,kptarg,JT(KFEAT),unamex
 8502 format(' ',a40,',',a40,',',a16,',',
     ...
     &UNITS(JP),SET2 ! -------------------------------------------------- Di EFF
 5963 format('Achieved river quality:',i3,'-percentile =',a10,1x,A4, !  07 or 08
     &16x,'Discharge 95-percentile =',a10,1x,A4,4x,a13/110('=')) ! ---- 07 or 08
     
      else ! if ( ical .ne. 09 ) ie ical equals 09 99999999999999999999999999999    
     
      if ( ireplace .eq. 1 ) then ! upstream quality has been replaced 999999999
      
      call sort format 2 (C(JP,1),TECM)
 ...
 5964 format(/110('~')/
     ...
     &25x,'Discharge mean =',a10,1x,A4,4x,a13)
      ...
      call sort format 2 (C(JP,icp),TECX)
      write(01,5961)kptarg,valchars10,UNITS(JP),valchars11, ! -------------- OUT
     &UNITS(JP),SET2
      write(31,5961)kptarg,valchars10,UNITS(JP),valchars11, ! -------------- EFF
     &UNITS(JP),SET2
      write(150+JP,5961)kptarg,valchars10,UNITS(JP),valchars11, ! ------- Di EFF
     &UNITS(JP),SET2
 5961 format(20x,i6,'-percentile =',a10,1x,A4,
...
*     ==========================================================================      
      else ! if ( ireplace .eq. 1 ) upstream quality had not been replaced 99999 
      TRCM = C(JP,1)
      TRCS = C(JP,2)
      TRCX = C(JP,icp)
      ...
      write(01,8163)valchars10,UNITS(JP),valchars11,UNITS(JP),SET1 ! ------- OUT
      write(31,8163)valchars10,UNITS(JP),valchars11,UNITS(JP),SET1 ! ------- EFF
      write(150+JP,8163)valchars10,UNITS(JP),valchars11,UNITS(JP),SET1 !  Di EFF
 8163 format('TARGET river quality achieved:   Mean =',a10,1x,a4,2x, ! -- Di EFF
     &'Required discharge quality:'6x'Mean =',a10,1x,A4,4x,a13) ! ------- Di EFF 
      write(01,7993)valchars12,UNITS(JP) ! --------------------------------- OUT
      write(31,7993)valchars12,UNITS(JP) ! --------------------------------- EFF
      write(150+JP,7993)valchars12,UNITS(JP) ! -------------------------- Di EFF
 7993 format(65x,'Discharge standard deviation =',a10,1x,a4)
      
      call sortformat 2 (TRCX,TECX)
      write(01,5904)kptarg,valchars10,UNITS(JP),valchars11,UNITS(JP), ! ---- OUT
     &SET2 ! --------------------------------------------------------------- OUT
      write(31,5904)kptarg,valchars10,UNITS(JP),valchars11, ! -------------- EFF
     &UNITS(JP),SET2 ! ----------------------------------------------------- EFF
      write(150+JP,5904)kptarg,valchars10,UNITS(JP),valchars11, ! ------- Di EFF
     &UNITS(JP),SET2 ! -------------------------------------------------- Di EFF
 5904 format('Achieved river quality:',i3,'-percentile =',a10,1x,A4, ! 999999999
     &16x,'Discharge 95-percentile =',a10,1x,A4,4x,a13/110('='))
     
      write(130+JP,5514)GIScode(feeture),unamex, ! ---------------------- Ei.CSV
     &rname(ireach),dname(JP),C(JP,1),C(JP,2),C(JP,icp),kptarg,
     &JT(KFEAT),unamex
...
*     99999999999999999999999999999999999999999999999999999999999999999999999999      
     ...
     &rname(ireach),dname(JP),Trqs,kptarg,JT(KFEAT),unamex
 7512 format(' ',a40,',',a40,',',a16,',',
     &'Target percentile river quality',
     &',',a11,1(',,,',1pe12.4),',',i2,'-percentile',
     &(',',i4),',',a35) ! ----------------------------------------------- Ei.CSV
