...
*     ==========================================================================
*     File: process a feature.for ... 6124 lines  ------------------------------
*     --------------------------------------------------------------------------
...
*     monitoring point ---------------------------------------------------------
      ...
      if ( igapcall .eq. 0 ) then ! not gap-filling ----------------------------
          
      call monitoring point ! usual and not gap-filling ------------------------
      goto 6000 ! return

*     gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg
      else ! gap-filling is running ++++++++++++++++++++++++++++++++++++++++++++
      if ( never fitted .eq. 0 .or. KSIM .eq. MAXNITZ ) then ! +++++++++++++++++
      if ( feeture .eq. JUgap ) then ! this Feature is used for Gap Filling ++++
      call monitoring point ! gap-filling for a monitoring point +++++++++++++++
      !KSIM = MAXNITZ + 1 ! ++++++++++++++++++++++++++++++++++++++++++++++++++++
      endif ! if ( feeture .eq. JUgap ) ++++++++++++++++++++++++++++++++++++++++
      goto 6000 ! return ! +++++++++++++++++++++++++++++++++++++++++++++++++++++
      endif ! if ( never fitted .eq. 0 .or. KSIM .eq. MAXNITZ ) ++++++++++++++++
      endif ! gap-filling ++++++++++++++++++++++++++++++++++++++++++++++++++++++
*     gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg

      else ! if not a monitoring point ! +++++++++++++++++++++++++++++++++++++++
      if ( never fitted .eq. 0 .or. KSIM .eq. MAXNITZ ) then ! +++++++++++++++++
      if ( feeture .eq. JUgap ) then ! this Feature is used for Gap Filling ++++
      call monitoring point ! gap-filling for Feat that is not a monitoring pt +
      !KSIM = MAXNITZ + 1 ! ++++++++++++++++++++++++++++++++++++++++++++++++++++
      goto 6000 ! return ! +++++++++++++++++++++++++++++++++++++++++++++++++++++
      endif ! if ( feeture .eq. JUgap ) ++++++++++++++++++++++++++++++++++++++++
      endif ! if ( never fitted .eq. 0 .or. KSIM .eq. MAXNITZ ) ++++++++++++++++
...
*     fish farm (62) ===========================================================
      if ( itype .eq. 62 ) then ! fish farm 62 ---------------------------------
      call fish farm ! 
      call get summaries of river quality from the shots ! fish farm -----------
...
*     ==========================================================================
      ...
      if ( NGAUGE .gt. 1 .and. KGAUGES(NGAUGE) .eq. KFEAT ) return
      ...
      call flow into lake
      call abstraction of flow ! ------------------------- flow into a lake (44)
      call get summaries of river quality from the shots ! --- interpolate point
      call get summaries of loads
      goto 6000 ! return
      endif ! ==================================================================
...
*     discharge with zero flow -------------------------------------------------
 ...
 1015 format(110('=')/
     ...
     &a40/110('-'))
      ...
      do jper = 1,ndet ! -------------------------------------------------------
      if ( qtype(jper).ne.4 ) write(170+jper,1015)uname(KFEAT) ! -------- Ci.GAP
...
*     gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg
 ...
 2099 format(77('-')/
     ...
     &7x,' Observed     ',F13.2,3x,a13,F13.2,i12/77('-'))
      ...
      !write(170+J,6399)int(A12P),SET5
...
*     (30) = 'Fish farms (62)'
      ...
      twloadsapp(kount bodies,jp,j13,01) = LMcontrib(01,jp,1) ! all discharges (3,12,5,3,60,61,62)
      
      twloadsapp(kount bodies,jp,j13,02) = LMcontrib(02,jp,1) ! sewage effluents (3) 
      twloadsapp(kount bodies,jp,j13,03) = LMcontrib(03,jp,1) ! intermittent discharges of sewage (12)
      twloadsapp(kount bodies,jp,j13,04) = LMcontrib(04,jp,1) ! industrial discharges (5) 
      twloadsapp(kount bodies,jp,j13,05) = LMcontrib(05,jp,1) ! mine waters (39)
      twloadsapp(kount bodies,jp,j13,17) = LMcontrib(17,jp,1) ! streams (2) and headwaters (10)
      twloadsapp(kount bodies,jp,j13,18) = LMcontrib(18,jp,1) ! Diffuse input (13)
      twloadsapp(kount bodies,jp,j13,19) = LMcontrib(19,jp,1) ! Diffuse input (15)
      twloadsapp(kount bodies,jp,j13,20) = LMcontrib(20,jp,1) ! Reach diffuse (no Feature code)

      twloadsapp(kount bodies,jp,j13,28) = LMcontrib(28,jp,1) ! other discharges (60)
      twloadsapp(kount bodies,jp,j13,29) = LMcontrib(29,jp,1) ! private waste waters (61)
      twloadsapp(kount bodies,jp,j13,30) = LMcontrib(30,jp,1) ! fish farms (62)
      
      twloadsapp(kount bodies,jp,j13,06) = LMcontrib(06,jp,1) ! livestock (25)
      twloadsapp(kount bodies,jp,j13,07) = LMcontrib(07,jp,1) ! arable (27)
      twloadsapp(kount bodies,jp,j13,08) = LMcontrib(08,jp,1) ! highway runoff (29)
      twloadsapp(kount bodies,jp,j13,09) = LMcontrib(09,jp,1) ! urban runoff (31)
      twloadsapp(kount bodies,jp,j13,10) = LMcontrib(10,jp,1) ! atmosphere deposition (33)
      twloadsapp(kount bodies,jp,j13,11) = LMcontrib(11,jp,1) ! natural background (35)
      twloadsapp(kount bodies,jp,j13,12) = LMcontrib(12,jp,1) ! septic tanks (37)
      twloadsapp(kount bodies,jp,j13,13) = LMcontrib(13,jp,1) ! aggregate CSOs (40)
      twloadsapp(kount bodies,jp,j13,14) = LMcontrib(14,jp,1) ! aggregated STWs (42)
      twloadsapp(kount bodies,jp,j13,15) = LMcontrib(15,jp,1) ! diffuse mines (46) 
      twloadsapp(kount bodies,jp,j13,16) = LMcontrib(16,jp,1) ! birds, boats and angling (48)
      twloadsapp(kount bodies,jp,j13,23) = LMcontrib(23,jp,1) ! user-defined (50)
      twloadsapp(kount bodies,jp,j13,24) = LMcontrib(24,jp,1) ! user-defined (52)
      twloadsapp(kount bodies,jp,j13,25) = LMcontrib(25,jp,1) ! user-defined (54)
      twloadsapp(kount bodies,jp,j13,26) = LMcontrib(26,jp,1) ! user-defined (56)
      twloadsapp(kount bodies,jp,j13,27) = LMcontrib(27,jp,1) ! user-defined (58)
      twloadsapp(kount bodies,jp,j13,30) = LMcontrib(30,jp,1) ! fish farms (62)
     
      twloadsapp(kount bodies,jp,j13,NTD) = LMcontrib(NTD,jp,1) ! overall total (NTD)
      twloads(kount bodies,jp,j13) = LMcontrib(NTD,jp,1)        ! overall total (NTD)
      twloadsapp(kount bodies,jp,j13,NTF) = LMcontrib(NTF,jp,1) ! total diffuse (NTF)
      
*     remove the loads coming from the upstream sub-catchments -----------------
*     subtract all upstream sub-catchments in connected with this one ----------
      if ( kount bodies .gt. 1 ) then ! ========================================
      do 102 kbody = 1, kount bodies - 1 ! bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
      ibodies = kount bodies - kbody ! bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
      irech = identify reaches(1,ibodies) ! identify reach number
      iupr1 = identify reaches(2,ibodies) ! identify upstream reach number
      iupr2 = identify reaches(3,ibodies) ! identify upstream reach number

*     remove the loads from the upstream sub-catchments ------------------------
      if ( ibodies .lt. kount bodies ) then
      twloads(kount bodies,jp,j13) = 
     &twloads(kount bodies,jp,j13) - twloads(ibodies,jp,j13)
      twloadsapp(kount bodies,jp,j13,NTD) =
     &twloadsapp(kount bodies,jp,j13,NTD) -
     &twloadsapp(ibodies,jp,j13,NTD)
      endif
      
*     change the load from upstream sub-catchments -----------------------------
      do iip = 1, n2prop ! pppppppppppppppppppppppppppppppppppppppppppppppppppppp
      ip = seqappbodies (iip) ! set the sequence of inputs +++++++++++++++++++++
*     change breakdowns of loads from upstream sub-catchments ------------------
      
      if ( twloadsapp(kount bodies,jp,j13,ip) .gt. 0.0000001 ) then ! ==========
      if ( nbodyprop(ip) .eq. 0 ) then ! number of feature type ================
*     (02) = 'Added by sewage effluents                      (3)'  1 
*     (03) = 'Intermittent discharges of sewage             (12)'  2
*     (04) = 'Added by industrial discharges                 (5)'  3 
*     (05) = 'Added by mine waters                          (39)'  4
*     (28) = 'Other point sources                           (60)'  5
*     (29) = 'Private wastewaters                           (61)'  6
*     (30) = 'Fish farms                                    (62)'  7
*     (01) = 'Mean from all discharges      (3,12,5,39,60,61,62)'  8
                    
      if ( ip.eq.02 .or. ip.eq.03 .or. ip.eq.04 .or. ip.eq.05 .or. ! ~~~~~~~~~~~
     &ip .eq. 01 .or. 
     &ip .eq. 17 .or. 
     &ip.eq.28 .or. ip.eq.29 .or. ip.eq.30 ) then ! ~~~~~~~~~~~~
      
      twloadsapp(kount bodies,jp,j13,ip) = 
     &twloadsapp(kount bodies,jp,j13,ip) - twloadsapp(ibodies,jp,j13,ip)

      endif ! if ( ip.eq.02 etc ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     
      else ! nbodyprop(ip) is greater than zero=================================
*     remove the load of upstream water bodies ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      !twloadsapp(kount bodies,jp,j13,ip) = 
      !&twloadsapp(kount bodies,jp,j13,ip) - twloadsapp(ibodies,jp,j13,ip)
      endif ! if ( nbodyprop(ip) .eq. 0 ) ======================================
      endif ! if ( twloadsapp(kount bodies,jp,j13,ip) .gt. 0.0000001 ) =========
      
      enddo ! do ip = 1, n2prop pppppppppppppppppppppppppppppppppppppppppppppppp
      
  102 continue ! do 102 ibodies = 1, kount bodies - 1 bbbbbbbbbbbbbbbbbbbbbbbbbb
      
      endif ! if ( kount bodies .gt. 1 ) then ==================================
...
*     flow gauge ---------------------------------------------------------------
 ...
 3045 format(77('=')/
     ...
     &38X,'95-percentile low flow =',a10,1x,A4/77('='))
      ...
      if ( ical .eq. 1 .or. ical .eq. 2 ) then ! whilst gap checking river flows
      write(170+jper,3445)uname(feeture),rname(ireach),
     &valchars10,FUNIT,valchars11,FUNIT
 3445 format(/77('=')/
     &'At ',a35,' on Reach ... ',a20/77('-')/
     &'Observed flow in the river ...',26X,'Mean =',a10,1x,a4/
     &38X,'95-percentile low flow =',a10,1x,A4/77('='))
      else ! if ( ical .ne. 1 ) ------------------------------------------------
      if ( KSIM .eq. 1 ) then ! ------------------------------------------------
      write(170+jper,3645)valchars10,valchars11,FUNIT,uname(feeture)
 3645 format('Observed mean & 95-percentile low flow ...',a10,
     &' & ',a10,1x,A4,11x,a35)
      endif ! if ( KSIM .eq. 1 ) -----------------------------------------------
      endif ! if ( ical .ne. 1 ) -----------------------------------------------
 ...
 3145 format(77('+')/
     ...
     &38X,'95-percentile low flow =',a10,1x,A4/77('+'))
      ...
      if ( ical13 .eq. 0 .or. ical .eq. 1 ) then ! =============================
      do jper = 1,ndet ! -------------------------------------------------------
      if ( qtype(jper).ne.4 ) then ! -------------------------------------------
      if ( KSIM .eq. 1) then ! -------------------------------------------------
      write(170+jper,9545)valchars10,FUNIT,valchars11,FUNIT
 9545 format(77('+')/
     &'Calculated flow in the river ...',24X,'Mean =',a10,1x,a4/
     &38X,'95-percentile low flow =',a10,1x,A4/77('+'))
      endif
      ...
      enddo ! do jper = 1,ndet -------------------------------------------------
      endif ! if ( ical13 .eq. 0 .or. ical .eq. 1 ) ============================

      
      do jper = 1,ndet ! -------------------------------------------------------
      if ( qtype(jper).ne.4 ) then ! -------------------------------------------
      if ( KSIM .eq. 1) then ! -------------------------------------------------
      write(170+jper,3545)valchars10,
     &valchars11,FUNIT,uname(feeture)
 3545 format(
     &'Calculated mean & 95-percentile flow ...  ',a10,' & ',
     &a10,1x,A4,11x,a35/110('-'))
      endif ! if ( KSIM .eq. 1) ------------------------------------------------
      endif !if ( qtype(jper).ne.4 ) -------------------------------------------
      enddo ! do jper = 1,ndet -------------------------------------------------
...
*     small stream or tributary ------------------------------------------------
 ...
 1009 format(/110('-')/
     &'River quality just upstream of tributary ...'/110('-'))
      !if ( ical13 .eq. 0 ) then
      !do jper = 1,ndet
      !if ( qtype(jper).ne.4 ) write(170+jper,1009)
      !enddo
      !endif
      endif ! if ( nobigout .le. 0 ) ===========================================
...
*     --------------------------------------------------------------------------     
      ...
      if ( JSKIP .eq. 0 ) then
      xnum = QNUM(IQ,JP)
      call get sampling rates for river quality ! tributary (2) ----------------
     &(fupmx,C(JP,1),QUALN(JP),EFM,ECM,xnum,IQDIST,0)

*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      idnum = 17 ! tributary (2)
      call get sampling rates for river quality ! tributary (2) ----------------
     &(fupmx,CMcontrib(idnum,JP,1),QUALNB(JP,idnum),EFM,ECM, ! tributary (2) ---
     &xnum,IQDIST,1)
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      
      endif ! if ( Qtype (JP) .eq. 4 ) -----------------------------------------
 6957 continue ! do 6957 JP = 1, ndet ==========================================
      
      call calculate summaries of river flow ! tributary (2)

      do JP = 1, ndet ! -------------------------------------------------------
      if ( Qtype (JP) .eq. 4 ) then ! ------------------------------------------
      call accumulate the loads ! ------ from tributary (2) and this determinand
      endif ! if ( Qtype (JP) .eq. 4 ) -----------------------------------------
      enddo ! do JP = 1, ndet --------------------------------------------------

      call get summaries of river quality from the shots ! tributary (2) -------
...
*     write out the results downstream for graph-plotting ----------------------
   ...
   47 continue
      ...
      if ( ical13 .eq. 0 ) then
      if ( nobigout .le. 0 ) then
      if ( ical13 .eq. 0 ) write(01,1023)
      !do jper = 1,ndet
      !if ( qtype(jper).ne.4 ) write(170+jper,1023) ! ------------------- Ci.GAP
      !enddo
 1023 format(110('-')/'River quality d/s of tributary ...'/110('-'))
    ...
    4 format(/77('-')/
     ...
     &1x,A4/46X,'95% exceedence =',a10,1x,A4/77('-'))
      !do jper = 1,ndet
      !if ( qtype(jper).ne.4 ) write(170+jper,5)valchars10,FUNIT,
      !&valchars11,FUNIT
      !enddo

      !5 format('Flow in river just downstream ...',23X,'Mean =',a10,
     ...
     &     JT (feeture) .eq. 62 ) then
      ...
      do jper = 1,ndet
      if ( qtype(jper) .ne. 4 ) ! ----------------------------------------------     
     &write(150+jper,4)valchars10,FUNIT,valchars11,FUNIT ! -------------- Di.EFF
      enddo
...
*     identify the file with the monthly data for river quality ------------------
 ...
 1822 format(33x,'Diffuse ',a11,' ... ',11x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4)
      ...
      if ( PDRC(IQ,J) .eq. 10 ) then
      write(33,1882)DNAME(J),PDRC(IQ,J),IQ,valchars10,UNITS(J),
     &valchars11,UNITS(J),quolity data(IQ,J,3),quolity data(IQ,J,4),
     &quolity data(IQ,J,5),quolity data(IQ,J,6)
 1882 format(110('=')/'Diffuse input from power curve for ',
     &a11/110('-')/'Data-type: 'i5,'     Data-set: 'i5/110('-')/
     &'Mean concentration ',a10,1x,a4/        
     &'Standard Deviation ',a10,1x,a4/
     &'       Power Index ',f10.3/
     ...
     &'       Correlation ',f10.3/110('=')/)
      ...
      write(200+J,1822)DNAME(J),valchars11,valchars10,UNITS(J)
      endif
      endif
 ...
 1823 format(33x,'Diffuse ',a11,' ... ',11x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4,'   (Non-parametric)')
      write(200+J,1823)DNAME(J),valchars11,valchars10,UNITS(J)
      endif
      endif
 ...
 1825 format(33x,'Diffuse ',a11,' ... ',19x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4,'   (Monthly distributions)')
      write(200+J,1825)DNAME(J),valchars11,valchars10,UNITS(J)
      endif
      endif
 ...
 1922 format(33x,'Diffuse ',a11,' LOADS ',9x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4)
      write(200+J,1922)DNAME(J),valchars11,valchars10,LUNITS(J)
      endif
      endif
 ...
 1923 format(33x,'Diffuse ',a11,' ... ',18x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4,'   (Non-parametric loads)')
      write(200+J,1923)DNAME(J),valchars11,valchars10,LUNITS(J)
      endif
      endif
 ...
 1824 format(33x,'Diffuse ',a11,' ... ',11x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4,'   (Monthly structure)')
      write(200+J,1824)DNAME(J),valchars11,valchars10,UNITS(J)
      endif
 ...
 2824 format(33x,'Diffuse ',a11,' ... ',18x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4,'   (Monthly structure)')
      write(200+J,2824)DNAME(J),valchars11,valchars10,LUNITS(J)
      endif
...
*     identify the file with the monthly structure data for river quality ------
 ...
 1916 format(33x,77('.')/33x,
     &'These data have a monthly structure ',
     &'(type 8) ... File: ',a64)
      write(200+J,1916) FLSTRUCTsmall(2,icod)
      endif
      endif !  ! monthly structure
 3974 continue
 ...
 1822 format(33x,a11,' ... ',19x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4)
      ...
      write(200+ic,1822)DNAME(J),valchars11,valchars10,UNITS(J)
      endif
      enddo
      endif
      endif
 ...
 1829 format(33x,a11,' LOADS ',17x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4)
      write(200+J,1829)DNAME(J),valchars11,valchars10,LUNITS(J)
      endif
      endif
 ...
 1723 format(33x,a11,' ... ',19x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4,'   (Non-parametric loads)')
      write(200+J,1723)DNAME(J),valchars11,valchars10,LUNITS(J)
      endif
      endif
...
*     targets -------------------------------------------------------------- SGR
  ...
  304 format(1x,a4,I1,A9,1(',',1PE11.4)) ! --------------------------------- SGR
      endif ! if ( QTYPE (J) .ne. 4 ) then ! +++++++++++++++++++++++++++++++ SGR
      enddo ! do j = 1, MP10 ! +++++++++++++++++++++++++++++++++++++++++++++ SGR

 1492 continue
      
      if ( JTK .eq. 24 .or. JTK .eq. 6  .or. JTK .eq. 45 .or. ! ========= Gi.CSV
     &     JTK .eq. 25 .or. JTK .eq. 27 .or. JTK .eq. 29 .or. ! ========= Gi.CSV 
     ...
     &     JTK .eq. 55 .or. JTK .eq. 57 .or. JTK .eq. 59 ) goto 3492 ! == Gi.CSV

      do j = 1, MP10 ! ++++++++++++++++++++++++++++++++++++++++++++++++++ Gi.CSV
      if ( QTYPE (J) .ne. 4 ) then ! ++++++++++++++++++++++++++++++++++++ Gi.CSV
      if ( JT(KFEAT) .eq. 1 ) then ! for monitoring stations ============ Gi.CSV
      if ( XA(j)+X90(j)+X95(j)+X99(j) .lt. 0.000001 ) then ! ............ Gi.CSV
      write(230+j,311)UNAME(KFEAT),RNAME(JREACH(KFEAT)), !--------------- Gi.CSV
     &jreach(kfeat),
  ...
  311 format(A40,',',A16,',',I4,',',i4,200(',',1PE11.4)) ! -------------- Gi.CSV
      write(230+j,361)RNAME(JREACH(KFEAT)), !---------------------------- Gi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)),
     &rlength(JREACH(KFEAT)),Length of main river,
     &(CL(JCP,J),JCP=1,12),(CD(JCP,J),JCP=1,12), ! ---------------------- Gi.CSV
     ...
     &(LMcontrib(ip,J,1),ip=1,n2prop),  ! ------------------------------- Gi.CSV
     &(CMcontrib(ip,J,1),ip=1,n2prop)  ! -------------------------------- Gi.CSV
  361 format('d/s of above',',',A16,',',I4,',',i4,200(',',1PE11.4)) ! --- Gi.CSV
      else ! if ( XA(j)+X90(j)+X95(j)+X99(j) .ge. 0.000001 ) ...................
      write(230+j,311)UNAME(KFEAT),RNAME(JREACH(KFEAT)), ! -------------- Gi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)),
     &rlength(JREACH(KFEAT)),Length of main river,
     ...
     &X95(j),X95l(j),X95u(j),X99(j),X99l(j),X99u(j) !  observed values -- Gi.CSV
      write(230+j,361)RNAME(JREACH(KFEAT)), !---------------------------- Gi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)), !      +1.0,
     &rlength(JREACH(KFEAT)),Length of main river,
     &(CL(JCP,J),JCP=1,12),(CD(JCP,J),JCP=1,12), ! ---------------------- Gi.CSV
     &FLOW(1),flow(3),FLOW(2),flow(4),RQO(J),confail(J), ! -------------- Gi.CSV
     &(in class(iclass,j),iclass=1,nclass), ! --------------------------- Gi.CSV
     &(LMcontrib(ip,J,1),ip=1,n2prop), ! -------------------------------- Gi.CSV
     &(CMcontrib(ip,J,1),ip=1,n2prop) ! --------=------------------------ Gi.CSV
      endif ! if ( XA(j)+X90(j)+X95(j)+X99(j) .lt. 0.000001 ) ..................
      else ! for features that are not monitoring stations =====================
      if ( XA(j)+X90(j)+X95(j)+X99(j) .lt. 0.000001 ) then ! ...................
      write(230+j,305)RNAME(JREACH(KFEAT)), !---------------------------- Gi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)),
     &rlength(JREACH(KFEAT)),Length of main river,
     &(CL(JCP,J),JCP=1,12),(CD(JCP,J),JCP=1,12), ! ---------------------- Gi.CSV
     ...
     &(LMcontrib(ip,J,1),ip=1,n2prop), ! -------------------------------- Gi.CSV
     &(CMcontrib(ip,J,1),ip=1,n2prop) ! --------------------------------- Gi.CSV
  305 format('d/s of above',',',A16,',',I4,',',i4, ! -------------------- Gi.CSV
     &200(',',1PE11.4)) ! ----------------------------------------------- Gi.CSV
      else ! if ( XA(j)+X90(j)+X95(j)+X99(j) .lt. 0.000001 ) ...................
      write(230+j,305)RNAME(JREACH(KFEAT)), ! --------------------------- Gi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)),
     &rlength(JREACH(KFEAT)),Length of main river,
     ...
     &(CMcontrib(ip,J,1),ip=1,n2prop) ! --------------------------------- Gi.CSV
      ...
      endif ! for features that are not monotoring stations ============= Gi.CSV
*     =================================================================== Gi.CSV
      endif ! if ( QTYPE (J) .ne. 4 ) then ! ++++++++++++++++++++++++++++ Gi.CSV
      enddo ! do j = 1, MP10 ! ++++++++++++++++++++++++++++++++++++++++++ Gi.CSV
      
 3492 continue
      if ( JTK .eq. 45 .or. ! =========================================== Wi.CSV
     &     JTK .eq. 25 .or. JTK .eq. 27 .or. JTK .eq. 29 .or. ! ========= Wi.CSV
     ...
     &     JTK .eq. 55 .or. JTK .eq. 57 .or. JTK .eq. 59 ) goto 7492 ! == Wi.CSV
      do j = 1, MP10 ! ++++++++++++++++++++++++++++++++++++++++++++++++++ Wi.CSV
      if ( QTYPE (J) .ne. 4 ) then ! ++++++++++++++++++++++++++++++++++++ Wi.CSV
*     =================================================================== Wi.CSV
      if ( JT(KFEAT) .eq. 1 ) then ! for monitoring stations ============ Wi.CSV
      if ( XA(j)+X90(j)+X95(j)+X99(j) .lt. 0.000001 ) then ! ............ Wi.CSV
      write(240+j,711)UNAME(KFEAT),RNAME(JREACH(KFEAT)), !--------------- Wi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)),
     &rlength(JREACH(KFEAT)),Length of main river,
  ...
  711 format(A40,',',A16,',',I4,',',i4,200(',',1PE11.4)) ! -------------- Wi.CSV
      else ! if ( XA(j)+X90(j)+X95(j)+X99(j) .ge. 0.000001 ) ...................
      write(240+j,711)UNAME(KFEAT),RNAME(JREACH(KFEAT)), ! -------------- Wi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)),
     ...
     &quolity data(IQ,j,MO+1),quolity data(IQ,j,MO+2), ! ---------------- Wi.CSV
     &quolity data(IQ,j,MO+3) ! ----------------------------------------- Wi.CSV
      endif ! if ( XA(j)+X90(j)+X95(j)+X99(j) ........................... Wi.CSV
      
      else ! for features that are not monitoring stations ============== Wi.CSV
          
      write(240+j,711)UNAME(KFEAT),RNAME(JREACH(KFEAT)), !--------------- Wi.CSV
     &jreach(kfeat),
     &JT(KFEAT),Length of main river-adist(jreach(kfeat)), !------------- Wi.CSV
     &rlength(JREACH(KFEAT)),Length of main river, !--------------------- Wi.CSV
     &(CL(JCP,J),JCP=1,12),(CD(JCP,J),JCP=1,12), ! ---------------------- Wi.CSV
     &FLOW(1),flow(3),FLOW(2),flow(4),RQO(J),confail(J), ! -------------- Wi.CSV
     &(in class(iclass,j),iclass=1,nclass), ! --------------------------- Wi.CSV
     &(LMcontrib(ip,J,1),ip=1,n2prop),  ! ------------------------------- Wi.CSV
     &(CMcontrib(ip,J,1),ip=1,n2prop)  ! -------------------------------- Wi.CSV
      endif ! for features that are not monotoring stations ============= Wi.CSV
*     =================================================================== Wi.CSV
      XAl(j) = 0.0
      X90l(j) = 0.0
      X95l(j) = 0.0
      ...
      X95u(j) = 0.0
      X99u(j) = 0.0
      endif ! if ( QTYPE (J) .ne. 4 ) then ! ++++++++++++++++++++++++++++ Wi.CSV
      enddo ! do j = 1, MP10 ! ++++++++++++++++++++++++++++++++++++++++++ Wi.CSV
 ...
 7492 continue ! ===============================================================
      ...
      write(72,2942)
     &UNAME(KFEAT),Length of main river-adist(jreach(kfeat)),
 ...
 2947 format("D/S ",A40,f8.2,10f9.1)
...
*     calculations for features - effluent discharges --------------------------
      ...
      i = JT(feeture)
*     number of apportionment group for this type of feature -------------------
*     numpfeet - number of apportionment group for this type of feature --------
*     nbodyprop - numbers of feature type in the current water body ------------
      nbodyprop (numpfeet(i)) = nbodyprop (numpfeet(i)) + 1

      fupmx = Flow(1) ! mean upstream river flow 
      fup95 = Flow(2)
...
*     the correlation coefficient is set to zero for industrial effluents ------
      ...
      if ( JT(KFEAT) .eq. 62 ) CO2 = 0.7 ! fish farms
...
*     mean and standard deviation for logged variables -------------------------
      ...
      ECX = get effluent percentile (ECshots) ! find the percentile ------------
...
*     call get effluent quality 95 percentile
      ...
      if ( QTYPE (jp) .ne. 4 ) then
      if ( MONQ .gt. 1 ) call write shots for river quality two ! effluent
      if ( MONQ .gt. 1 ) call write shots for river flow three ! effluent
      endif
...
*     check whether to exclude intermitent discharges (12) =====================
      ...
      kdecision(JP) = 12 ! retain current quality of effluent
      if ( nobigout .le. 0 ) then
      write(01,7875)DNAME(JP) ! -------------------------------------------- OUT
      write(31,7875)DNAME(JP) ! -------------------------------------------- EFF
      write(150+JP,7875)DNAME(JP) ! ------------------------------------- Di.EFF
 7875 format('Intermittent discharge is excluded from ',
     &'meeting the target for: ',A11/
     &'The current discharge quality has been retained ... ')      
      istx = mrqs(JP)      
      namestat = 'mean'
      if ( istx .gt. 0 .and. istx .lt. 7 ) then
      ...
      write(01,7475)namestat ! --------------------------------------------- OUT
      write(31,7475)namestat ! --------------------------------------------- EFF
      write(150+JP,7475)namestat,istx ! --------------------------------- Di.EFF
 7475 format(
     &'The downstream river quality target is a ',a15,i7/77('#'))       
      else
      write(01,7885) ! ----------------------------------------------------- OUT
      write(31,7885) ! ----------------------------------------------------- EFF
      write(150+JP,7885) ! ---------------------------------------------- Di.EFF
 7885 format(77('#'))       
      endif
...
*     fish farms(62) are excluded ==============================================
      if ( JT(KFEAT) .eq. 62 .and. mrqs(JP) .lt. 6 ) then ! exclude discharge ==
      kdecision(JP) = 12 ! retain current quality of effluent
      if ( nobigout .le. 0 ) then
      write(01,7775)DNAME(JP) ! -------------------------------------------- OUT
      write(31,7775)DNAME(JP) ! -------------------------------------------- EFF
      write(150+JP,7775)DNAME(JP) ! ------------------------------------- Di.EFF
 7775 format('Fish farms are is excluded from ',
     &'meeting the target for: ',A11/
     &'The current discharge quality has been retained ... ')
      ...
      if ( istx .eq. 5 ) namestat = '10-percentile'
      if ( istx .eq. 6 ) namestat = '99-percentile'
      write(01,8475)namestat ! --------------------------------------------- OUT
      write(31,8475)namestat ! --------------------------------------------- EFF
      write(150+JP,8475)namestat ! -------------------------------------- Di.EFF
 8475 format(
     &'The downstream river quality target is a ',a15/77('#'))       
      else
      write(01,7575) ! ----------------------------------------------------- OUT
      write(31,7575) ! ----------------------------------------------------- EFF
      write(150+JP,7575) ! ---------------------------------------------- Di.EFF
 7575 format(77('#'))       
      endif
...
*     of river quality ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      cold(JP,27) = eload(JP,i13) ! set final load for the discharge
      call load calculation per determinand ! 
      cold(JP,25) = XLOAD(JP,1,i13) ! set final load for the d/s river
...
*     of river quality ---------------------------------------------------------
      ...
      call load calculation per determinand ! calculate XLOAD
...
*     ( 2) = 'Added by sewage effluents (3)'
*     ( 3) = 'Added by intermittent discharges of sewage (12)'
*     ( 4) = 'Added by industrial discharges (5)'
*     ( 5) = 'Added by mine waters (39)         '
      if ( JT(KFEET) .eq. 03 ) idnum = 2
      if ( JT(KFEET) .eq. 05 ) idnum = 4
      if ( JT(KFEET) .eq. 60 ) idnum = 28
      if ( JT(KFEET) .eq. 61 ) idnum = 29
      if ( JT(KFEET) .eq. 62 ) idnum = 30
      if ( JT(KFEET) .eq. 12 ) idnum = 3
      if ( JT(KFEET) .eq. 39 ) idnum = 5
      usCMcont = CMcontrib(idnum,JP,1) ! store the upstream value for idnum
      usCMcont1 = CMcontrib(1,JP,1) ! store the upstream value for total
...
*     ( 4) = 'Added by industrial discharges (5)'
*     ( 5) = 'Added by mine waters (39)         '
      if ( JT(KFEET) .eq. 3 ) idnum = 2
      if ( JT(KFEET) .eq. 5 ) idnum = 4
      if ( JT(KFEET) .eq. 60 ) idnum = 28
      if ( JT(KFEET) .eq. 61 ) idnum = 29
      if ( JT(KFEET) .eq. 62 ) idnum = 30
      if ( JT(KFEET) .eq. 12 ) idnum = 3
      if ( JT(KFEET) .eq. 39 ) idnum = 5
      if ( idnum > 0 ) then
      call get sampling rates for river quality ! effluent discharges (3, etc) -
     &(fupmx,usCMcont,QUALNB(JP,idnum),EFM,ECM,xnum,IQDIST,1) ! (3, etc) -------

      endif ! if ( idnum > 0 ) then
      idnum = 1
      call get sampling rates for river quality ! total discharge concentration
     &(fupmx,usCMcont1,QUALNB(JP,idnum),EFM,ECM,xnum,IQDIST,1)
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      endif ! if ( JSKIP .eq. 0 ) ==============================================

 6930 continue ! do 6930 JP = 1, NDET ---- the loop on determinands has finished

      if ( Mtargit .lt. 2 )then
      call update summaries of contribution ! --------------- effluent discharge !!!!!!!!!!!!!!!!!!!!!!
      endif
...
*     write out results d/s of discharge for graph plotting --------------------
      ...
      write(150+J,2823)uname(feeture) ! --------------------------------- Di EFF
 2823 format('River quality downstream of the discharge ... ',a37/ ! ---- Di EFF
     &110('-')) ! ------------------------------------------------------- Di EFF

      
*     iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
      if ( JT(feeture) .eq. 12 ) then ! iiiiiiiiiiii intermittent discharge (12)  
*     iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
      if ( detype(J) .lt. 900 ) then ! iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
      if ( MRQS(J) .ne. 4 .and. MRQS(J) .ne. 5 ) then ! iii not dissolved oxygen
          
      if ( MRQS(J) .eq. 2 ) then ! 95-percentile ! iiiiiiiiiiiiiiiiiiiiiiiiiiiii
      ipl = 95 ! 95-percentile
      ipc = 3 ! value of C(J,3) for 95-percentile
      endif ! if ( MRQS(J) .eq. 2 ) --------------------------------------------
      !if ( MRQS(J) .ne. 4 .and. MRQS(J) .ne. 5 ) then ! == not dissolved oxygen
      if ( MRQS(J) .eq. 3 ) then ! 90-percentile ! -----------------------------
      ipl = 90 ! 90-percentile
      ipc = 4 ! value of C(J,4) for 90-percentile
      endif ! if ( MRQS(J) .eq. 3 ) --------------------------------------------
      if ( MRQS(J) .eq. 6 ) then ! 99-percentile -------------------------------
      ipl = 99 ! 99-percentile
      ipc = 5 ! value of C(J,5) for 99-percentile
      endif ! if ( MRQS(J) .eq. 6 ) --------------------------------------------
      
      write(130+J,7512)GIScode(feeture),uname(feeture),dname(J), ! ------ Ei.CSV
     &C(J,1),C(J,2),C(J,ipc),ipl,JT(feeture),uname(feeture) ! ----------- EI.CSV
 7512 format(' ',a40,',',a40,',', ! ------------------------------------- Ei.CSV
     &'Intermittents (12) are excluded from Modes 7 8 or 9',',', ! ------ Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! --- intermittent discharges (12) - Ei.CSV
     &',',a11,3(',',1pe12.4),',',i2,'-percentile',',',i4,',',a35) ! ----- Ei.CSV
       
      else ! if ( MRQS(idet) .ne. 4 .and. MRQS(idet) .ne. 5 ) ==================
      if ( MRQS(J) .eq. 4 ) ! 5-percentile =====================================
     &write(130+J,7562)GIScode(feeture),uname(feeture),dname(J), ! ------ Ei.CSV
     &C(J,1),C(J,2),C(J,3),JT(feeture),uname(feeture) ! ----------------- EI.CSV
 7562 format(' ',a40,',',a40,',', ! ------------------------------------- Ei.CSV
     &'Intermittents (12) are excluded from Modes 7 8 or 9',',', ! ------ Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! --- intermittent discharges (12) - Ei.CSV
     &',',a11,3(',',1pe12.4),',','05-percentile',(',',i4),',',a35) ! ---- Ei.CSV
      if ( MRQS(J) .eq. 5 ) ! 10-percentile ====================================
     &write(130+J,7552)GIScode(feeture),uname(feeture),dname(J), ! ------ Ei.CSV
     &C(J,1),C(J,2),C(J,4),JT(feeture),uname(feeture) ! ----------------- EI.CSV
 7552 format(' ',a40,',',a40,',', ! ------------------------------------- Ei.CSV
     &'Intermittents (12) are excluded from Modes 7 8 or 9',',', ! ------ Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! --- intermittent discharges (12) - Ei.CSV
     &',',a11,3(',',1pe12.4),',','10-percentile',(',',i4),',',a35) ! ---- Ei.CSV
     
      endif ! if ( MRQS(idet) .ne. 4 .and. MRQS(idet) .ne. 5 ) iiiiiiiiiiiiiiiii
      endif ! if ( detype(JP) .lt. 900 ) iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
*     iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii


*     oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
      else ! if ( JT(feeture) .eq. 12 ) ! all other discharges ooooooooooooooooo
      
      if ( MRQS(J) .ne. 4 .and. MRQS(J) .ne. 5 ) then ! ========================
      if ( Kdecision(J) .eq. 12 ) then     
      write(130+J,7556)GIScode(feeture),uname(feeture), ! --------------- Ei.CSV
     &rname(IREACH),dname(J), ! ----------------------------------------- Ei.CSV
     &C(J,1),C(J,2),C(J,3),JT(feeture),uname(feeture) ! ----------------- EI.CSV
      endif
 7556 format(' ',a40,',',a40,',',a16,',', ! ----------------------------- Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! ----------- not used ------------- Ei.CSV
     &',',a11,3(',',1pe12.4),',','95-percentile',(',',i4),',',a35) ! ---- Ei.CSV
      else ! if ( MRQS(J) .ne. 4 .and. MRQS(J) .ne. 5 ) ========================
      if ( MRQS(J) .eq. 4 ) ! 05-percentile ====================================
     &write(130+J,7596)GIScode(feeture),uname(feeture), ! --------------- Ei.CSV
     &rname(IREACH),dname(J), ! ----------------------------------------- Ei.CSV
     &C(J,1),C(J,2),C(J,3),JT(feeture),uname(feeture) ! ----------------- EI.CSV
 7596 format(' ',a40,',',a40,',',a16,',', ! ----------------------------- Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! --------- 5-percentile ----------- Ei.CSV
     &',',a11,3(',',1pe12.4),',','05-percentile',(',',i4),',',a35) ! ---- Ei.CSV
      if ( MRQS(J) .eq. 5 ) ! 10-percentile ====================================
     &write(130+J,7196)GIScode(feeture),uname(feeture), ! --------------- Ei.CSV
     &rname(IREACH),dname(J), ! ----------------------------------------- Ei.CSV
     &C(J,1),C(J,2),C(J,4),JT(feeture),uname(feeture) ! ----------------- EI.CSV
 7196 format(' ',a40,',',a40,',',a16,',', ! ----------------------------- Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! -------- 95-percentile ----------- Ei.CSV
     &',',a11,3(',',1pe12.4),',','10-percentile',(',',i4),',',a35) ! ---- Ei.CSV
      endif ! if ( MRQS(J) .ne. 4 .and. MRQS(J) .ne. 5 ) =======================
      endif ! if ( JT(feeture) .eq. 12 ) ! all other discharges ================
      
      endif ! if ( qtype(J) .ne. 4 ) -------------------------------------------
      enddo ! do J = 1,ndet ----------------------------------------------------
      
      do jper = 1,ndet ! -------------------------------------------------------
      !if ( qtype(jper).ne.4 ) write(170+jper,2023) ! ------------------- Ci.GAP
 2023 format(110('Y'))
      enddo ! do jper = 1,ndet -------------------------------------------------
      
      endif ! if ( nobigout .le. 0 ) ===========================================
      
      call write out loads and quality (6) ! d/s of effluent discharge
...
*     4 poor upstream quality
      ...
      if (itype .eq. 62 ) typename = 'Fish Farm'
     ...
     &'10% Deterioration in Discharge'  
      ...
      xxlode = 0.0
      do is = 1,ns
      xxlode = xxlode+FMS(is)*CMS(JP,is)    
      enddo
      xxlode = xxlode / float(NS)
     ...
     &changerload                  ! Riv_load_mean-down_drop 
      ...
      XxP = cold(JP,20)
      IQDIST = PDEC(IQ,JP)
      call get the summary statistics of discharge quality 2 
     &(JP,XxM,XxS,DCP) 
 ...
 5568 format(110('-')/'Different change in river & discharge ',
     &'load for ',a37,2x,a11,1x,a10,' %'/110('-'))
      write(150+JP,5567)uname(KFEAT),valchars10 ! ----------------------- Di EFF     
 5567 format(110('-')/'Difference in the change in river & discharge ',
     &'load ...',5x,a37,2x,a10,' %'/110('-'))
...
*     print out the input data on discharge quality ----------------------------
      ...
      write(150+idet,2055)IQ,PDEF(IQ),uname(feeture) ! ------------------ Di EFF
      endif
      enddo
 2055 format(/77('=')/
...
*     identify the file with the non-parametric data ---------------------------
 ...
 1965 format('Non-parametric distribution ... File: ',a64/77('-'))
      do idet = 1,ndet
      if ( qtype(idet) .ne. 4 ) then
      write(150+idet,1965)flnamesmall(3,icod) ! ------------------------- Di EFF
      endif
      enddo
      endif
      endif
...
*     identify the file with the monthly data ----------------------------------
 ...
 1995 format(77('-')/'These data are expressed as ',
     &'monthly distributions ... File: ',a64/77('-'))
      ...
      do idet = 1,ndet
      if ( qtype(idet) .ne. 4 ) then
      write(150+idet,1995)FLMONTHsmall(3,icod) ! ------------------------ Di EFF
      endif
      enddo
      endif
...
*    &seas4(i),seas0(i), ! ------ flow
 ...
 1994 continue
    ...
    1 continue
      ...
      if ( JT(feeture) .eq. 60 .or. JT(feeture) .eq. 61 ) xxxc = 0.0
      if ( JT(feeture) .eq. 62 ) xxxc = 0.0
      endif
 ...
 1212 format(77('#')/'Flow discharged as effluent:',
     ...
     &1x,a4/33X,'Correlation with river flow =',f10.4/77('='))
      ...
      do idet = 1,ndet
      if ( qtype(idet) .ne. 4 ) then
      write(150+idet,1212)valchars10,funit,valchars11,funit,xxxc ! ------ Di EFF
      endif
      enddo
      else
 ...
 1012 format(77('=')/'Flow discharged as effluent:',20x,
     ...
     &1x,a4/33X,'Correlation with river flow =',f10.4/77('='))
      ...
      do idet = 1,ndet
      if ( qtype(idet) .ne. 4 ) then
      write(150+idet,1012)valchars10,funit,valchars11,funit,xxxc ! ------ Di EFF
      endif
      enddo
      endif
 ...
 1912 format(41X,'              Shift =',a10,1x,a4/77('='))
      do idet = 1,ndet ! -------------------------------------------------------
      if ( qtype(idet) .ne. 4 ) then
      write(150+idet,1912)valchars10,funit ! ---------------------------- Di EFF
      endif
      enddo ! do idet = 1,ndet ! -----------------------------------------------
...
*     print out the input data on discharge quality ----------------------------
     ...
     &.or. ical .gt. 6 ) then
      ...
      do J = 1, ndet
      if ( qtype(J) .ne. 4 ) then
      write(150+J,2055)IQ,uname(feeture) ! ------------------------------ Di EFF
      endif
      enddo
      endif
...
*     default correlation coefficients -----------------------------------------
      ...
      write(150+J,7576)CO5,EFCL(J),pollution data (IQ,J,MO)
 7576 format('!!!!!!! Correlation =',3f8.4)
     ...
     &CO5 ! ---------------------------------------------------------------- EFF
      write(150+J,1934)DNAME(J),valchars10,UNITS(J),valchars11, ! ------- Di EFF
     &UNITS(J),CO5 ! ---------------------------------------------------- Di EFF
...
*     default correlation coefficients -----------------------------------------
     ...
     &LUNITS(J),CO5 ! ------------------------------------------------------ EFF
      write(150+J,8924)DNAME(J),valchars10,LUNITS(J),valchars11, ! ------ Di EFF
     &LUNITS(J),CO5 ! ------------------------------------------------------ EFF
...
*     identify the file with the non-parametric data ---------------------------
 ...
 1970 continue
     ...
     &LUNITS(J)
      ...
      write(150+J,1965) flnamesmall(4,icod) ! --------------------------- Di EFF
 1965 format(
 ...
 1924 format(/77('=')/a11,37X,' Annual mean =',a10,1x,A4/
     ...
     &UNITS(J)
      write(150+JP,1924)DNAME(J),valchars10,UNITS(J),valchars11, ! ------ Di EFF
     &UNITS(J)
      endif
      write(01,1965) flnamesmall(4,icod)
      write(31,1965) flnamesmall(4,icod) ! --------------------------------- EFF
...
*     identify the file with the monthly data ----------------------------------
 ...
 1990 continue
      ...
      write(150+J,1966)dname(J),FLMONTHsmall(4,icod) ! ------------------ Di EFF
      endif
...
*    &'Monthly input data on discharge quality ... ',a11/77('-')/
     ...
     &'  Correlation'/25x,'  Deviation',12x,
     &'(discharge flow & quality)'/77('-'))
      write(01,2399)(seas1(i),seas2(i),seas3(i),seas4(i),seas0(i), ! --- quality
     &i=1,12)
      write(31,2399)(seas1(i),seas2(i),seas3(i),seas4(i),seas0(i), ! --- quality
     &i=1,12)
      write(150+J,2399)(seas1(i),seas2(i),seas3(i),seas4(i),seas0(i), !  quality
     &i=1,12)
 2399 format(
     &'January ...  ',3f11.2,f13.4,i18/
     &'February ... ',3f11.2,f13.4,i18/
     ...
     &'December ... ',3f11.2,f13.4,i18/77('-'))
      ...
      ECM = pollution data(IQ,J,1) ! mean effluent quality ---------------------
      ECS = pollution data(IQ,J,2) ! standard deviation ------------------------
     ...
     &pollution data(IQ,J,2)) 
      if ( ical .eq. 0 .or. ical .eq. 2. .or. ical .eq. 4 
     &.or. ical .gt. 6 ) then
      write(01,2934)DNAME(J),valchars10,UNITS(J),valchars11,UNITS(J), ! ---- OUT
     &CO5
      write(31,2934)DNAME(J),valchars10,UNITS(J),valchars11,UNITS(J), ! ---- EFF
     &CO5
      write(150+J,2934)DNAME(J),valchars10,UNITS(J),valchars11, ! ---------- EFF  
     &UNITS(J),CO5 ! ---------------------------------------------------- Di EFF
 2934 format(a11,37X,' Annual mean =',a10,1x,A4/
     &42X,'Standard deviation =',a10,1x,A4/
     &13x,'       Correlation (discharge flow and quality) =',f10.4,
     &29x,' (CO5)'/77('='))
      endif
     
*     calculate the correlation coefficient ==================================== 
      call set up monthly data for discharge flow ! EFshots --------------------
      call set up monthly data for discharge quality ! ECshots -----------------

      ECM = get the mean discharge quality (ECshots)
      ECS = get standard deviation of discharge quality (ECshots)
      call get effluent quality 95 percentile

      xsum = 0.0
      do IS = 1, NS
      xsum = xsum + EFshots(IS)*ECshots(IS) 
      enddo
      
      CO5 = (xsum-NS*EFM*ECM)/((NS-1)*FE(IQ,1)*FE(IQ,2))
      pollution data(IQ,J,4) = CO5
      
      pollution data(IQ,J,1) = ECM
      pollution data(IQ,J,2) = ECS
      pollution data(IQ,J,3) = 0.0
      PDEC(IQ,J) = 2 ! change the monthly data to annual log-normal experiment
...
*     print out the input data on discharge quality ----------------------------
      subroutine write fish farm discharge quality
      include 'COMMON DATA.FOR'

      if ( JSKIP .eq. 1 .or. ical .eq. 1 ) return
      if ( ical .eq. 0 .or. ical .eq. 2. .or. ical .eq. 4 
     &.or. ical .gt. 6 ) then
      write(01,2055)IQ,uname(feeture) ! ------------------------------------ OUT
      write(31,2055)IQ,uname(feeture) ! ------------------------------------ EFF
      do J = 1, ndet
      if ( qtype(J) .ne. 4 ) then
      write(150+J,2055)IQ,uname(feeture) ! ------------------------------ Di EFF
 ...
 2055 format(/77('=')/'Loads discharged from fish farm: Set:',I6,
     &3x,a30/77('-')/
     &'(Fish farms are not yet included as part of ',
     &'Modes 7, 8 and 9)'/77('-'))
...
*     loop on the determinands -------------------------------------------------
      ...
      if ( QTYPE (J) .eq. 4 ) goto 6926
*     --------------------------------------------------------------------------
      if ( PDEC(IQ,J) .lt. 4 ) then ! concentrations are treated as loads for --
      if ( nobigout .le. 0 ) then   ! fish farms -------------------------------
      call sort format 2 (pollution data(IQ,J,1),
     &pollution data(IQ,J,2)) 
      if ( ical .eq. 0 .or. ical .eq. 2. .or. ical .eq. 4 
     &.or. ical .gt. 6 ) then
      write(01,1934)DNAME(J),valchars10,LUNITS(J),valchars11, ! ------------ OUT
     &LUNITS(J),EFCL(J) ! -------------------------------------------------- OUT
      write(31,1934)DNAME(J),valchars10,LUNITS(J),valchars11, ! ------------ EFF
     &LUNITS(J),EFCL(J) ! -------------------------------------------------- EFF
      write(150+J,1934)DNAME(J),valchars10,LUNITS(J),valchars11, ! ------ Di EFF
     &LUNITS(J),EFCL(J) ! ----------------------------------------------- Di EFF
 1934 format(a11,26x,' Annual mean load added =',a10,1x,A4/
     &42X,'Standard deviation =',a10,1x,A4/
     &42X,'       Correlation =',f10.4/77('='))
      goto 6926
      endif
      endif ! if ( nobigout .le. 0 )
      endif ! if ( PDEC(IQ,J) .lt. 4 ) concentrations --------------------------
*     --------------------------------------------------------------------------
*     note whether the data are for loads --------------------------------------
      if ( PDEC(IQ,J) .eq. 6 ) then ! normal loads
      if ( nobigout .le. 0 ) then
      call sort format 2 (pollution data(IQ,J,1),
     &pollution data(IQ,J,2)) 
      write(01,8924)DNAME(J),valchars10,LUNITS(J),valchars11,LUNITS(J) ! --- OUT
      if ( ical .eq. 0 .or. ical .eq. 2. .or. ical .eq. 4 
     &.or. ical .gt. 6 ) then
      write(31,8924)DNAME(J),valchars10,LUNITS(J),valchars11, ! ------------ EFF
     &LUNITS(J),EFCL(J) ! -------------------------------------------------- EFF
      write(150+J,8924)DNAME(J),valchars10,LUNITS(J),valchars11, ! ------ Di EFF
     &LUNITS(J),EFCL(J)  ! ---------------------------------------------- Di EFF
 8924 format(a11,26X,'Annual mean load added =',a10,1x,a4/
     &42X,'Standard deviation =',a10,1x,A4/
     &42X,'       Correlation =',f10.4/77('='))
      goto 6926
      endif
...
*     identify the file with the non-parametric data ---------------------------
 ...
 1965 format(
     &'Non-parametric distribution ... File: ',a64/77('-'))
      ...
      call sort format 2 (pollution data(IQ,J,1),
     &pollution data(IQ,J,2)) 
      write(01,1924)DNAME(J),valchars10,UNITS(J),valchars11,LUNITS(J) ! ---- OUT
 1924 format(/77('=')/a11,32X,' Annual mean load =',a10,1x,A4/
     &42X,'Standard deviation =',a10,1x,A4/77('='))
      if ( ical .eq. 0 .or. ical .eq. 2. .or. ical .eq. 4 
     &.or. ical .gt. 6 ) then
      write(31,1924)DNAME(J),valchars10,UNITS(J),valchars11, ! ------------- EFF
     &LUNITS(J)
      write(150+J,1924)DNAME(J),valchars10,UNITS(J),valchars11, ! ------- Di EFF
     &LUNITS(J)
      endif
      write(01,1965) flnamesmall(4,icod)
      write(31,1965) flnamesmall(4,icod) ! --------------------------------- EFF
...
*     identify the file with the monthly data ----------------------------------
 ...
 1990 continue
      if ( nobigout .le. 0 ) then
      write(01,1966)dname(J),FLMONTHsmall(4,icod)
      write(31,1966)dname(J),FLMONTHsmall(4,icod) ! ------------------------ EFF
      write(150+J,1966)dname(J),FLMONTHsmall(4,icod) ! ------------------ Di EFF
      endif
 1966 format(//77('+')/'Discharge loads expressed as monthly ',
     &'distributions ... ',a11/
     &'File: ',a64/77('+'))
...
*     the amount of flow abstracted --------------------------------------------
      ...
      do jdet = 1, ndet ! ====== effect of abstraction on discharge loads -- 160
      if ( qtype (jdet) .ne. 4 ) then ! ----------------------------------------
      do iworks = 1, kountworks
      kprune det = jdet
      TELODEshots(iworks,jdet,is) = ! - effect of abstraction on discharge loads
     &TELODEshots(iworks,jdet,is) * (FMS(IS)/oldFF) ! ---- effect of abstraction
      prune shots (kprune det, is) = FMS(IS)/oldFF
      enddo ! do iworks = 1, kountworks
      endif ! if ( qtype (jdet) .ne. 4 ) ---------------------------------------
      enddo ! do jdet = 1, ndet ============================================ 160
...
*     accumulate total loads removed by abstractions ===========================
      ...
      do J1 = 1, nx ! ----------------------------------------------------------   
      TBLODE2(jdet,J1) = TBLODE2(jdet,J1) - abload(jdet,J1) ! ----- abstractions 
*     calculate the effect of losses on accumulated loads ======================
      prune load (J1) = 1.0 ! ------------------------------------- abstractions 
      if ( abload(jdet,J1) .gt. 0.0 ) then
      if ( abs ( LMcontrib(NTD,jdet,1) ) .gt. 0.000000001 ) then
      if ( LMcontrib(NTD,jdet,1) - abload(jdet,J1) .gt. 0.0 ) then
      prune load (J1) = (LMcontrib(NTD,jdet,1) - abload (jdet,J1)) ! abstraction
     &                /  LMcontrib(NTD,jdet,1)
      endif ! if ( abs ( LMcontrib(NTD,jdet,1) ) .gt. 0.000000001 ) ------------
      endif ! if ( LMcontrib(NTD,jdet,1) - abload(jdet,J1) .gt. 0.0 ) ----------
      endif ! if ( abload(jdet,J1) .gt. 0.0 ) ----------------------------------
      enddo ! do J1 = 1, nx ----------------------------------------------------
      
      
*     wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
*     remove the abstracted loads from individual discharges wwwwwwwwwwwwwwwwwww
      if ( kountworks .gt. 0 ) then ! wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwW
      do 5400 iw = 1, kountworks ! wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwW
      
      ip = 0
      jworks = identify werks(iworks) ! WARNING
      if ( JT(jworks) .eq. 03 ) ip = 2
      if ( JT(jworks) .eq. 12 ) ip = 3
      if ( JT(jworks) .eq. 05 ) ip = 4
      if ( JT(jworks) .eq. 39 ) ip = 5
      if ( JT(jworks) .eq. 60 ) ip = 28
      if ( JT(jworks) .eq. 61 ) ip = 29
      if ( JT(jworks) .eq. 62 ) ip = 30
      if ( ip .eq. 0 ) goto 5400

      do IS = 1, NS ! ----------------------------------------------------------
      Y(IS) = CTMS(ip,jdet,IS) ! concentrations from types of feature ----------
      enddo ! do IS = 1, NS ----------------------------------------------------

      call calculate summaries of contribution (ip,jdet,Y,1) ! after abstraction
      call use shots to get mean load for works (iw,jdet,wloadm)
      
      TELOADAV(iw,jdet) = wloadm
      
 5400 continue ! do iw = 1, kountworks wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwW
...
*     trim back the loads  -------------------------------- abstractions of flow
      ...
      call update summaries of contribution ! -------------- abstraction of flow
...
*     include regulation with tributaries (Feature 17) -------------------------      
      CTMS(17,JP,IS) = ((FMS(is)*CTMS(17,JP,IS)) + (EC*FADD)) /fxx !  regulation
*     include regulation with total concentrations -----------------------------      
      CTMS(NTD,JP,IS) = ((FMS(is)*CTMS(NTD,JP,IS)) + (EC*FADD)) /fxx !regulation

*     dilute the remaining contributions ----------------- river flow regulation
      do ip = 1, n2prop ! ================================ river flow regulation
      if ( ip .ne. 17 .or. ip .eq. NTD ) then ! exclude streams from this ------
      CTMS(IP,JP,IS) = (FMS(is) * CTMS(IP,JP,IS)) / fxx
      endif
      enddo ! ip = 1, n2prop ===================================================
...
*     compute the new flow, after regulation -----------------------------------
      ...
      do jper = 1,ndet ! -------------------------------------------------------
      if ( qtype(jper).ne.4 ) then ! -------------------------------------------
      !write(170+jper,2013)valchars10,FUNIT,valchars11,FUNIT ! ---------- Ci.GAP
 2013 format('Flow in river just downstream ...',23X, ! ------- river regulation
     &'Mean =',a10,1x,A4/46X,'95% exceedence =',a10,1x,A4)
      !write(170+jper,1023)
 1023 format(/110('-')/
     &'River quality just downstream ... '/110('-'))
      endif ! if ( qtype(jper).ne.4 ) then ------------------------------------
      enddo ! do jper = 1,ndet ! -----------------------------------------------     
      write(01,1023)
...
*     initialise ---------------------------------------------------------------
      ...
      Creg(2,IS) = CMS(jdet,is) ! upstream river quality for regression ########
      enddo
      k1 = 1 ! river flow
      !k2 = 4 ! river quality
      k2 = 2 ! river quality
      ...
      !write(33,1)RRR,quolity data (IQ,jdet,MO),dname(jdet)
    1 format(77('-')/
     ...
     &'(',F8.4,') for ',a11/77('-'))
      ...
      include 'COMMON DATA.FOR'
      do IS = 1, NS
      cms(JP,IS) = ucms(JP,IS) ! upstream river quality 
      fms(IS) = ufms(IS) ! upstream river flow
      do ip = 1, n2prop
...
*     store the abstracted quality ---------------------------------------------
      ...
      do iworks = 1, kountworks !  ! - effect of abstraction on discharge loads
      kprune det = jdet
      TELODEshots(iworks,jdet,is) = ! - effect of abstraction on discharge loads
     &TELODEshots(iworks,jdet,is) * (FMS(IS)/oldFF) ! ---- effect of abstraction
      prune shots (kprune det, is) = FMS(IS)/oldFF
...
*     accumulate total loads removed by abstractions ==== negative discharge one
      ...
      if ( munthly structure .eq. 0 ) nx = 1 !  no requirement for monthly loads
      do J1 = 1, nx      
      TBLODE2(jdet,J1) = TBLODE2(jdet,J1) - abload(jdet,J1)
*     calculate the effect of losses on accumulated loads ======================
      prune load (J1) = 1.0 ! --------------------------- negative discharge one
      if ( abload(jdet,J1) .gt. 0.0 ) then
      if ( abs ( LMcontrib(NTD,jdet,1) ) .gt. 0.000000001 ) then
      if ( LMcontrib(NTD,jdet,1) - abload(jdet,J1) .gt. 0.0 ) then
      prune load (J1) = (LMcontrib(NTD,jdet,1) - abload  (jdet,J1)) 
     &                /  LMcontrib(NTD,jdet,1)
      endif
      endif ! if ( abs ( LMcontrib(NTD,jdet,1) ) .gt. 0.000000001 )
      endif ! if ( abload(jdet,J1) .gt. 0.0 )
...
*     trim back the loads for abstractions -------------- negative discharge one
      call scale loads after an abstraction ! ----------- negative discharge one
...
*     store the abstracted quality ---------------------------------------------
      ...
      kprune det = jdet
      TELODEshots(iworks,jdet,is) = ! - effect of abstraction on discharge loads
     &TELODEshots(iworks,jdet,is) * (FMS(IS)/START FLOW) ! effect of abstraction
      prune shots (kprune det, is) = FMS(IS)/START FLOW
      enddo ! do iworks = 1, kountworks ---------------------------------------
...
*     trim back the loads for abstractions -------------- negative discharge two
      call scale loads after an abstraction ! ----------- negative discharge two
      endif ! if ( QTYPE (jdet) .ne. 4 ) then ==================================
      enddo ! do jdet = 1, ndet ================================================
   
      if ( MONF.gt.1 ) call write shots for river flow !  negative discharge two
      call calculate summaries of river flow ! ---------- negative discharge two
      if ( IPRINT .eq. 1 .or. JSKIP .eq. 1 )return ! ---- negative discharge two
      call sort format 2 (FLOW(1),FLOW(2))
      if ( nobigout .le. 0 ) write(01,1013)valchars10,FUNIT,
     &valchars11,FUNIT
 1013 format('River flow downstream of the abstraction:',14X,' Mean =',
...
*     FISH FARM (Feature Type 62) ----------------------------------------------
*     The abstraction data are specified as mean and standard deviation --------

      subroutine fish farm ! Feature Type 62
      include 'COMMON DATA.FOR'
      dimension A1load(MP10,13),A2load(MP10,13)
      dimension abmean(MP10),abstdev(MP10),absq95(MP10)
      dimension abvalues(MP10,MS),Y(MS),abmean2(MP10)

      fupmx = FLOW(1) ! store the upstream mean river flow at fish farm
      
*     initialise the abstracted loads XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX NOT USED
      do jdet = 1, ndet
      A1LOAD(jdet,i13) = 0.0
      A2LOAD(jdet,i13) = 0.0
      abmean(jdet) = 0.0
      ...
      ABLOAD (jdet,L13) = 0.0
      enddo
      enddo ! initialise the abstracted loads XXXXXXXXXXXXXXXXXXXXXXXXX NOT USED

*     identify the code number for the fish farm data-set ----------------------
      IF = JQ(KFEAT)
      IQ = JQ(KFEAT)
      if ( IF .lt. 1 .or. IF .gt. NV ) then
      if ( nobigout .le. 0 ) write(01,1020)KFEAT
      if ( iscreen .lt. 3 ) write( *,1020)KFEAT
      write(09,1020)KFEAT
      write(33,1020)KFEAT
 1020 format(/'*** Abstraction data specified for Fish Farm', ! - type (62)
     &I6,' (Feature Type 62) ... do not exist')
      if ( nobigout .le. 0 ) write(01,1006)
      if ( iscreen .lt. 3 ) write( *,1006)
 1006 format('*** No removal made ***'/80('-'))
      return
      endif
...
*     correlation of abstracted flow on main river flow ------------------------
      CO2 = 0.0
*     special correlation ------------------------------------------------------
      if ( FE(IF,MO) .ne. -9.9 ) CO2 = FE(IF,MO) ! correlation coefficient -----
      call set up the correlation coefficients ! ----- fish farm ---------------
      IFDIST = PDEF(IF) ! the type of distribution -----------------------------
      if ( JSKIP .eq. 0 .and. IPRINT .eq. 0 ) then
      if ( nobigout .le. 0 ) then
      if ( IFDIST .gt. 0 ) then
      call sort format 3 (FE(IF,1),FE(IF,2),CO2)
      write(01,2012) valchars10,FUNIT,valchars11,FUNIT,valchars12
 2012 format('Flow removed from the river at this fish farm:', ! - fish farm --
     &8X,'  Mean =',a10,1X,A4/
     &42x,'Standard deviation =',a10,1x,a4/
     &42x,'       Correlation =',a10/77('-'))
      write(31,2012) valchars10,FUNIT,valchars11,FUNIT,valchars12
      do idet = 1, ndet
      if ( qtype (idet) .ne. 4 ) then
      write(150+idet,2012) valchars10,FUNIT,valchars11,FUNIT,valchars12
      endif
      enddo
      
      else ! if ( IFDIST .gt. 0 ) 
      call sort format 1 (FE(IF,1))
      write(01,2812) valchars10,FUNIT
 2812 format('Flow removed from the river at this fish farm:', ! ----- constant
     &6x,'Constant =',a10,1X,A4/77('-'))
      write(31,2812) valchars10,FUNIT ! ----------------------------------- EFF
      do idet = 1, ndet
      if ( qtype (idet) .ne. 4 ) then
      write(150+idet,2812) valchars10,FUNIT ! -------------------------- Di EFF
      endif
      enddo
      endif ! if ( IFDIST .gt. 0 ) then
      endif ! if ( nobigout .le. 0 )
...
*     identify the file with the non-parametric data ---------------------------
 ...
 1969 continue
      ...
      write(33,7813) 
 7813 format('There is no non-parametric data file ',
     &'for an abstraction for a fish farm ...'/77('-'))
      call stop ! there is no non-parametric data file
 1970 continue
      if ( nobigout .le. 0 ) write(01,7163) flnamesmall(3,icod)
 7163 Format('File: ',a64/77('-'))
...
*     note whether the data are monthly ----------------------------------------
      if ( IFDIST .eq. 5 ) then
      if ( nobigout .le. 0 ) write(01,1966)
 1966 format('The Fish Farm abstraction is represented by ',
     &'monthly data ...')
*     identify the file with the monthly data ----------------------------------
      do 2969 i = 1, M8
      icod = i
 ...
 7363 format('The file is: ',a64/77('-'))
      ...
      if ( FE(IF,1) .gt. 1.0E-10 ) then ! check for zero flows  

*     deal with for the log-normal distribution of abstracted flows ============
      if ( IFDIST .eq. 2 .or. IFDIST .eq. 3 ) then ! log normal distribution ===
      EFM = FE(IF,1) ! mean abstracted flow (effluent type)
      EFS = FE(IF,2) ! standard deviation
      EF3 = 0.0 ! shift
      if ( IFDIST .eq. 3 ) EF3 = FE(IF,3)
*     compute mean and standard deviation of logged flows from the mean and ----
*     standard deviation -------------------------------------------------------
      GEFM = 0.0 ! initialise mean in the log domain
      GEFS = 0.0 ! initialise corresponding standard deviation
      EM3 = EFM+EF3
      if ( EM3 .lt. 1.0e-9 ) then
      write( *,9099)UNAME(KFEAT)
      write(01,9099)UNAME(KFEAT)
 9099 format(///'### Flow error for Feature called - ',A32//
     &'### Simulation stopped at Fish Farm')
      call stop
      endif ! if ( EM3 .lt. 1.0e-09 )
      GEFM = ALOG ((EM3*EM3)/SQRoot(1049,EM3*EM3+EFS*EFS))
      GEFS = SQRoot(1050, ALOG(1.0+(EFS*EFS)/(EM3*EM3)))
 ...
 1299 format(
	...
	if ( nobigout .le. 0 ) then
      call sort format 2 (EFM,EFS)
      !write(01,2572) valchars10,FUNIT,valchars11,FUNIT
 2572 format(77('-')/'Flow to be removed from the river:',20X,
     &'  Mean =',a10,1X,A4/
     &42x,'Standard deviation =',a10,1x,a4/77('-'))
      endif ! if ( nobigout .le. 0 )
      !endif ! if ( MONF .gt. 1 )
...
*     deal with for power curve distribution distribution of abstracted flows ==
      if ( IFDIST .eq. 10 ) then ! power curve distribution ====================
      EFM = FE(IF,1) ! mean abstracted flow
      EFS = FE(IF,2) ! standard deviation
      EF3 = 0.0 ! shift
      if ( IFDIST .eq. 3 ) EF3 = FE(IF,3)
*     compute mean and standard deviation of logged flows from the mean and ----
*     standard deviation -------------------------------------------------------
 ...
 2512 format(77('-')/'Flow to be removed from the river:',20X,
     ...
     &42x,'Standard deviation =',a10,1x,a4/77('-'))
      ...
      if ( nobigout .le. 0 ) write(01,52)BM(3),BS(3)
   52 format(
     &'Corrections for sampling errors: normal removals',
     &' ...'/77('-')/'Mean',15x,'=',f8.3/'Standard deviation =',F8.3)
      call generate removed normal effluent flows (GM,GS,F(IF,1))
      endif ! if ( MONF .gt. 1 )
      endif ! normal distribution ----------------------------------------------
      ...
      if ( IFDIST .eq. 4) then ! non-parametric distribution -------------------
      call set up the shots for non parametric stream flow ! -------- fish farms
      if ( MONF .gt. 1 ) call shots for removed flows ! ------------- fish farms
      endif ! non-parametric distribution --------------------------------------
...
*     prepare to abstract the flows --------------------------------- fish farms
      FACT = -1.0
*     prepare to count the shots where the abstraction is greater than zero ----
	numflows = 0
...
*     now do the substraction --------------------------------------------------
      START FLOW = RF
      HANDS OFF = 0.0
      REM FLOW = amax1 (HANDS OFF, RF + FACT * EF) ! fish farms ----------------
      EF = START FLOW - REM FLOW
      FMS(IS) = REM FLOW
      if ( EF .gt. 1.0e-10) numflows = numflows + 1
...
*     calculate the abstracted load XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX NOT USED
      do 3000 jdet = 1, ndet
      if ( QTYPE (jdet) .ne. 4) then 
      do 3050 J13 = 1, N13
      K13 = J13
...
*     store the abstracted quality ---------------------------------------------
 ...
 3000 continue ! end of loop on determinands XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX 
*     calculate the abstracted load XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX NOT USED
      
 2000 continue ! end of loop on shots ++++++++++++++++++++++++++++++++++++++++++

      
*     calculate the abstracted load XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
      do 1000 jdet = 1, ndet
      if ( qtype (jdet) .ne. 4 ) then
      ABLOAD (JDET,I13) = ABLOAD (JDET,I13) / float(NS)
      A1LOAD (JDET,I13) = A1LOAD (JDET,I13) / float(NS)
...
*     rank the values of abstracted quality for the calculation of percentiles -
 ...
 1000 continue ! end of loop on determinands XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
*     XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
      do jdet = 1, ndet ! ======================================================
      if ( QTYPE (jdet) .ne. 4 ) then ! ========================================
      kprune det = jdet ! -------------------------------------------- fish farm
*     accumulate total loads removed by abstractions ================= fish farm
      nx = n13
      if ( munthly structure .eq. 0 ) nx = 1 !  no requirement for monthly loads
      do J1 = 1, nx ! ----------------------------------------------------------     
      TBLODE2(jdet,J1) = TBLODE2(jdet,J1) - abload(jdet,J1)
*     calculate the effect of losses on accumulated loads ======================
      prune load (J1) = 1.0 ! ---------------------------------------- fish farm
      if ( abload(jdet,J1) .gt. 0.0 ) then
      if ( abs ( LMcontrib(NTD,jdet,1) ) .gt. 0.000000001 ) then
      if ( LMcontrib(NTD,jdet,1) - abload(jdet,J1) .gt. 0.0 ) then
      prune load (J1) = (LMcontrib(NTD,jdet,1) - abload  (jdet,J1)) 
...
*     trim back the loads for abstractions --------------------------- fish farm
      call scale loads after an abstraction ! ------------------------ fish farm
      endif ! if ( QTYPE (jdet) .ne. 4 ) then ==================================
      enddo ! do jdet = 1, ndet ================================================
*     XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
      ...
      if ( MONF.gt.1 ) call write shots for river flow ! ------------- fish farm
      call calculate summaries of river flow ! ----------------------- fish farm
      if ( IPRINT .eq. 1 .or. JSKIP .eq. 1 )return ! ----------------- fish farm
      call sort format 2 (FLOW(1),FLOW(2))
      if ( nobigout .le. 0 ) then
      write(01,1013)valchars10,FUNIT,valchars11,FUNIT
 1013 format('River flow downstream of the removal:',18X,' Mean =',
     &a10,1x,a4/46X,'95% exceedence =',a10,1x,a4/77('-'))
      write(31,1013)valchars10,FUNIT,valchars11,FUNIT
      do idet = 1, ndet
      if ( qtype(idet) .ne. 4 ) then
      write(170+idet,1013)valchars10,FUNIT,valchars11,FUNIT
      endif
      enddo
      endif
 ...
 1813 format('Time in operation',51x,'=',F6.1,' %'/77('-'))
      ...
      if ( QTYPE (JP) .ne. 4 ) then ! ------------------------------------------
      if ( nobigout .le. 0 ) then ! --------------------------------------------
      write(01,1713)DNAME(jp),Abmean(jp),
     &UNITS(jp),Abstdev(jp),UNITS(jp),
     &absq95(jp),UNITS(jp),abmean2(jp),UNITS(jp)
 1713 format('Abstracted river quality for ',a11,19X,'Mean =',
     &F7.2,1x,a4/
     ...
     &39X,'           95-percentile =',F7.2,1x,a4/
     &39X,' (Mean over entire year) =',F7.2,1x,a4/77('-'))
      write(31,1713)DNAME(jp),Abmean(jp),
     &UNITS(jp),Abstdev(jp),UNITS(jp),
     &absq95(jp),UNITS(jp),abmean2(jp),UNITS(jp)
      write(150+jp,1713)DNAME(jp),Abmean(jp),
     &UNITS(jp),Abstdev(jp),UNITS(jp),
     &absq95(jp),UNITS(jp),abmean2(jp),UNITS(jp)
      endif ! if ( nobigout .le. 0 ) -------------------------------------------
      endif ! if ( QTYPE (JP) .ne. 4 ) ----------------------------------------- 
      ...
      if ( nobigout .le. 0 ) then
      call sort format 2 (Flow(1),Flow(2))
      !write(01,6572) valchars10,FUNIT,valchars11,FUNIT
 6572 format(/77('=')/'Flow after abstraction from fish farm:',16x,
     &'  Mean =',a10,1X,A4/
     &47x,'95-percentile =',a10,1x,a4/77('='))
      !write(31,3572) valchars10,FUNIT,valchars11,FUNIT
      do idet = 1, ndet
      if ( qtype(idet) .ne. 4 ) then
      !write(150+idet,3572) valchars10,FUNIT,valchars11,FUNIT
      endif
      enddo
      endif
      ...
      kountworks = kountworks + 1 ! add a fish farm
      need works = need works + 1
      mark works = 1
      
      identify werks ( kountworks ) = feeture
      
      call write fish farm discharge quality


*     put the abstraction back in the river
      do IS = 1, NS
      FMS(IS) = FMS(IS) + FTMS(IS) 
      enddo
      call calculate summaries of river flow 
      if ( nobigout .le. 0 ) then
      call sort format 2 (Flow(1),Flow(2))
      write(01,3572) valchars10,FUNIT,valchars11,FUNIT ! ------------------- OUT
 3572 format(/77('=')/'Flow downstream of fish farm:',25X,
     &'  Mean =',a10,1X,A4/
     &47x,'95-percentile =',a10,1x,a4/77('=')/
     &'River quality downstream of the fish farm ...')
      write(31,3572) valchars10,FUNIT,valchars11,FUNIT ! ------------------- EFF
      do idet = 1, ndet
      if ( qtype(idet) .ne. 4 ) then
      write(150+idet,3572) valchars10,FUNIT,valchars11,FUNIT ! ---------- Ei EFF
      endif
      enddo
      ...
      ipl = kptarg
      ipc = 3
      if ( MRQS(idet) .eq. 3 ) then ! 90-percentile ! --------------------------
      ipl = 90
      ipc = 4
      endif ! if ( MRQS(idet) .eq. 3 ) -----------------------------------------
      if ( MRQS(idet) .eq. 6 ) then ! 99-percentile ----------------------------
      ipl = 99
      ipc = 5
      endif ! if ( MRQS(idet) .eq. 6 ) -----------------------------------------

      write(130+idet,7516)GIScode(feeture),uname(feeture), ! ------------ Ei.CSV
     &dname(idet),C(idet,1),C(idet,2),C(idet,ipc),ipl, ! ---------------- EI.CSV
     &JT(feeture),uname(feeture) ! -------------------------------------- Ei.CSV
 7516 format(' ',a40,',',a40,',', ! ------------------------------------- Ei.CSV
     &'Fish farms are not dealt with in Modes 7 8 or 9',',', ! ---------- Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! --------- fish farms (62) -------- Ei.CSV
     &',',a11,3(',',1pe12.4),',',i2,'-percentile',',',i4,',',a35) ! ----- Ei.CSV
      !endif
      else
      if ( MRQS(idet) .eq. 4 ) ! 05-percentile =================================
     &write(130+idet,7566)GIScode(feeture),uname(feeture), ! ------------ Ei.CSV
     &dname(idet),C(idet,1),C(idet,2),C(idet,3), ! ---------------------- EI.CSV
     &JT(feeture),uname(feeture) ! -------------------------------------- Ei.CSV
 7566 format(' ',a40,',',a40,',', ! ------------------------------------- Ei.CSV
     &'Fish farms are not dealt with in Modes 7 8 or 9',',', ! ---------- Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! --------- fish farms (62) -------- Ei.CSV
     &',',a11,3(',',1pe12.4),',','05-percentile',(',',i4),',',a35) ! ---- Ei.CSV
      if ( MRQS(idet) .eq. 5 ) ! 10-percentile =================================
     &write(130+idet,7556)GIScode(feeture),uname(feeture), ! ------------ Ei.CSV
     &dname(idet),C(idet,1),C(idet,2),C(idet,4), ! ---------------------- EI.CSV
     &JT(feeture),uname(feeture) ! -------------------------------------- Ei.CSV
 7556 format(' ',a40,',',a40,',', ! ------------------------------------- Ei.CSV
     &'Fish farms are not dealt with in Modes 7 8 or 9',',', ! ---------- Ei.CSV
     &'RESULTING d/s RIVER QUALITY', ! --------- fish farms (62) -------- Ei.CSV
     &',',a11,3(',',1pe12.4),',','10-percentile',(',',i4),',',a35) ! ---- Ei.CSV
      endif ! if ( MRQS(J) .ne. 4 .and. MRQS(J) .ne. 5 ) ========================
      ...
      GECM = ECM ! initialise mean in log domain -------------------------------
      GECS = ECS
      EC3 = 0.0 ! initialise shift ---------------------------------------------
      if ( PDRC (IQ,JP) .eq. 3) EC3 = pollution data(IQ,JP,3) ! shift ----------
     
*     default correlation for added quality and abstracted flow ----------------
      CO5 = EFCL(JP) ! correlation for add quality on abstracted flow ----------
*     --------------------------------------------------------------------------     
      ...
      CO2 = 1.0 ! default correlation of abstracted flow on main river flow ----
      if ( FE(IF,MO) .ne. -9.9 ) CO2 = FE(IF,MO) ! special correlation ---------

      call get the summary statistics of discharge quality (ECM,ECS) ! fish farm
      call bias in river or discharge flow and quality (ECM,ECS) ! --- fish farm

      xxxload = 0.0
      xxxfarm = 0.0
      
      do IS = 1, NS ! ==========================================================
      call get the quality of the discharge (IS,R4,EC,ECM) ! --------- fish farm     
      
*     mass balance: treating "concentration" as load ----------------- fish farm  
      CMS(JP,IS) = (FMS(IS)*CMS(JP,IS) + EC)/FMS(IS) ! mass balance at fish farm
      Y(IS) = CMS(JP,IS) ! store downstream shots of river quality
      CTMS(01,JP,IS) = CTMS(01,JP,IS) + EC/FMS(IS) ! total discharges
      CTMS(idnum,JP,IS) = CTMS(idnum,JP,IS) + EC/FMS(IS) ! fish farms
      xxxload = xxxload + CMS(JP,IS)*FMS(IS)
      xxxfarm = xxxfarm + EC
      TELODEshots(kountworks,JP,IS) = EC ! ----------------------0---- fish farm
      enddo ! do IS = 1, NS ! ==================================================
     
      usCMcont30 = CMcontrib(30,JP,1) ! get the upstream value for fish farms
      usCMcont01 = CMcontrib(01,JP,1) ! get the u/s value for total effluent
      usCMcontNTD = CMcontrib(NTD,JP,1) ! get the u/s value for total quality
  
      call get sampling rates for river quality ! for all fish farms -----------
     &(fupmx,usCMcont30,QUALNB(JP,30),EFM,ECM,xnum,IQDIST,6) ! fish farms (62) -
      
      call get sampling rates for river quality ! for river at this fish farm --
     &(fupmx,usCMcontNTD,QUALNB(JP,NTD),EFM,ECM,xnum,IQDIST,6) ! fish farm (62) 
      
      call get sampling rates for river quality ! for total discharges ---------
     &(fupmx,usCMcont01,QUALNB(JP,01),EFM,ECM,xnum,IQDIST,6) ! total discharges
      
      xxxload = xxxload  / float(NS) ! total load in downstrean river ----------
      xxxfarm = xxxfarm  / float(NS) ! total load added by this fish farm ------
      
      call calculate summaries of river quality (3,JP,Y)

      if ( ical13 .eq. 0 .and. IPRINT .eq. 0 ) then
      write(01,4230)dname(JP),C(JP,1),UNITS(JP),C(JP,2), ! ----------------- OUT
     &UNITS(JP),xxxload,LUNITS(JP)
      write(31,4230)dname(JP),C(JP,1),UNITS(JP),C(JP,2), ! ----------------- EFF
     &UNITS(JP),xxxload,LUNITS(JP)
      write(150+JP,4230)dname(JP),C(JP,1),UNITS(JP),C(JP,2), ! ---------- Di.EFF
     &UNITS(JP),xxxload,LUNITS(JP)
 4230 format(77('=')/
     &a11,27x,   '          Mean quality =',f10.3,1x,a4/
     &38x,       '    Standard deviation =',f10.3,1x,a4/77('-')/
     &38x,       '       Total mean load =',f10.3,1x,a4)
      write(150+JP,2176)
      endif
...
*     calculate loads and the summary statistics of load -----------------------
      call load calculation ! fish farm
