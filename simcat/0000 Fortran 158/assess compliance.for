...
*     ==========================================================================
*     File assess compliance.for    1562 lines ---------------------------------
*     --------------------------------------------------------------------------
...
*     standards expressed as a mean ----------------------------------------mean
      ...
      call write the mean standard (01, idet, targit, kplace)
     ...
     &     JT (feeture) .eq. 62 ) then
      !call write the mean standard (31, idet, targit, kplace)
...
*     95-percentile target ===================================================95
      if ( kstat .eq. 2 ) then
      xpoint=95.0
      call write the percentile standard (01,idet,targit,xpoint,kplace) ! ----95
      if ( JT (feeture) .eq. 03 .or. JT (feeture) .eq. 05 .or.
     &     JT (feeture) .eq. 12 .or. JT (feeture) .eq. 39 .or.
     &     JT (feeture) .eq. 60 .or. JT (feeture) .eq. 61 .or.
     &     JT (feeture) .eq. 62 ) then
      !call write the percentile standard (31,idet,targit,xpoint,kplace) ! ---95
      endif ! JT (feeture) .eq. 03 
...
*     99-percentile target ===================================================99
      if ( kstat .eq. 6 ) then
      xpoint=99.0
      call write the percentile standard (01,idet,targit,xpoint,kplace) ! ----99
      if ( JT (feeture) .eq. 03 .or. JT (feeture) .eq. 05 .or.
     &     JT (feeture) .eq. 12 .or. JT (feeture) .eq. 39 .or.
     &     JT (feeture) .eq. 60 .or. JT (feeture) .eq. 61 .or.
     &     JT (feeture) .eq. 62 ) then
      !call write the percentile standard (31,idet,targit,xpoint,kplace) -----99
      endif
      if ( COMPS/COMPM .gt. 0.0001 .and. COMPM .gt. 0.0000001 ) then
      call change to log domain (test,xpoint,COMPM,COMPS,testm,tests)
...
*     90-percentile target ===================================================90
      if ( kstat .eq. 3 ) then
      xpoint=90.0
      call write the percentile standard (01,idet,targit,xpoint,kplace) ! ----90
      if ( JT (feeture) .eq. 03 .or. JT (feeture) .eq. 05 .or.
     &     JT (feeture) .eq. 12 .or. JT (feeture) .eq. 39 .or.
     &     JT (feeture) .eq. 60 .or. JT (feeture) .eq. 61 .or.
     &     JT (feeture) .eq. 62 ) then
      !call write the percentile standard (31,idet,targit,xpoint,kplace) ! ---90
      endif
      if ( COMPS/COMPM .gt. 0.0001 .and. COMPM .gt. 0.0000001 ) then
      call change to log domain (test,xpoint,COMPM,COMPS,testm,tests)
...
*     5-percentile target ====================================================05
      if ( kstat .eq. 4 ) then
      xpoint=5.0
      call write the percentile standard (01,idet,targit,xpoint,kplace) ! ----05
      if ( JT (feeture) .eq. 03 .or. JT (feeture) .eq. 05 .or.
     &     JT (feeture) .eq. 12 .or. JT (feeture) .eq. 39 .or.
     &     JT (feeture) .eq. 60 .or. JT (feeture) .eq. 61 .or.
     &     JT (feeture) .eq. 62 ) then
      !call write the percentile standard (31,idet,targit,xpoint,kplace) ! ---05
      endif
...
*     10-percentile target ===================================================10
      if ( kstat .eq. 5 ) then
      xpoint=10.0
      call write the percentile standard (01,idet,targit,xpoint,kplace) ! ----10
      if ( JT (feeture) .eq. 03 .or. JT (feeture) .eq. 05 .or.
     &     JT (feeture) .eq. 12 .or. JT (feeture) .eq. 39 .or.
     &     JT (feeture) .eq. 60 .or. JT (feeture) .eq. 61 .or.
     &     JT (feeture) .eq. 62 ) then
      !call write the percentile standard (31,idet,targit,xpoint,kplace) ! ---10
      endif
*     estimate mean and standard deviation for percentile --------------------10
     ...
     &     JT (feeture) .eq. 62 ) then
      ...
      write(150+idet,701)dname(idet),tenstan,units(idet) ! -------------- Di EFF
  701 format(a11,30x,'      Mean standard =',a10,1x,a4)
      endif
      if ( detype(idet) .eq. 909) then ! +++++++++++++++++++++++++++++++++++++++
      if ( RQO(idet) .gt. 0.0000001 ) then
      write(31,      741)dname(idet),tenstan,units(idet) ! ----------------- EFF
      write(150+idet,741)dname(idet),tenstan,units(idet) ! -------------- Di EFF
  741 format(a11,' (dissolved)',18x,'      Mean standard =',a10,1x,a4)
      endif
      endif
      if ( detype(idet) .eq. 900) then ! +++++++++++++++++++++++++++++++++++++++
      write(31,      751)dname(idet),tenstan,units(idet) ! ----------------- EFF
      write(150+idet,751)dname(idet),tenstan,units(idet) ! -------------- Di EFF
  751 format(a11,' (total)',22x,'      Mean standard =',a10,1x,a4)
      endif      
      endif ! if (KPLACE .ne. 3 ) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     &     JT (feeture) .eq. 62 ) then
      ...
      write(150+idet,702)dname(idet),anu,tenstan,units(idet) ! ---------- Di EFF
  702 format(a11,27x,a2,'-percentile standard =',a10,1x,a4)
      ...
      write(200+idet,70)valchars10,units(idet),SET1,valchars12 ! -------- Di OUT
   70 format(46x,'Test statistic =',a10,1x,a4,4x,a13/ 
     ...
     &     JT (feeture) .eq. 62 ) then
      ...
      write(150+idet,270)valchars10,units(idet),SET1,valchars12 ! ------- Di EFF
  270 format(46x,'Test statistic =',a10,1x,a4,4x,a13/ 
     ...
     &     JT (feeture) .eq. 62 ) then
      ...
      write(150+idet,78)valchars10,units(idet),SET1,valchars12 ! -------- Di EFF
...
*     identify any targets entered for the feature number "KU" -----------------
      ...
      noclass(idet) = 1 ! there are class limits
      endif ! ------------------------------------------------------------------
      if ( EQS reach(IREACH,idet) .gt. 0 ) then ! ------------------------------
      noclass(idet) = 1 ! there are class limits
      endif ! ------------------------------------------------------------------
...
*     set up the heading -------------------------------------------------------
 ...
 1910 format(//////110('=')/'Classification upstream of: ',A40/110('-')/
     ...
     &'of the Reach called ',A16)
      ...
      write(150+idet,1940)UNAME(feeture),RNAME(IREACH),feeture, ! ------- Di EFF
     &DIST(feeture) ! --------------------------------------------------- Di EFF
 1940 format(/////110('+')/'Classification upstream of: ',A40,
     &' ... on reach ',a16,5x,i7/110('-')/
     &'Location:  ',F5.1,' km downstream from the head ', ! ----- classification
     &'of the reach ...')
      else
      write(150+idet,1941)UNAME(feeture),RNAME(IREACH),feeture, ! ------- Di EFF
     &DIST(feeture) ! --------------------------------------------------- Di EFF
...
*     dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd
 ...
 8820 format(/110('=')/'Classification downstream of ',A40,
     &' ... Reach: ',a21)
      ...
      if (EQS reach (IREACH,idet) .gt. 0 ) then
      if ( kplace .ne. 1 .and. kplace .ne. 3 ) then
      if (class limmits2 (1,idet) .gt. 0.0000001 ) then
      write(150+idet,8820)UNAME(feeture),RNAME(IREACH) ! ---------------- Di EFF
      endif
      endif
      endif
      endif
...
*     for class limits expressed as a mean -------------------------------------
      if ( kstat .eq. 1 ) then
*     call write the mean standard (48, idet, targit, kplace)
      call calculate compliance with mean standard (COMPM,COMPS,
...
*     95-percentile class limits -----------------------------------------------
      if ( kstat .eq. 2 ) then
      xpoint=95.0
*     call write the percentile standard (48,idet,targit,xpoint,kplace)
      call change to log domain (test,xpoint,COMPM,COMPS,testm,tests)
      call calculate compliance with percentile standard (testm, ! -----------95
     &tests,qualn(idet),targit,xpoint,EX,testq,testql,testqu,KU,idet)
...
*     initialise the face value class -----------------------------------------
 ...
 1046 format(110('=')/
     ...
     &'     Class boundaries ',22('.'),' Statistic'/110('='))
      write(150+idet,2221)Dname(idet),(in class(iclass,idet), ! --------- Di EFF
     &iclass = 1,nclass),(abs(Class limmits(ii,idet)),
     &ii = 1,nclass-1),valchars10 
      else
 ...
 2841 format(A11,5f9.2,4x,4('         -'),f10.3)
      if ( noclass(idet) .eq. 1 ) then
      write(150+idet,2841)Dname(idet),(in class(iclass,idet), ! --------- Di EFF
     &iclass = 1,nclass),valchars10
      endif
      endif ! if ( kplace .ne. 1 .and. kplace .ne. 3 )
      endif ! if ( JT (feeture) .eq. 03 etc ====================================
...
*     ==========================================================================
      ...
      subroutine write the mean standard (ic, idet, targit, kplace)
      include 'COMMON DATA.FOR'
      
      if ( nobigout .gt. 0 ) return
      if ( targit .lt. 1.0e-20 .or. targit .gt. 1.0e6 ) then
      if ( ic .eq. 1 )then
      write(200+idet,9)dname(idet),units(idet)
    9 format(a11,38x,'NO STANDARD =','     -----',1x,a4)
      return
      endif
      endif
    ...
    1 format(a11,36x,'Mean standard =',a10,1x,a4)
      write(150+idet,1)dname(idet),valchars10,units(idet) ! ------------- Di.EFF
      endif ! if ( ic idet.eq. 31 etc ------------------------------------------
      if ( ic .eq. 01 ) then
      write(200+idet,1)dname(idet),valchars10,units(idet) ! ------------- Di.OUT
      endif ! if ( ic idet.eq. 1 ) ---------------------------------------------
    ...
    2 format('(Dissolved) ',a11,24x,'Mean standard =',a10,1x,a4)
      write(150+idet,2)dname(idet),valchars10,units(idet) ! ------------- Di EFF
      endif ! if ( ic .eq. 31 etc )
      !if ( ic .eq. 01 .and. kplace .ne. 1 .and. kplace .ne. 3 ) then
      if ( ic .eq. 01 ) then
      write(ic,2)dname(idet),valchars10,units(idet)
!   2 format('(Dissolved) ',a11,24x,'Mean standard =',a10,1x,a4)
      write(200+idet,2)dname(idet),valchars10,units(idet) ! ------------- Di OUT
    ...
    3 format('(Total) ',a11,28x,'Mean standard =',a10,1x,a4)
      ...
      if ( ic .eq. 01 ) then
      write(200+idet,3)dname(idet),valchars10,units(idet) ! ------------- Di OUT
      endif ! if ( ic idet.eq. 31 )
    ...
    1 format(a11,24x,i5,'-percentile standard =',a10,1x,a4)
      write(200+idet,1)dname(idet),ipoint,valchars10,units(idet)
      
      else ! if ( ic. ne. 31 ) ic .eq. 31 --------------------------------------
      if ( kplace .ne. 1 .and. kplace .ne. 3 ) then
      write(150+idet,1)dname(idet),ipoint,valchars10,units(idet)
      endif
      endif ! if ( ic .eq. 31 ) ------------------------------------------------
 ...
 7009 format(/110('~')/'Assessment of compliance with standards ',
     &70('.')/110('~'))
      do idet = 1,ndet
      if ( qtype(idet) .ne. 4 ) write(200+idet,7709) dname(idet)
      enddo
      if ( JT (feeture) .eq. 03 .or. JT (feeture) .eq. 05 .or.
     &     JT (feeture) .eq. 12 .or. JT (feeture) .eq. 39 .or.
     &     JT (feeture) .eq. 60 .or. JT (feeture) .eq. 61 .or.
     &     JT (feeture) .eq. 62 ) then
      ...
      write(31,7009) ! ----------------------------------------------------- EFF
      do idet = 1, ndet ! ======================================================
      if ( qtype(idet) .ne. 4 1 ) then ! ---------------------------------------
      if ( RQO(idet) .gt. 0.0000001 ) then ! -----------------------------------
      write(150+idet,7709) dname(idet) ! -------------------------------- Di EFF
 7709 format(110('~')/'Assessment of compliance with standards for ',
     &a11,55('.')/110('~'))
      endif ! if ( RQO(idet) .gt. 0.0000001 ) ----------------------------------
      endif ! if ( qtype(idet) .ne. 4 ------------------------------------------
      enddo ! do idet = 1, ndet ================================================
...
*     dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd
 ...
 7809 format(110('+')/'Assessment of compliance d/s of ', ! ---------------- OUT
     &a40,f10.1,' km'/110('-')) ! ------------------------------------------ OUT
      do J = 1,ndet
      if ( qtype(J) .ne. 4 ) write(200+J,7809)UNAME(feeture), ! --------- Di OUT
     &Length of main river ! -------------------------------------------- Di OUT
      enddo
      if ( JT (feeture) .eq. 03 .or. JT (feeture) .eq. 05 .or. ! ---------------
     &     JT (feeture) .eq. 12 .or. JT (feeture) .eq. 39 .or.
     &     JT (feeture) .eq. 60 .or. JT (feeture) .eq. 61 .or.
     &     JT (feeture) .eq. 62 ) then
      ...
      if ( RQO(J) .gt. 0.0000001 ) then
      write(150+J,7809)UNAME(feeture),Length of main river ! ------------ Di EFF
      endif
...
*     dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7509)UNAME(feeture)
      enddo
 ...
 8509 format(110('-')/'Assessment of compliance upstream of ',
     &a40/110('-'))
      ...
      if ( qtype(J) .ne. 4 ) then
      if ( RQO(J) .gt. 0.0000001 ) then
      write(150+J,8579)UNAME(feeture),dname(J) ! ------------------------ Di EFF
 8579 format(110('-')/'Assessment of compliance upstream of ',
     &a40,' for ',a11/110('-'))
      else
      write(150+J,8569)UNAME(feeture),dname(J) ! ------------------------ Di EFF
 8569 format(110('-')/'There is no river quality standard for ',
     &a40,' and ',a11/110('+'))         
      endif
      endif
      enddo
