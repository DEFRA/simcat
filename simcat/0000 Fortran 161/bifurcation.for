...
*     ==========================================================================
*     File bifurcation.for ... 1106 lines --------------------------------------
*     --------------------------------------------------------------------------
...
*     ==========================================================================
      ...
      dimension Y(MS)
      dimension YF(MS) ! storage for the read-in flows ----------------------161
...
*     ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff 161    
*     read the file containing the effluent loads data from u/s reaches wwwwwwww
      kworks = 0
      do iworks = 1, kountworks ! wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww 
      do is = 1,ns ! -----------------------------------------------------------
      Y(is) = 0.0 ! for TELODEshots
      YF(is) = 0.0 ! for river flows
      enddo ! do is = 1,ns -----------------------------------------------------
      do JP = 1,ndet ! =========================================================
      TELODEshots(iworks,JP,is) = 0.0
      
*     read the discharge data for the first reach ------------------------------
      read(300+ireech,*,end=4002,err=4002)i1,ISTORx,iw,JPx,
     &(Y(is),is=1,NS),(YF(IS),is=1,NS) ! read works load and river flow
      if (i1 .ne. ireech) goto 4002
      kworks = iworks
     
      do is = 1, ns
      TELODEshots(iw,JPx,is) = Y(IS) ! set loads for each discharge ------------  
      enddo ! do is = 1, ns
      
      enddo ! do JP = 1, ndet ! ================================================
      enddo ! do iworks = 1, kountworks ! wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
...
*     eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee 160
      ...
      call use shots to get mean load for works (iworks,JPL,wloadm)
      TELOADAV(iworks,JPL) = wloadm
...
*     bifurcation ... feature type 11 ------------------------------------------
      ...
      dimension abmean(MP10),abstdev(MP10),AXLODE(MP10,13)
      dimension Y(MS)
      dimension YF(MS) ! storage for the read-in flows ----------------------160
      dimension xxlode(nprop,13),xxlode00(13)
...
*     read the file containing the effluent loads data from u/s reaches wwwwwwww
      ...
      enddo ! do is = 1,ns -----------------------------------------------------
      do JP = 1,ndet ! =========================================================
      if ( qtype(JP) .ne. 4 ) then ! ---------------------------------------- 11
      TELODEshots(iworks,JP,is) = 0.0 ! ------------------------- bifurcation 11
      
*     read the discharge data for the first reach ------------------------------
      read(300+ireech,*,end=4002,err=4002)i1,ISTORx,iw,JPx, ! --- bifurcation 11
     &(Y(is),is=1,NS),(YF(IS),is=1,NS) ! read works load and river flow
      ...
      do is = 1,ns
      xis1 = xis1 + Y(is)
      TELODEshots(iw,JPx,is) = SPLIT*Y(IS) ! set loads for each discharge --- 11 
      FMS(IS) = SPLIT*YF(IS) ! -------------- set river flows for bifurcation 11
      xis2 = xis2 + TELODEshots(iw,JPx,is)
      enddo ! do is = 1,ns
      
      xis1 = xis1 / float(NS)
      xis2 = xis2 / float(NS)
 
      endif ! if ( qtype(JP) .ne. 4 ) --------------------------------------- 11
      enddo ! do JP = 1,ndet ! ============================================== 11
      enddo ! do iworks = 1,kountworks ! wwwwwwwwwWwwwwwwwwwwwwwwwwwwwwwwwwww 11
