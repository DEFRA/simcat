...
*     ==========================================================================
*     File: set up data.FOR ... 2076 lines -------------------------------------
*     --------------------------------------------------------------------------
...
*     write out sets of data on effluent flow and quality ----------------------
      ...
      if ( ICAL .eq. 1 .or. MK(1) .eq. 0 ) return
*     5 - monthly distribution
*     8 - monthly structure
      numb5 = 0
      numb8 = 0
...
*     set the starting values of variables -------------------------------------
      ...
      kount22 = 0 ! number of lines of graph plotting data ---------------------
      kountd1 = 0
      kountd2 = 0
      iqmon = 0
...
*     initialise shots ---------------------------------------------------------
      ...
      CMX(idet,IS) = 0.0
      do ip = 1, nprop
      LMS(ip,iDET,IS) = 0.0 ! concentrations from various types of feature ------
      ULMS(ip,iDET,IS) = 0.0 ! u/s concentrations from various types of feature
      enddo
      enddo
      enddo
...
*     arrays for storing data for the end of a reach ---------------------------
      ...
      T50LOAD3 (IR,JP,J1) = 0.0
      T52LOAD3 (IR,JP,J1) = 0.0
      T54LOAD3 (IR,JP,J1) = 0.0
      T56LOAD3 (IR,JP,J1) = 0.0
      T58LOAD3 (IR,JP,J1) = 0.0
      TBLOAD3  (IR,JP,J1) = 0.0
      enddo ! do J1 = 1, N13
      do iworks = 1, NUED
      TELOADAVrch(iworks,IR,JP) = 0.0
      
      if ( n251 .eq. 1 ) then ! l333333333333333333 apportionment of percentiles
      do is = 1, NS ! 33333333333333333333333333333 apportionment of percentiles
*     if ( JP .eq. ndshot ) TELOADrchshots(iworks,IR,is) = 0.0 ! 3333333333333
