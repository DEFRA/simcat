...
*     ========================================================================
*     Summary of the SIMCAT FORTRAN code
*     ========================================================================
*     File name: read the features.FOR (1959 lines)
*     ------------------------------------------------------------------------

*     read the data on Features ----------------------------------------------
      subroutine read the features
      include 'COM.FOR'
      character *200 line
      character *1 AU(37)
...
*     final value for the total number of valid features ---------------------
      MU = feetcount - 1
...
*     final value for the total number of valid features -----------------------
      ...
      if ( DIST (feetcount) .lt. -0.0000001 ) then
	call change colour of text (38) ! dull green
      write( *,1882)JREACH(feetcount)
 1882 format(
     &'* Feature entered with negative distance',11x,'...',7x,
...
*     check for errors in the distances set for features -----------------------
	...
	if ( suppress1 .le. 6 ) then
      ...
      write( *,8492)UNAME(itemp)
 8492 format('* ERROR in the sequence of features',
     &16x,'...',7x,'at ',a37,4x,'and afterwards ..........')
      call set screen text colour
      endif
 ...
 8792 format('* ERROR in the sequence of features',
     &16x,'...',7x,'at ',a37,4x,'and elsewhere ...........')
      call set screen text colour
      endif
      endif ! if ( kerror .eq. 1 )
      irh = JReach(feetcount)
      write(01,8412)rname(irh),UNAME(itemp),UNAME(feetcount)
 ...
 8412 format(77('-')/
     ...
     &'*** Features - ',A40/'***            ',A40/
     &'*** Distances have been changed ... '/77('-'))
...
*     test for an illegal Reach Code -------------------------------------------
 ...
 4812 format('* Illegal Reach Code for Feature number -',I6/
...
*     check for legal Code for river flow dataset ------------------------------
     ...
     &KREACH,RNAME(KREACH)
 1021 format(77('-')/
     &'*** Data for feature have been entered wrongly ...'/
...
*     check whether Flow Dataset for gap filling has been read in --------------
 ...
 2101 continue
      ...
      if ( IT .eq.  3 .or. IT .eq. 15 .or. IT .eq. 5  .or.
     &     IT .eq. 12 .or. IT .eq. 17 .or. 
     &     IT .eq. 39 .or. IT .eq. 42 ) then
 ...
 8281 format(
     &'* Removed incorrect flow data for an effluent',6x,'...',
     &7x,'at ',a40,' Check ERR or OUT files ..')
      call set screen text colour
     ...
     &KREACH,RNAME(KREACH)
 8221 format(77('-')/
     &'*** The data for a feature have been entered wrongly ... '/
     &'*** The code for a set of river flow data numbered:',i6/
     &'*** Has been entered for an effluent feature ... type',i4/
     &'*** Name of Feature: ',A32,' Number:',I4/
     &'*** Located',F5.1,' km. from the head of ',
     &'Reach',I5,' (',A16,') ... '/
     &'*** The entry has been re-set to zero ... '/
     &77('-'))
      IF = 0
      endif
      endif
...
*     check for a legal Code for the River Quality Dataset ---------------------
      if ( IT .ne.  3 .and. IT .ne. 15 .and. IT .ne. 5  .and.
...
*     apply to [3] [5] [12] [39] [15] [16] [42] ---------- check for legal codes
      if ( IT .eq. 3 .or. IT .eq. 5 .or. IT .eq. 12 .or. IT .eq. 15 .or.
     &     IT .eq. 16  .or. IT .eq. 39 .or. IT .eq. 42 ) then
...
*     check whether the discharge dataset has been read in ---------------------
 ...
 3110 continue
      ...
      write(01,8831)IQ,UNAME(feetcount),feetcount,DIST(feetcount),
     &             KREACH,RNAME(KREACH)
      write( *,8831)IQ,UNAME(feetcount),feetcount,DIST(feetcount),
     &             KREACH,RNAME(KREACH)
      write(09,8831)IQ,UNAME(feetcount),feetcount,DIST(feetcount),
     &             KREACH,RNAME(KREACH)
      write(33,8831)IQ,UNAME(feetcount),feetcount,DIST(feetcount),
     &             KREACH,RNAME(KREACH)
 8831 format(77('-')/
     &'*** The discharge dataset specified for a Feature has not ',
     &'been entered ... '/'*** Code:',i6/
     &'*** Name of Feature: ',A32,' Number:',I6/
     &'*** Located',F5.1,' km. from the head of ',
     &'Reach',I5,' (',A16,') ...'/
     &'*** The calculations have been stopped ...'/77('-'))
...
*     check data on river flow -------------------------------------------------
      ...
      if ( iscreen .lt. 3 ) write( *,4213)UNAME(feetcount)
 ...
 7154 format(    
     &'* Zero flow specified for a stream',17x,'...',7x,
     &'at ',A40,1x,25('.'))
      call set screen text colour
      endif
...
*     check for illegal dataset code -------------------------------------------
      ...
      if ( iscreen .lt. 3 ) write( *,3855)UNAME(feetcount)
 3855 format(77('-')/
...
*     check for illegal dataset code -------------------------------------------
      ...
      write(09,3855)UNAME(feetcount)
      write(33,3855)UNAME(feetcount)
      if ( iscreen .lt. 3 ) write( *,7855)UNAME(feetcount)
 7855 format(77('-')/
     ...
     &'*** The name of the feature is: ',A33/77('-'))
      ...
      if ( PNUM(IQ,1) .lt. 0 ) then
      if ( iscreen .lt. 3 ) write( *,7892)UNAME(feetcount)
      suppress17 = suppress17 + 1 ! deleted feature
      write(01,7862)UNAME(feetcount)
      write(09,7862)UNAME(feetcount)
      write(33,7862)UNAME(feetcount)
...
*     --------------------------------------------------------------------------
      ...
      if ( PDEF(IQ) .ne. 6 .and. FE(IQ,1) .lt. 1.0E-10) then
*     if ( iscreen .lt. 3 ) write( *,7596)UNAME(feetcount)
      write(09,7516)UNAME(feetcount)
      write(33,7516)UNAME(feetcount)
 7596 format(77('-')/
     &'*** Zero flow has been specified for an effluent discharge'/
...
*     check for an illegal dataset code ----------------------------------------
      ...
      write(09,3855)UNAME(feetcount)
      write(33,3855)UNAME(feetcount)
      if ( iscreen .lt. 3 ) write( *,2855)UNAME(feetcount)
 2855 format(77('-')/
     ...
     &'*** The name of the feature is: ',A33/77('-'))
      ...
      if ( PNUM(IQ,1) .lt. 0 ) then
      if ( iscreen .lt. 3 ) write( *,2892)UNAME(feetcount)
      suppress17 = suppress17 + 1 ! deleted feature
      write(01,7862)UNAME(feetcount)
      write(09,7862)UNAME(feetcount)
      write(33,7862)UNAME(feetcount)
 2892 format(77('-')/
     ...
     &'*** Feature deleted ... '/77('-'))
      ...
      if ( PDEF(IQ) .ne. 6 .and. FE(IQ,1) .lt. 1.0E-10) then
*     if ( iscreen .lt. 3 ) write( *,2596)UNAME(feetcount)
      suppress19 = suppress19 + 1 ! unneeded data ignored (39)
      write(09,7516)UNAME(feetcount)
      write(33,7516)UNAME(feetcount)
 2596 format(77('-')/
     &'*** Zero flow has been specified for a mine water'/
     &'*** The name of the feature is: ',A33/77('-'))
      endif
...
*     sub-catchment boundary (24) ----------------------------------------------
 ...
 2156 format(77('-')/
     ...
     &'*** boundary. Reference to the dataset removed ... '/
     &'*** The name of the feature is: ',A33/77('-'))
      JF(feetcount) = 0
      endif
...
*     write(09,7816)UNAME(feetcount)
 ...
 7816 format(77('-')/
     &'*** A quality dataset was given to a Zero-flow ',
     &'Discharge (Feature Type 17) ...'/
     &'*** Reference to the data-set removed ... '/
     &'*** Name of Feature: ',A33/77('-'))
      JQ(feetcount) = 0
      endif
      endif
...
*     check data on river flow -------------------------------------------------
 ...
 4363 format(77('-')/
     &'*** No valid flow Data-Set Code for Abstraction Type 19  '/
     &'*** Feature type 18 '/
     &'*** The name of the feature is: ',A33/77('-'))
      endif
...
*     abstraction - the negative discharge (19) --------------------------------
      if ( IT .eq. 19 ) then
*     check data on abstraction (effluent flow data) ---------------------------
      if ( IQ .lt. 0 .or. IQ .gt. NE ) then
 ...
 4867 format(77('-')/
     &'*** Illegal effluent flow dataset specified for ',
     &'Abstraction Point '/
     &'*** Feature type 19 '/
     &'*** The name of the feature is: ',A33/77('-'))
      endif
      if ( IQ .eq. 0 ) then
      write(01,5867)UNAME(feetcount)
      if ( iscreen .lt. 3 ) write( *,5867)UNAME(feetcount)
      suppress19 = suppress19 + 1 ! unneeded data ignored
      write(09,5867)UNAME(feetcount)
      write(33,5867)UNAME(feetcount)
 5867 format(77('-')/
     &'*** No effluent flow dataset specified for ',
     &'Abstraction Point '/
     &'*** Feature type 19 -      '/
     &'*** The name of the feature is: ',A33/77('-'))
      endif
*     check data on river flow -------------------------------------------------
*     95-percentile in flow data is taken as a standard deviation --------------
      if ( IF .gt. 0 ) then
      write(01,4863)UNAME(feetcount)
      if ( iscreen .lt. 3 ) write( *,4863)UNAME(feetcount)
      suppress19 = suppress19 + 1 ! unneeded data ignored
      write(09,4863)UNAME(feetcount)
      write(33,4863)UNAME(feetcount)
 4863 format(77('-')/
     &'*** River flow Data-Set is entered for Abstraction Type 19 '/
     &'*** This should be an effluent quality dataset '/
     &'*** The name of the feature is: ',A33/77('-'))
      call stop
      endif
...
*     flow gauge (4) -----------------------------------------------------------
 ...
 7894 format(77('-')/
     &'* Zero flow specified for a flow gauge',13x,'...',7x,
     &'at ',A40,1x,25('.'))
      call set screen text colour
      endif
...
*     river regulation point (9) -----------------------------------------------
 ...
 6252 format(77('-')/
     ...
     &'*** Name of Feature: ',A33,'   '/
     &'*** The option of a non-parametric distribution is not   '/
 ...
 7876 format(
     &'* Zero flow specified for Regulation Point ',8x,'...',7x,
     &'at ',A40,1x,25('.'))
      call set screen text colour
      endif
...
*     ====================================================================
*     Bifurcation (22 or 23)

      if ( IT .eq. 22 .or. IT .eq. 23) then
      if ( IQ .le. 0 ) then
 ...
 7406 format(77('-')/
     &'*** Error in data for Bifurcation (Feature Type 22 or 23) ... '/
     &'*** No valid source given for discharge flow and quality data '/
     &'*** Reach Code needed at fifth item in data record ...  '/
     &'*** Name of Feature: ',A33/77('-'))
	call stop
      endif
...
*     check for illegal dataset code
      ...
      if ( iscreen .lt. 3 ) write( *,4855)UNAME(feetcount)
 4855 format(77('-')/
...
*     switch on Diffuse Pollution (13)
 ...
 7256 format(
     &'* Zero flow specified for Diffuse Pollution',8x,'...',7x,
     &'at ',A40,1x,25('.'))
      call set screen text colour
      endif
	if ( nobigout .le. 0 ) write(01,7156)UNAME(feetcount)
...
*     switch on Diffuse Pollution (15 ... 42) ----------------------------------
      ...
      call switch on diffuse pollution 15
      endif
      goto 38

*     switch off Diffuse Pollution (16 ... 43) ---------------------------------
      if ( IT .eq. 16 .or. IT. eq. 43 ) then
      call switch off diffuse pollution 16
...
*     eliminate unnecessary features -------------------------------------------
 ...
 8322 format(110('-')/
     &'*** Error in data-sets for Features ... Number:',i5/
...
*     switch-off Diffuse Pollution (16) ----------------------------------------
      subroutine switch off diffuse pollution 16
      include 'COM.FOR'
...
*     switch-on Diffuse Pollution (15) -----------------------------------------
      subroutine switch on diffuse pollution 15
      include 'COM.FOR'
 ...
 3852 format(77('-')/
	...
	ihalt diffuse discharge = ihalt diffuse discharge + 1
	if ( ihalt diffuse discharge .le. 2 ) then
      if ( iscreen .lt. 3 ) then
	call change colour of text (21) ! dull red
      write( *,7596)UNAME(feetcount)
 7596 format('* Zero flow specified for Diffuse ',
     &'Pollution ... ',A33)
      call set screen text colour
      endif
