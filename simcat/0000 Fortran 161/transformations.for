...
*     ==========================================================================
*     File: transformations.for ... 4072 lines ---------------------------------
*     --------------------------------------------------------------------------
...
*     straight forward use of this routine - not for Gap Filling ===============
 ...
 2200 format(//110('=')/'Stretch: ',a16,1x,'from the start of the ',
     &'Reach',17x,'to ',a37/110('='))
      ...
      write(200+ic,2270)RNAME(IREACH),vname(KFEET)
 2270 format(////110('=')/'Stretch: ',a16,1x,'from the start of the ',
     &'Reach',17x,'to ',a37/110('='))
      endif
      enddo
      else
...
*    &24x,' to the end of the Reach'/110('='))
      do ic = 1, ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2230) RNAME(IREACH)
      endif
      enddo
 ...
 2000 format(/////110('=')/'Stretch: ',a16,' from: ',a37,' to ',a37/ ! ----- OUT
     &110('='))
      do ic = 1, ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2700) RNAME(IREACH),uname (KFEET-1),vname(KFEET) ! --- Di.OUT
 2700 format(///110('=')/'Stretch: ',a16,' from: ',a37,' to ',a37/ ! ---- Di.OUT
     &110('='))
      endif
      enddo
      endif ! ------------------------------------------------------------------
 ...
 2100 format (///110('=')/'Stretch: ',a16,' from: ',a37,17x,
     &' to the end of the Reach'/110('='))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2100) RNAME(IREACH),
     &uname(KFEET)    
      enddo
      endif
      endif ! if ( iend .eq. 1 )
...
*     --------------------------------------------------------------------- (13)
 ...
 7482 format(33x,77('=')/33x,
     ...
     &'Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7482)IF
      enddo
 ...
 1482 format(33x,77('-')/
     ...
     &a10,1x,A4)
      ...
      write(200+J,1482)valchars11,valchars10,funit
 ...
 1422 format(33x,'Starting ',A11,15x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4)
      write(200+J,1422)DNAME(J),valchars11,valchars10,UNITS(J)
...
*     add in diffuse pollution (river type - unassigned) ------------------ (13)
 ...
 2082 format(33x,
     ...
     &'Mean =',a10,1x,a4/33x,77('-')) ! ----------------------------------- (13)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (13)
      endif
      enddo
      endif ! if ( nobigout .le. 0 ) -------------------------------------------
      endif ! if ( ical13 .eq. 0 ) ---------------------------------------------
...
*     identify the file with the monthly data ----------------------------------
 ...
 7363 format(33x,'These flows are from monthly distributions ...',
     &' File: ',a64/33x,77('='))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7363) FLMONTHsmall(1,icod)
      enddo
...
*     identify the file with the monthly structure data ------------------------
 ...
 2044 format(33x,'Quality of this diffuse inflow (13): ',
     &'Data Set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2044)IQ
      enddo
      endif
...
*     --------------------------------------------------------------------- (25)
 ...
 2944 format(33x,77('+')/33x,'Diffuse inflow (25) from livestock ',
     &'farming: Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2944)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,1482)valchars11,valchars10,funit
      endif
      enddo
      do J = 1, ndet
      if ( QTYPE (J) .ne. 4 ) then
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (25)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 2094 format(33x,'Quality of diffuse inflow from livestock (25): ',
     &'Data Set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2094)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (27)
 ...
 4944 format(33x,77('+')/33x,'Diffuse inflow (27) from arable ',
     &'farming: Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,4944)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (27)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ (27)
 ...
 4994 format(33x,'Quality of diffuse inflow from arable (27): ',
     &'Data Set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,4994)IQ
      enddo 
      endif
      call write mean and standard deviation 33
      endif !  if ( IPRINT .ne. 1 .and. JSKIP .ne. 1 .and. ICAL .ne. 1 )
...
*     --------------------------------------------------------------------- (29)
 ...
 2244 format(33x,77('+')/33x,'Diffuse inflow (29) from highway ',
     &'runoff: Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2244)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (29)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 2994 format(33x,'Quality of diffuse inflow from highway runoff (29): ',
     &'Data Set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2994)IQ
      enddo
      endif      
...
*     --------------------------------------------------------------------- (31)
 ...
 1244 format(33x,77('+')/33x,'Diffuse inflow (31) from urban runoff: ',
     &'Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,1244)IF
      enddo
      ...
      enddo
      call sort format 3 (DINT*F(IF,1),DINT*F(IF,2),dint)
      write(01,2082)valchars12,valchars11,valchars10,funit ! ------- (31) -- OUT
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! - (31) - Di.OUT
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 1994 format(33x,'Quality of diffuse inflow from urban runoff (31): ',
     &'Data Set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,1994)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (33)
 ...
 1884 format(33x,77('+')/33x,'Diffuse inflow (33) from atmospheric ',
     &'deposition: Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,1884)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (33)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 3994 format(33x,'Quality of diffuse inflow from atmospheric ',
     &'deposition (33): data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,3994)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (35)
 ...
 1824 format(33x,77('+')/33x,'Diffuse inflow (35) from natural ',
     &'background: data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,1824)IF
      enddo

      call sort format 2 (FLOW(1),FLOW(2))
      write(01,1482)valchars11,valchars10,funit ! ------------------------- (35)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,1482)valchars11,valchars10,funit
      endif
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (35)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 3924 format(33x,'Quality of diffuse inflow from natural ',
     &'background (35): data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,3924)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (46)
 ...
 6824 format(33x,77('+')/33x,'Diffuse inflow (46) from mines: ',
     &'Data Set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,6824)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (46)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 3984 format(33x,'Quality of diffuse inflow from mines (46): ',
     &'Data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,3984)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (48)
 ...
 7824 format(33x,77('+')/33x,'Diffuse inflow (48) from ',
     &'birds, boats and angling: ','Data Set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7824)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (48)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 7984 format(33x,'Quality of diffuse inflow from birds, ',
     &'boats and angling (48): ','Data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7984)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (50)
 ...
 7504 format(33x,77('+')/33x,'Diffuse inflow (50) .....',
     &'......................... ','Data Set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7504)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (50)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 6504 format(33x,'Quality of diffuse inflow from ....... ',
     &'User-defined (50): ','Data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,6504)IQ
      enddo     
      endif
...
*     --------------------------------------------------------------------- (52)
 ...
 7524 format(33x,77('+')/33x,'Diffuse inflow (52) .....',
     &'......................... ','Data Set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7524)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (52)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 6574 format(/110('=')/'Quality of diffuse inflow from ....... ',
     &'User-defined (52): ','Quality data set:',I6/110('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,6524)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (54)
 ...
 7544 format(33x,77('+')/33x,'Diffuse inflow (54) .....',
     &'......................... ','Data Set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7544)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (54)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 6544 format(33x,'Quality of diffuse inflow from ....... ',
     &'User-defined (54): ','Data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,6544)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (56)
 ...
 7564 format(33x,77('+')/33x,'Diffuse inflow (56) .....',
     &'......................... ','Data Set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7564)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (56)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 6564 format(33x,'Quality of diffuse inflow from ....... ',
     &'User-defined (56): ','Data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,6564)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (58)
 ...
 7584 format(33x,77('+')/33x,'Diffuse inflow (58) .....',
     &'......................... ','Data Set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,7584)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (58)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 6584 format(33x,'Quality of diffuse inflow from ....... ',
     &'User-defined (58): ','Data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,6584)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (37)
 ...
 1784 format(33x,77('+')/33x,'Diffuse inflow (37) from septic tanks: ',
     &'Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,1784)IF
      enddo
      ...
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,2082)valchars12,valchars11,valchars10,funit ! ---------- (37)
      endif
      enddo
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 3794 format(33x,'Quality of diffuse inflow from septic tanks (37): ',
     &'Data Set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,3794)IQ
      enddo 
      endif
...
*     --------------------------------------------------------------------- (40)
 ...
 3784 format(33x,77('+')/33x,'Diffuse inflow (40) from aggregated ',
     &'CSOs: Flow data set:',I6)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,3784)IF
      enddo
      ...
      do J = 1, ndet ! -------------------------------------------------- Di.OUT 
      if ( QTYPE (J) .ne. 4 ) then
      write(200+J,2082)valchars12,valchars11,valchars10,funit ! ----------- (40)
      endif
      enddo ! do J = 1, ndet -------------------------------------------- Di.OUT
      endif ! if ( nobigout .le. 0 )
      endif ! if ( ical13 .eq. 0 )
      endif ! if ( JSKIP .eq. 0 .AND. IPRINT .eq. 0 ) 
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 3714 format(33x,'Quality of diffuse inflow from aggregated ',
     &'CSOs (40): data set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,3714)IQ
      enddo 
      endif
...
*     add in diffuse pollution (effluent type) ---------------------------- (15)
 ...
 5944 format(33x,77('+')/33x,'Discharge-type diffuse inflow (15)',
     &': data set:',I6)
      do J = 1, ndet
      if ( QTYPE (J) .ne. 4 ) write(200+J,5944)IQ
      enddo
 ...
 3210 format(33x,77('-')/
     ...
     &'St.dev =',a10,3x,'Mean =',a10,1x,a4,2x,a10,' km'/33x,77('-'))
      do J = 1, ndet
      if ( QTYPE (J) .ne. 4 ) then
      write(200+J,3210)valchars11,valchars10,funit,valchars12
      endif
      enddo
      endif ! if ( ical13 .eq. 0 )
      endif ! if ( nobigout .le. 0 )
...
*     --------------------------------------------------------------------- (15)
 ...
 2194 format(33x,'Quality of diffuse effluent (15): ',
     &'data set:',I6/33x,77('-'))
      do ic = 1, ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2194)IQ
      enddo
      endif
...
*     add in aggregated STWs (effluent type) ------------------------------ (42)
 ...
 5994 format(33x,77('-')/33x,'Aggregated STWs (42)',
     &': flow and quality data set:',I6)
      do ic = 1, ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,5994)IQ
      enddo
      ...
      do J = 1, ndet
      if ( QTYPE(J) .ne. 4 ) then
      write(200+J,3210)valchars11,valchars10,funit,valchars12 ! ----------- (42)
      endif
      enddo
...
*     ----------------------------------------------------- aggregated STWs (42)
 ...
 2894 format(33x,'Quality of diffuse aggregated STWs (42): ',
     &'Data Set:',I6/33x,77('-'))
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) write(200+ic,2894)IQ
      enddo
      endif
...
*     check for xero distance and for inflow from a lake -----------------------
      ...
      do iw = 1, kountworks   
      call use shots to get mean load for works (iw,3,wloadm)
      enddo
      endif ! ============================================================== 160
      
      call update summaries of contribution ! ------------- natural purification
      
      if ( kountworks .gt. 0 ) then ! ==========================================
      do iw = 1, kountworks   
      call use shots to get mean load for works (iw,3,wloadm)
      enddo
      endif ! if ( kountworks .gt. 0 ) ! =================================== 160
      ...
      enddo
      call calculate summaries of river quality (0,JP,RQ) 
      endif ! if ( QTYPE (jp) .ne. 4 )
      enddo ! do jp = 1, ndet
  
      call update summaries of contribution ! ---------------- diffuse pollution
...
*     concentrations from various types of feature -----------------------------
      CTMS(ip,JDET,IS) = CTMS(ip,JDET,IS) * CMS(JDET,IS) / old CMS ! decay -----
      enddo
      else
      do ip = 1, n2prop
      CTMS(ip,JDET,IS) = 0.0 ! concentrations from various types of feature -----
      enddo
      CMS(JDET,IS) = 0.0
      endif
...
*     concentrations from various types of feature --------------------------AMM
      CTMS(ip,JDET,IS) = CTMS(ip,JDET,IS) * CMS(JDET,IS) / old CMS
      enddo
      else
      do ip = 1, n2prop
      CTMS(ip,JDET,IS) = 0.0 ! concentrations from various types of feature -AMM
      enddo
      CMS(JDET,IS) = 0.0
      endif
...
*     adjust concentrations from various types of feature +++++++++++++++++++ DO
      if (ip .eq. 2 ) oldCTMS = CTMS(ip,JDET,IS)
      CTMS(ip,JDET,IS) = CTMS(ip,JDET,IS) * CMS(JDET,IS) / old CMS
      enddo
      else
      do ip = 1, n2prop
      CTMS(ip,JDET,IS) = 0.0 ! concentrations from various types of feature - DO 
      enddo
      CMS (JDET,IS) = 0.0
...
*     write(27,2000) dint
 2000 format(33x,77('-')/33x,
     &'Length of this stretch = ',f7.2,' kilometres (km)')
      ...
      do jper = 1,ndet 
      if ( qtype(jper) .gt. 1 .and. qtype(jper) .ne. 4 ) then
      !if ( KSIM .lt. 2 ) write(170+jper,2300)avel,atim
 2300 format(
     &59x,'          Average velocity of flow = ',f7.2,' km/day'/
     &59x,'            Average time of travel = ',f7.2,'  hours')
      !if ( KSIM .lt. 2 ) write(170+jper,2030) velo95, 24.0*time95
*     write(27,2030) velo95, 24.0*time95
 2030 format(
     ...
     &33x,77('-'))
      ...
      do idet = 1,ndet ! =======================================================
      if ( qtype(idet) .ne. 0 .and. qtype(idet) .ne. 4 ! =======================
     &.and. KSIM .eq. 1 ) then ! -----------------------------------------------
      if ( half life(idet) .gt. 0.000001 ) then ! ------------------------------
      write(170+idet,4542)dint ! ---------------------------------------- Ci.GAP
 4542 format(33x,77('=')/33x,
     &'Changes in river quality from natural purification over', ! ------ Ci.GAP
     &f7.2,' km'/
     &33x,'(Upstream of the monitoring point that is being ',
     &'targeted ...)') ! ------------------------------------------------ Ci.GAP
      redu = 100.0 * (1.0 - exp (-kay0(idet) * atim/24.0 ))
      if ( nobigout .le. 0 .and. KSIM .lt. 2) 
     &write(170+idet,2200)Dname(idet),half life(idet),redu ! ------------ Ci.GAP
 2200 format(33x,77('-')/33x,'Average half-life for ',a11,' =', ! ------- Ci.GAP
     &f6.1,' hours',' ..... average loss =',f7.2,' %'/33x,77('=')) ! ---- Ci.GAP
      endif ! if ( half life (idet) .gt. 0.000001 ) ----------------------------
      endif ! if ( qtype(idet) .ne. 0 etc ======================================
      enddo ! do idet = 1,ndet =================================================

      endif ! if ( ipur .ne. 0 .and. nobigout .le. 0 ) =========================
      endif ! if ( ical13 .eq. 0 ) +++++++++++++++++++++++++++++++++++++++++++++
      endif ! if ( dint .gt. 0.000001 ++++++++++++++++++++++++++++++++++++++++++
...
*     calculate the difference in load caused by natural purification ----------
      ...
      if ( old shots(idet,is) .gt. 0.00000001 ) then ! --------------------- 160
      prune shots(idet,is) = xshots(idet,is)/old shots(idet,is)
      endif ! -------------------------------------------------------------- 160
...
*     calculate the effect of losses on the accumulated loads ------------------
      ...
      if ( abs ( LMcontrib(NTD,idet,J1) ) .gt. 0.000000001 ) then
      if ( xdownload (idet,J1) .lt. 0.0 ) then
      if ( LMcontrib(NTD,idet,J1) + xdownload (idet,J1) .gt. 0.0 ) then
      prune load (J1) = (LMcontrib(NTD,idet,J1) + xdownload (idet,J1)) 
     &                /  LMcontrib(NTD,idet,J1)
      endif
      endif ! if ( xdownload (idet,J1) .lt. 0.0 )
      endif ! if ( abs ( LMcontrib(NTD,idet,J1) ) .gt. 0.000000001 )
      enddo ! do J1 = 1, 1  ------------------------------- natural purification
...
*     trim loads from individual works =========================================
      if ( kountworks .gt. 0 .and. kprune det .gt. 0 ) then ! =================
      do iworks = 1, kountworks ! =============================================
      sumte = 0.0 ! initialise the sum of loads from all pruned shots ------ 160
      do is = 1, NS ! ..........................................................
      TELODEshots(iworks,kprune det,is) = ! trim loads after losses ............
     &TELODEshots(iworks,kprune det,is) * prune shots (kprune det,is) ! ........
      sumte = sumte + TELODEshots(iworks,kprune det,is) ! sum the loads --------
      enddo ! do is = 1, NS ................................................ 160
      TELOADAV(iworks,kprune det) = sumte / float (NS) ! calculate mean load ---
      enddo ! do iworks = 1, kountworks ! =====================================
      endif ! if ( kountworks .gt. 0 .and. kprune det .gt. 0 ) ================


      
*     loop through all types of feature tracked for inputs =====================
      do IP = 1, n2prop ! ======================================================
      do IS = 1, NS ! ----------------------------------------------------------
      if ( IP .eq. NTD ) then ! for the total concentration --------------------
      Y(IS) = CMS(kprune det,IS) ! concentration in the river ------------------   
      else ! for all othe values of IP -----------------------------------------
      Y(IS) = CTMS(IP,kprune det,IS) ! concentration from all type IP ---------- 
      endif ! if ( IP .eq. NTD ) -----------------------------------------------
      enddo ! do IS = 1, NS ----------------------------------------------------
      call calculate summaries of contribution (IP,kprune det,Y,0) ! -----------
...
*     annual loads (i13) from upstream sub-catchments --------------------------
      TWLOADS(ibodies,kprune det,i13) = ! scale loads after losses -------------
     &TWLOADS(ibodies,kprune det,i13) * prune load (i13) ! scale loads ---------
      
      do ip = 1, n2prop ! scale loads after losses -----------------------------
*     breakdown of annual loads (i13) from upstream sub-catchments -------------
      TWLOADSapp(ibodies,kprune det,i13,ip) = ! scale loads after losses -------
...
*     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      return ! scale loads after natural purification
      end ! scale loads after natural purification
*     ##########################################################################
*     trim back the loads for natural purification #############################
*     ##########################################################################
*     ##########################################################################
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
*     trim back the loads after an abstraction ---------------------------------
      subroutine scale loads after an abstraction ! ----------------------------
      include 'COMMON DATA.FOR'
      dimension Y(MS)
...
*     trim loads from individual works =========================================
      if ( kountworks .gt. 0 .and. kprune det .gt. 0 ) then ! ==================
      do iworks = 1, kountworks ! ==============================================
      call use shots to get mean load for works (iworks,kprune det, ! ==========
     &wloadm)
      TELOADAV(iworks,kprune det) = wloadm ! annual load -----------------------
      enddo ! do iworks = 1, kountworks ! ======================================
      endif ! if ( kountworks .gt. 0 .and. kprune det .gt. 0 ) =================
...
*     loop through all types of feature tracked for inputs ~~~~~~~~~~~~~~~~~~~~~
      do IP = 1, n2prop ! ======================================================
      do IS = 1, NS ! ----------------------------------------------------------
      if ( IP .eq. NTD ) then ! for the total concentration in the river -------
      Y(IS) = CMS(kprune det,IS) ! concentration in the river ------------------   
      else ! for all othe values of IP -----------------------------------------
      Y(IS) = CTMS(IP,kprune det,IS) ! concentration from all type IP ---------- 
      endif ! if ( IP .eq. NTD ) -----------------------------------------------
...
*     annual loads (i13) from upstream sub-catchments --------------------------
     ...
     &TWLOADS(ibodies,kprune det,i13) * prune load (i13) ! scale loads ---------
      ...
      do ip = 1, n2prop ! scale loads after losses by abstraction --------------
*     breakdown of annual loads (i13) from upstream sub-catchments -------------
      TWLOADSapp(ibodies,kprune det,i13,ip) = ! scale loads after losses -------
     &TWLOADSapp(ibodies,kprune det,i13,ip) * prune load (i13) ! ---------------
      enddo ! do ip = 1, n2prop ------------------------------------------------
...
*     aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
      ...
      do J1 = 1, nx ! ----------------------------------------------------------      
      if ( prune load (J1) .lt. 1.0 ) then ! -----------------------------------
*     prune any total load by losses from abstraction  =========================
      if ( TNLOADUP2(kprune det,J1) .gt. 0.0 ) then ! --------------------------
      TNLOADUP2(kprune det,J1) = TNLOADUP2(kprune det,J1) ! after abstraction --
     &                         * prune load (J1) ! after abstraction -----------
      endif ! if ( TNLOADUP2(kprune det,J1) .gt. 0.0 ) -------------------------
      endif ! if ( prune load (J1) .lt. 1.0 ) ----------------------------------
      enddo ! do J1 = 1, nx ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

      return ! scale loads after an abstraction --------------------------------
      end ! scale loads after an abstraction -----------------------------------
*     ========================================= scale loads after an abstraction
*     ##########################################################################
*     trim back the loads after an abstraction ---------------------------------
*     aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
*     aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
      ...
      subroutine sort format 4 (v1,v2,v3,v4)
      include 'COMMON DATA.FOR'
      character *10 valfetch
      valchars10 = '       - '
      valchars11 = '       - '
   ...
   99 continue
    ...
    4 format(33x,77('-')/33x,
     ...
     &a10,1x,A4)
      do ic = 1,ndet
      if ( qtype(ic) .ne. 4 ) then
      write(200+ic,4)valchars11,valchars10,funit
      endif
      enddo

      do jper = 1,ndet ! -------------------------------------------------------
      if ( qtype(jper) .ne. 4 ) then ! -----------------------------------------
      if ( ical .ne. 1 .and. ical .ne. 2 .and. ical .ne. 3 .and. ! -------------
     &     KSIM .lt. 2 .and. distp .gt. 0.001 ) then
      !write(01,4)valchars10,valchars11,funit
      !4 format(33x,
      !&'Resulting river flow',15X,'Mean =',a10,3x,'95%tle =',a10,1x,A4)
      endif ! if ( ical .ne. 1 etc --------------------------------------------- 
      endif ! if ( qtype(jper) .ne. 4 ) ----------------------------------------
      enddo ! do jper = 1,ndet -------------------------------------------------
      
      if ( IPRINT .ne. 1 ) then
      !if ( add diffconc (KFEAT) .eq. 0 ) then 
*     write(01,7282)KFEAT-1,add diffconc (KFEAT-1)
*7282 format(33x,77('=')/33x,'Results of diffuse inflow ...',2i3/   
*    &33x,77('-')) 
      !else
      !write(01,9282)
      !9282 format(33x,77('#')/33x,'No extra diffuse flows added ...'/   
      !&33x,77('#')) 
      !endif
      do J = 1, ndet ! =========================================================
      if ( QTYPE (J) .ne. 4 ) then ! -------------------------------------------
     ...
     &     idp = PDEC(IQ,J)
      ...
      call sort format 2 (C(J,1),C(J,2))
      if ( C(J,1) .gt. 0.0000001 ) then ! ======================================
          
      if ( ical .ne. 1 .and. ical .ne. 2 .and. ical .ne. 3 .and. ! -------------
     &     KSIM .lt. 2 .and. distp .gt. 0.00001 ) then
      write(01,1)DNAME(J),valchars11,valchars10,UNITS(J)
    1 format(33x,'Resulting ',A11,14x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4)
      write(200+J,1)DNAME(J),valchars11,valchars10,UNITS(J)      
      endif ! if ( ical .ne. 1 etc ---------------------------------------------
      ...
      if ( ical .ne. 1 .and. ical .ne. 2 .and. ical .ne. 3 .and. ! -------------
     &     KSIM .lt. 2 .and. distp .gt. 0.001 ) then
      write(01,2)DNAME(J),valchars10,UNITS(J)
    2 format(33x,'Resulting ',A11,35x,'Mean =',a10,1x,a4)
      write(200+J,2)DNAME(J),valchars10,UNITS(J)
      endif
      else ! for a non-zero mean -----------------------------------------------
      if ( ical .ne. 1 .and. ical .ne. 2 .and. ical .ne. 3 .and. ! -------------
     &     KSIM .lt. 2 .and. distp .gt. 0.001 ) then
      write(01,3)DNAME(J),valchars11,valchars10,UNITS(J)
    3 format(33x,'Resulting ',A11,14x,'St.dev =',a10,3x,
     &'Mean =',a10,1x,a4)
      write(200+J,3)DNAME(J),valchars11,valchars10,UNITS(J)
      write(200+J,6631)
      endif
 ...
 6631 format(33x,77('+'))
     ...
     &     KSIM .lt. 2 .and. distp .gt. 0.001 ) then
      do jper = 1,ndet ! -------------------------------------------------------
      if ( qtype(jper) .ne. 4 ) then !------------------------------------------
      write(01,5)jper,KSIM
    5 format(33x,77('-'),2i4)
      endif ! if ( qtype(jper) .ne. 4 ) ---------------------------------------     
      enddo ! do jper = 1,ndet ------------------------------------------------
      if ( diffuse type .eq. 15 .or. diffuse type .eq. 40 )
     &write(01,5)diffuse type
      endif ! if ( ical .ne. 1 .and. KSIM .lt. 2 ) =============================
