...
*     ==========================================================================
*     File: process each reach in turn.for ... 6864 lines ----------------------
*     --------------------------------------------------------------------------
...
*     NTD = Total                           end of the reach          
      ...
      TRLODE3   (ISTOR,JPL,J1) = TRLODE2   (JPL,J1) ! boundaries and streams
      TELODE3   (ISTOR,JPL,J1) = TELODE2   (JPL,J1)
      ...
      T62LOAD3  (ISTOR,JPL,J1) = T62LOAD2  (JPL,J1)
      ...
      TRLODE4   (ISTOR,JPL,J1) = TRLODE1   (JPL,J1) ! boundaries and streams
      TELODE4   (ISTOR,JPL,J1) = TELODE1   (JPL,J1)
      ...
      T62LOAD4  (ISTOR,JPL,J1) = T62LOAD1  (JPL,J1)
      ...
      T58LOAD4  (ISTOR,JPL,J1) = T58LOAD1  (JPL,J1) ! ====================== 158
...
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
*     total loads from fish farms (62) -----------------------------------------
      kp = 0
      do jdet = 1, ndet
      Tenchars1(idet) = '      ....'
      if ( QTYPE (jdet) .ne. 4 ) then
      if ( abs (T62LOAD1(jdet,i13)) .gt. 1.0e-10 ) kp = 1
      call set format for printout(Tenchars1(jdet),T61LOAD1(jdet,i13))
      endif
      enddo
      !if ( kp .eq. 1 ) write(27,7903)(Tenchars1(jdet),jdet=1,ndet)
 7903 format('+',2x,'Total from fish farms (62)',10x,10A10)
*     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
*     write for the monitoring points ++++++++++++++++++++++++++++++++++++++++++
      ...
      if ( output mode .eq. 0 ) then ! ================ deal with 95-percentiles
      !write(LINE,9295)(B44(jdet),jdet=1,ndet)
 9295 format(5X,'(calculated 95 and 5-percentiles',13x,10A7,')')
      if ( iscreen .lt. 3 ) write( *,7511)LINE ! write to the screen -----------
      !write(09,7511)LINE ! feature - calculated 95-percentile quality ----- SCN
      !write(33,7511)LINE ! feature - calculated 95-percentile quality ----- EFF
      endif ! if ( output mode .eq. 0 ) =============== deal with 95-percentiles
    
      if ( output mode .eq. 2 ) then ! ================ deal with 90-percentiles
      !write(LINE,9195)(B44(jdet),jdet=1,ndet)
 9195 format(5X,'(calculated 90 and 10-percentiles',12x,10A7,')')
      if ( iscreen .lt. 3 ) write( *,7511)LINE
      !write(09,7511)LINE ! feature - calculated 90-percentile quality
      !write(33,7511)LINE ! feature - calculated 90-percentile quality
      endif ! if ( output mode .eq. 2 ) =============== deal with 90-percentiles
 
      if ( output mode .eq. 3 ) then ! ================ deal with 99-percentiles
      !write(LINE,9395)(B44(jdet),jdet=1,ndet)
 9395 format(5X,'(calculated 99 and 1-percentiles',13x,10A7,')')
      if ( iscreen .lt. 3 ) write( *,7511)LINE
      !write(09,7511)LINE ! feature - calculated 99-percentile quality
      !write(33,7511)LINE ! feature - calculated 99-percentile quality
      endif ! if ( output mode .eq. 3 ) =============== deal with 99-percentiles
...
*     write loads from river regulation point ==================================
      ...
      if ( QTYPE (jdet) .ne. 4 ) then
      call set format for printout (BB1(jdet),eload(jdet,i13))
*     accumulate total loads from tributaries and headwaters =============== 158
      nx = n13
      if ( munthly structure .eq. 0 ) nx = 1 ! no requirement for monthly loads
      do J1 = 1, nx      
...
*     total load introduced to reaches by clean diffuse sources ------------ 158
      TDDLOAD2(ii,J1) = 0.0

*     total loads introduced by gap filling for river flows ---------------- 158
      TILOADUP2(ii,J1) = 0.0

*     total loads removed by gap filling for river flows -----------------------
      TILOADDN2(ii,J1) = 0.0

*     total load introduced by gap filling for river quality --------------- 158
      TALOADUP2(ii,J1) = 0.0
