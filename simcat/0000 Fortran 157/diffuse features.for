...
*     ==========================================================================
*     File: diffuse features.for   2759 ----------------------------------------
*     --------------------------------------------------------------------------
...
*     set the "ifdiffuse" to "1" for diffuse pollution -------------------------
      ...
      !call check the correlation coefficients by regression ! from diffuse -----
...
*     accumulate total loads from diffuse sources ========================== 157
      do J1 = 1, nx ! ====================================================== 157      
...
*     mean flow ----------------------------------------------------------------
 ...
 1422 format(33x,'Starting ',A11,15x,'St.dev =',a10,3x, ! ---------- reach-based
     &'Mean =',a10,1x,a4)
      ...
      write(34,2182)valchars12,valchars11,valchars10,funit ! ------- reach-based
 2182 format(33x,
...
*     contributions from different sources of pollution for this shot ----------
      do ip = 1, nprop ! -------------------------------------------------------
      RLC(ip) = LMS(ip, idet, IS ) ! contribution for this shot ----------------
      enddo ! do ip = 1, nprop -------------------------------------------------
...
*     pppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp          
      ...
      do ip = 1, nprop ! ----------------------------------------- reach diffuse
      if ( ip .eq. 20 ) then ! --------------- for this type of diffuse addition
      ...
      enddo ! do ip = 1, nprop ----------------------------------- reach diffuse
...
*     and accumulate the load --------------------------------------------------
    ...
    2 continue ! do 2 IS = 1, NS +++++++++++++++++++++++++++++++++++++++++++++++
      ...
      do ip = 1, nprop ! -------------------------------------------------------
      do IS = 1, NS
      YFLOW(IS) = LMS(ip,idet,IS) ! concentrations from types of feature -------
      enddo
      ...
      ELOAD(idet,I13) = ELOAD(idet,I13) / float (NS) ! ----------- added diffuse
	...
	if ( NSM(idet,J13) .gt. 0 ) then
      ...
      TRLODE1(idet,I13) = TRLODE1(idet,J13) + eload(idet,J13) ! -- reach diffuse
      TRLODE2(idet,I13) = TRLODE2(idet,J13) + eload(idet,J13) ! -- reach diffuse
      endif
      enddo
     ...
     &quolity data(IQ,idet,1),xnum,IQDIST)
      endif
      endif
      enddo
...
*     calculations for features - nameless diffuse pollution 2 ------------ (52)
 ...
 2009 format(/77('=')/
     ...
     &'but adding no further flows'/77('='),1x,72('='))
      endif
      endif
      endif
      endif
