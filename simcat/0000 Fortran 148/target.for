...
*     ==========================================================================
*     File name: Target.FOR   1274 lines ---------------------------------------
*     compute effluent standards needed to meet mean river targets -------------
*     --------------------------------------------------------------------------
...
*     write(120+JP,*)'001 TELODE2 =',TELODE2(JP,i13)
...
*     check for no standard ----------------------------------------------------
 ...
 1486 format(77('-')/'No target for ',A11,' .... ',
     ...
     &77('-'))
      ...
      call accumulate the loads
*     write(120+JP,*)'002 TELODE2 =',TELODE2(JP,i13)
      call get summaries of river quality from the shots
      return ! return to subroutine effluent discharge -------------------------
...
*     compute the effluent standard needed to meet the river target ------------
 ...
 1705 format(110('-')/'Effluent quality needed to achieve mean river ',
...
*     write out the details for determinands -----------------------------------
 ...
 7503 format(a40,',',a40,',',a16,',',
     ...
     &rname(ireach),dname(JP),targit,0,0,Mtargit
...
*     shift parameter (for 3-parameter log-normal distribution) ----------------
      ...
      call accumulate the loads
*     write(120+JP,*)'004 TELODE2 =',TELODE2(JP,i13)
      call get summaries of river quality from the shots
      return ! return to subroutine effluent discharge -------------------------
...
*     shift parameter (for 3-parameter log-normal distribution) ----------------
      ...
      call mass balance ! from meet river targets set as averages
      call accumulate the loads
*     write(120+JP,*)'005 TELODE2 =',TELODE2(JP,i13)
      call get summaries of river quality from the shots
      return ! return to subroutine effluent discharge -------------------------
...
*     the effluent quality needed to achieve set mean of river quality ---------
*     downstream ---------------------------------------------------------------
...
*     set the mean, standard deviation and 95-percentile -----------------------
	...
	call ASSEM (xlim1,xlim2,SET1)
      ...
      write(21,3000)SET1,RCM,RCS
      write(31,3000)SET1,RCM,RCS
 3000 format(110('=')/
...
*     XECM(JP) = TECM
      ...
      if ( ical .eq. 09 .and. classob .gt. 0 ) then
 ...
 3964 format('"Achieved" river quality:        Mean =',a10,1x,A4,
     &'  from current discharge quality:  Mean =',a10,1x,A4/110('='))
      else
 ...
 3963 format('Achieved river quality:          Mean =',a10,1x,A4,
     &'  from current discharge quality:  Mean =',a10,1x,A4/110('-'))
      endif
...
*     check whether current discharge quality meets the river target ===========
      ...
      if ( TRCX .le. TRQS ) then ! if target is already met 99999999999999999999 
...
*     set quality equal to the current quality ---------------------------------
      ...
      if ( ITER .gt. 1 ) goto 890

*     set-up or replace second set ---------------------------------------------
  893 TECM1 = TECM
      TECX1 = TECX
      TRCX1 = TRCX
...
*     replace worst set with new result ----------------------------------------
...
*     check whether calculated discharge quality is infeasibly bad =============
 ...
 3463 format(
     ...
     &'at: ',a37,3x,'and other cases .........')
      ...
      if ( nobigout .le. 0 ) then
      write(01,8763)dname(jp),TRQS,UNITS(JP),TECM,UNITS(JP),
     &TRCX,UNITS(JP)
      write(21,8763)dname(jp),TRQS,UNITS(JP),TECM,UNITS(JP),
     &TRCX,UNITS(JP)
      write(31,8763)dname(jp),TRQS,UNITS(JP),TECM,UNITS(JP),
     &TRCX,UNITS(JP)
 8763 format(110('-')/'The calculated discharge quality is ',
     &'bigger than 300000 ... for ',a11/
     ...
     &TRCX,UNITS(JP)
      goto 9095 ! ????????? differs from q95 which has 9995
...
*     call accumulate the loads
*     write(120+JP,*)'006 TELODE2 =',TELODE2(JP,i13),
*    &' 9995 in target ...'
...
*     for a mean river quality standard ---------------------------------------- 
	...
	call ASSEM (testql,testqu,SET2)
      ...
      write(21,4963)valchars10,UNITS(JP),valchars11,UNITS(JP),SET1
      write(31,4963)valchars10,UNITS(JP),valchars11,UNITS(JP),SET1
 4963 format('TARGET river quality:',12x,'Mean =',a10,1x,A4,2x,
     &'Required discharge quality:'6x'Mean =',a10,1x,A4,4x,a13)
      ...
      write(21,5963)valchars10,UNITS(JP),valchars11,UNITS(JP),SET2
      write(31,5963)valchars10,UNITS(JP),valchars11,UNITS(JP),SET2
 5963 format('Achieved river quality:',10x,'Mean =',a10,1x,A4,2x,
     &14x,'Discharge 95-percentile =',a10,1x,A4,4x,a13/110('-'))
      ...
      write(21,5964)valchars10,UNITS(JP),valchars11,UNITS(JP),SET2
      write(31,5964)valchars10,UNITS(JP),valchars11,UNITS(JP),SET2
 5964 format('"Achieved" river quality:',8x,'Mean =',a10,1x,A4,2x,
     ...
     &rname(ireach),dname(JP),Trqs,Mtargit
 ...
 7513 format(a40,',',a40,',',a16,',',
     ...
     &rname(ireach),dname(JP),C(JP,1),Mtargit
