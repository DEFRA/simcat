...
*     ==========================================================================
*     General routines.... 4390 lines
*     File Name: SERVICE.FOR
*     Calculate percentiles
*     --------------------------------------------------------------------------
      subroutine get summaries and write out loads and quality (IBR)
      include 'COM.FOR'
      logical exists
...
*     check for zero standard deviation ----------------------------------------
      if (C(jper,2) .lt. 1.0E-08) goto 10
...
*     --------------------------------------------------------------------------
      ...
      write(01,1094)DNAME(jper),valchars10,UNITS(jper)
      write(21,1094)DNAME(jper),valchars10,UNITS(jper)
 1094 format(A11,44X,' Mean =',a10,1X,A4,4x,a13)
      if ( A .lt. 1.0e-9 ) goto 11
      write(01,3194)valchars11,UNITS(jper)
      write(21,3194)valchars11,UNITS(jper)
 3194 format(42X,'Standard deviation =',a10,1X,A4)
      call sort format 2 (C(jper,4),C(jper,3)) 
      write(01,1194)valchars10,UNITS(jper),SET3,valchars11,
     &UNITS(jper),SET2
      write(21,1194)valchars10,UNITS(jper),SET3,valchars11,
     &UNITS(jper),SET2
 1194 format(42X,'     90-percentile =',a10,1X,A4,4x,a13/
     &42X,'     95-percentile =',a10,1X,A4,4x,a13)
      call sort format 1 (C(jper,5))
      write(01,1294)valchars10,UNITS(jper),SET4
      write(21,1294)valchars10,UNITS(jper),SET4
...
*     --------------------------------------------------------------------------
      call sort format 2 (A,S) 
      write(01,2094)DNAME(jper),valchars10,UNITS(jper)
      write(21,2094)DNAME(jper),valchars10,UNITS(jper)
      if ( ical .lt. 07 ) then
      write(31,2094)DNAME(jper),valchars10,UNITS(jper)
 2094 format(A11,44X,' Mean =',a10,1X,A4,4x,a13)
      endif
      if ( A .lt. 1.0e-9 ) goto 11
      write(01,4094)valchars11,UNITS(jper)
      write(21,4094)valchars11,UNITS(jper)
      if ( ical .lt. 07 ) then
      write(31,4094)valchars11,UNITS(jper)
      endif
 4094 format(42X,'Standard deviation =',a10,1X,A4)
      call sort format 2 (C(jper,4),C(jper,3)) 
      write(01,2194)valchars10,UNITS(jper),SET1,valchars11,UNITS(jper)
      write(21,2194)valchars10,UNITS(jper),SET1,valchars11,UNITS(jper)
      if ( ical .lt. 07 ) then
      write(31,2194)valchars10,UNITS(jper),SET1,valchars11,UNITS(jper)
      endif
 2194 format(42X,'     10-percentile =',a10,1X,A4,4x,a13/
     &42X,'      5-percentile =',a10,1X,A4,4x,a13)
      call sort format 1 (C(jper,5))
      write(01,2294)valchars10,UNITS(jper),SET4
      write(21,2294)valchars10,UNITS(jper),SET4
      if ( ical .lt. 07 ) then
      write(31,2294)valchars10,UNITS(jper),SET4
      endif
 2294 format(42X,'      1-percentile =',a10,1X,A4,4x,a13)
   11 continue
      
      write(01,8000)
      write(21,8000)
*     if ( ical .lt. 07 ) write(31,8000)
 8000 format(77('-'))
...
*     non-parametric distributions --------------------------------------------- 
   ...
   40 write(01,1854)DNAME(jper),A,UNITS(jper)
...
*     identify the file with the non-parametric data ---------------------------
 ...
 1969 continue
	goto 1974
 1970 continue
	if ( nobigout .le. 0 ) write(01,1965) flnamesmall(2,icod)
 1965 format(
     &'Non-parametric distribution ... File: ',a64)
      write(01,8000)
...
*     identify the file with the monthly data ----------------------------------
	...
	if ( nobigout .le. 0 ) then
      write(01,2965)DNAME(jper)
 2965 format(77('-')/
     &'The river quality data for the head of Reach were ',
     &'extracted from monthly'/'distributions for ... ',a11)
      endif
...
*     pessimistic confidence limits --------------------------------------------
 ...
 3000 continue
      call sort format 2 (A,S) 
      write(01,1094)DNAME(jper),valchars10,UNITS(jper),SET1,
     &valchars11,UNITS(jper)
 1094 format(A11,' (dissolved)',32x,' Mean =',a10,1X,A4,4x,a13/
 ...
 1194 format(47X,'90-percentile =',a10,1X,A4,4x,a13/
     &47X,'95-percentile =',a10,1X,A4,4x,a13)
      ...
      write(01,1294)valchars10,UNITS(jper),SET4
 1294 format(47X,'99-percentile =',a10,1X,A4,4x,a13/77('-'))
      goto 11
...
*     identify the file with the monthly data ----------------------------------
      if ( PDRC(IQ,jper) .eq. 5 .and. IBR .eq. 1 ) then
	if ( nobigout .le. 0)write(01,2965)
 2965 format(77('-')/
     &'The river quality data for the head of Reach were ',
...
*     non-parametric distributions --------------------------------------------- 
	if ( QTYPE (jper) .eq. 03 .or. cpart2 (jper,1) .gt. -1.0E-25) goto 32
...
*     XU   .. is the upper confidence limit on the mean
*     DCP ... is the calculated value of the 95-percentile
*     D95L... is the lower confidence limit on the percentile
*     D95U... is the upper confidence limit on the percentile
*     D90 ... is the calculated value of the 90-percentile
*     D90L... is the lower confidence limit on the percentile
*     D90U... is the upper confidence limit on the percentile
*     D99  .. is the calculated value of the 99-percentile
...
*     storage for writing out confidence limits for the mean and the percentiles
	...
	write(21,9015)
 9015 format(77('=')/
     &'Observed river quality at this monitoring point ...'/77('='))
      endif
	endif
...
*     set the mean and initialise the percentiles ------------------------------
	...
	XU = A
      ...
      write(21,7500)DNAME(JP),valchars10,UNITS(JP),SET1
...
*     arrange to print out the confidence limits -------------------------------
      ...
      write(21,4500)DNAME(JP),valchars10,UNITS(JP),SET1
 4500 format(A11,45X,'Mean =',a10,1X,A4,4x,a13)
      ...
      write(21,5500)DNAME(JP),valchars10,UNITS(JP),SET1
...
*     pessimistic Confidence Limits on low log-normal percentiles --------------
      ...
      write(21,4000)DNAME(JP),valchars10,UNITS(JP),SET1
      write(01,4001)valchars11,UNITS(JP),SET3
      write(21,4001)valchars11,UNITS(JP),SET3
      call sort format 2 (C95,C99)
      write(01,4002)valchars10,UNITS(JP),SET2
      write(21,4002)valchars10,UNITS(JP),SET2
      write(01,4003)valchars11,UNITS(JP),SET4
      write(21,4003)valchars11,UNITS(JP),SET4
...
*     end of section on log-normal distributions -------------------------------
      endif ! if ( QTYPE (jp) .ne. 3 .and. QTYPE (jp) .ne. 5 )


*     section for Normal Distribution ------------------------------------------
 	if ( PDRC (IQ,JP) .eq. 1 ) then
...
*     Normal Pessimistic Confidence Limits on high normal percentiles ----------
     ...
     &valchars11,UNITS(JP),SET3
      write(21,613)DNAME(JP),valchars10,UNITS(JP),SET1,
     &valchars11,UNITS(JP),SET3
  613 format(A11,45X,'Mean =',a10,1X,A4,4x,a13/
     ...
     &valchars11,UNITS(JP),SET4
      write(21,623)valchars10,UNITS(JP),SET2,
     &valchars11,UNITS(JP),SET4
...
*     Normal Pessimistic Confidence Limits on low percentiles ------------------
     ...
     &valchars11,UNITS(JP),SET3
      write(21,6513)DNAME(JP),valchars10,UNITS(JP),SET1,
     &valchars11,UNITS(JP),SET3
 6513 format(A11,45X,'Mean =',a10,1X,A4,4x,a13/
     ...
     &valchars11,UNITS(JP),SET4
      write(21,6523)valchars10,UNITS(JP),SET2,
     &valchars11,UNITS(JP),SET4
...
*     identify the file with the non-parametric data ---------------------------
 ...
 1970 continue
	...
	if ( nobigout .le. 0 ) then
      ...
      write(21,1965) flnamesmall(2,icod)
...
*     this was set in "get non parametric file for river quality" (READ.FOR) ---
	...
	write(21,87)DNAME(JP),valchars10,UNITS(JP),valchars11,UNITS(JP)
   87 format(A11,45X,'Mean =',a10,1X,A4/
     &32X,'Non-parametric 95-percentile =',a10,1X,A4/77('='))
      ...
      write(21,54)DNAME(JP),valchars10,UNITS(JP),valchars11,UNITS(JP)
...
*     identify the file with the monthly data ----------------------------------
 ...
 2970 continue
	...
	if ( nobigout .le. 0 ) then
      ...
      write(21,2965) flnamesmall(2,icod)
...
*     this was set in "get non parametric file for river quality" (READ.FOR) ---
	...
	write(21,187)DNAME(JP),valchars10,UNITS(JP),valchars11,UNITS(JP)
  187 format(A11,45X,'Mean =',a10,1X,A4/
     &40x,'95-percentile =',a10,1X,A4/77('='))
      else
      write(01,154)DNAME(JP),valchars10,UNITS(JP),valchars11,UNITS(JP)
      write(21,154)DNAME(JP),valchars10,UNITS(JP),valchars11,UNITS(JP)
...
*     now deal with non-parametric distributions -------------------------------
*     deal with the monthly distributions? ------------------------- Richard III
      if ( PDRC(IQ,JP) .eq. 4 .or. PDRC(IQ,JP) .eq. 9 ) then
...
*     version of "calculate summaries of river flow" for tributaries ...
      subroutine calculate summaries for removed flows
      include 'COM.FOR'
      dimension Y(MS),YE(MS)
...
*     sequence the flows in the month -----------------------------------------
    ...
    4 continue ! loop on months
      ...
      write(33,4398)
 4398 format(46('-')/
     &'Generated data on flows ...'/46('-')/
     &'Month        ','       Mean','        95%','     Number',
     &/26x,'exeedence',4x,'of days'/46('-'))
	write(01,8599)(FM (imon),F5 (imon),NFK(imon),imon = 1,12)
...
*     calculate the overall mean -----------------------------------------------
 ...
 4119 format(46('-')/'Annual ...   ',2f11.2,i11/46('-'))
      endif
      endif
      return
      end
...
*     calculate the summary statistics from the shots --------------------------
      ...
      do IS = 1,NS
      Y(IS) = CMS(JDET,IS)
      if ( Y(IS) .gt. 1.0e9 ) then
      mygod = 1
      xxxxxx = Y(IS)
      ixxxxx = IS
      Y(IS) = C(JDET,1)
      write(01,5282)ixxxxx,dname(jdet),xxxxxx,FMS(IS),C(JDET,1)
      write(33,5282)ixxxxx,dname(jdet),xxxxxx,FMS(IS),C(JDET,1)
 5282 format(
     &'*** SHOT',i5,' greater than a billion for ',a11,'=',
     &f30.0,'  River flow =',f16.4,' ***'/
 	...
 	if ( ical13 .eq. 1 ) goto 1

      do ip = 1, nprop
      do IS = 1, NS
      Y(IS) = LMS(ip,JDET,IS)
      enddo
      call calculate summaries of contribution (ip, JDET, Y)
      do IS = 1, NS
      Y(IS) = LMS(ip,JDET,IS)
      enddo
      call calculate monthly summaries of contribution (ip, JDET, Y)
      if ( ip .eq. 17 ) then
      endif
	enddo
...
*     33333333333333333333333333333333333333333333333333333333333333333333333333
      ...
      include 'COM.FOR'
      dimension Y(MS),JY(K90) ! 333333333333333333333333333333333333333333333333
      ...
      JS95 = NS
      JS98 = NS
      JS99 = NS ! 33333333333333333333333333333333333333333333333333333333333333
      JS995 = NS
      JS999 = NS
      do 1 I = 1, k90
      ymax = -999.9
    ...
    1 continue
      ...
      JS95 = JY(K95)
      JS98 = JY(K98)
      JS99 = JY(K99) ! 333333333333333333333333333333333333333333333333333333333
      JS995 = JY(K995)
      JS999 = JY(K999)

      return
      end ! 33333333333333333333333333333333333333333333333333333333333333333333
*     33333333333333333333333333333333333333333333333333333333333333333333333333


      subroutine calculate summaries of river quality (JDET,Y)
      include 'COM.FOR'
	...
	Pb(IS) = Y(IS) ! for solid metal
      ...
      if ( detype (jdet) .ge. 900 ) then ! dissolved and solid metal
      if ( partition (jdet) .gt. 0.01 ) then
      gkay = 10.0 ** partition (jdet)
      do IS = 1, NS
      spm = BMS(2,IS)
	totalx = Y(IS)
	Pa(IS) = totalx / (1.0 + ((spm*gkay)/1000000.0)) ! dissolved metal
      Pb(IS) = totalx - Pa(IS) ! solid metal
	PMS1(IS) = Pa(IS) ! dissolved metal
	PMS2(IS) = Pb(IS) ! solid metal
      enddo
      else
      do IS = 1, NS
...
*     compute 5-percentile -----------------------------------------------------
   ...
   99 continue
	...
	PX2 = Pb(IS) ! solid metal
      ...
      subroutine calculate summaries of contribution (ip,JDET,Y)
      include 'COM.FOR'
      dimension Y(MS)
...
*     compute 5-percentile -----------------------------------------------------
   ...
   12 L(ip,JDET,1) = amax1 ( 0.0, CM )
      L(ip,JDET,2) = CS
      L(ip,JDET,3) = amax1 (0.0,Y(k95))
      L(ip,JDET,4) = amax1 (0.0,Y(k90))
      L(ip,JDET,5) = amax1 (0.0,Y(k99))
   ...
   99 continue
      ...
      include 'COM.FOR'
      dimension Y(MS),Pa(MS),Pb(MS),xx1(12),xx2(12)
...
*     set the month for this shot ----------------------------------------------
	...
	Pa(IS) = Y(is) ! dissolved metal
      Pb(IS) = 0.0 ! solid metal
      enddo
	endif ! if ( partition (jdet) .gt. 0.01 )
	endif ! dissolved and solid metal
...
*     set the month for this shot ----------------------------------------------
	...
	NP2 = NP2 + 1
      ...
      p1mon(JDET,1,jmon) = amax1 ( 0.0, PM1)
      p1mon(JDET,2,jmon) = PS1
      P2mon(JDET,1,jmon) = amax1 ( 0.0, PM2)
      P2mon(JDET,2,jmon) = PS2
   88 continue

      do i = 1, 12
      xx1 (i) = p1mon(JDET,1,i) 
      xx2 (i) = p1mon(JDET,2,i) 
	enddo
      do i = 1, 12
      p1mon(JDET,1,i) = cmon(JDET,1,i)
      p1mon(JDET,2,i) = cmon(JDET,2,i)
	enddo
      do i = 1, 12
      cmon(JDET,1,i) = xx1 (i)
      cmon(JDET,2,i) = xx2 (i)
...
*     batch runs ----------------------------------------------------------------
      if ( check for infeasible quality .lt. 3 ) then
      check for infeasible quality = check for infeasible quality + 1 	
      suppress14 = suppress14 + 1
	call change colour of text (21) ! dull red
	write( *,8306)uname(KU)
 8306 format('* Encountered infeasibly huge concentrations',7x,'...',7x,
     &'at ',a40,1x,25('.'))
 ...
 8366 format('Encountered infeasibly big concentrations in assessing ',
     &'compliance ...'/
     &'This is probably caused by adding (gap filling) load to ',
     &'near-zero river flows ... Check the ERR file for details ')
	write(01,8366)
      endif
 	else ! non batch runs
  	write(33,8866)DCM,DCS,int(EN),DCC
  	write(01,8866)DCM,DCS,int(EN),DCC
  	write( *,8866)DCM,DCS,int(EN),DCC
 8866 format('Infeasibly big concentrations in assessing ',
     &'compliance ...'/
     &'This is probably caused by adding (gap filling) load to ',
     &'near-zero river flows ...  '/
     &'WARNING: mean quality =',F19.4/
     &'   Standard deviation =',F19.4/
...
*     Compare area of Error Distribution of estimate of 95-percentile with 
*     standard ... This is an estimate of the probability that the standard has
*     been achieved ...
...
*     update the integrated sampling rate (QUALN) ------------------------------
     ...
     &(RFM,RCM,QUOLN,EFM,ECM,ECN)
      qqq = QUOLN
      Dd1 = RFM * RCM
      Dd2 = EFM * ECM
      Dd3 = Dd1 + Dd2
      if (Dd3 .lt. 1.0E-8) return
      qqq = ( qqq * Dd1 + ECN * Dd2) / Dd3
      QUOLN = amin1 (amax1 (5.9,qqq), 500.0)
      return
      end
...
*     calculate the mean load of partitioned metals ----------------------------
      XLOAD1 (JDET,1,i13) = XLOAD1 (JDET,1,i13) / float (NS) ! for dissolved metal
      XLOAD2 (JDET,1,i13) = XLOAD2 (JDET,1,i13) / float (NS) ! for solid metal
      if ( munthly structure .eq. 1 ) then ! fill monthly loads
      do J13 = 2, N13
      if ( NSM(jdet,J13) .gt. 1 ) then
      XLOAD1 (jdet,1,J13) = XLOAD1 (jdet,1,J13) / float (NSM(jdet,J13)) ! for dissolved metal
...
*     initialise ---------------------------------------------------------------
	...
	k13 = imonth + 1
      ...
      if ( partition (JP) .gt. 0.01 ) then ! DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
	...
	Pb(IS) = Y(IS) ! for solid metal DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
      gkay = 10.0 ** partition (JP) ! DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
      spm = BMS(2,IS)
	totalx = Pa(IS)
	...
	PMS1(IS) = Pa(IS) ! dissolved metal DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
	PMS2(IS) = Pb(IS) ! solid metal DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
	...
	Pa(IS) = Y(is) * dfms(IS) ! dissolved metal DDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
      ...
      endif ! if ( partition (JP) .gt. 0.01 ) DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
...
*     special version for any percentile ---------------------------------------
     ...
     &(DCM,DCS,EN,DCC,PER,EX,DCP,DCPL,DCPU,KU)
      include 'COM.FOR'
...
*     take opportunity to check for strange numbers 
	...
	call change colour of text (21) ! dull red
	write( *,8306)uname(KU)
 8306 format('* Encountered infeasibly big concentrations',8x,'...',7x,
     &'at ',a40,1x,25('.'))
      call set screen text colour
  	endif
	else
*     non batch runs ------------------------------------------------------------
  	write( *,8866)DCM,DCS,int(EN),DCC
  	write(01,8866)DCM,DCS,int(EN),DCC
  	write(09,8866)DCM,DCS,int(EN),DCC
  	write(33,8866)DCM,DCS,int(EN),DCC
 8866 format('Infeasibly big concentrations in assessing ',
     &'compliance ...'/
     &'This is probably caused by adding (gap filling) load to ',
...
*     compare area of Error Distribution that is lesss than the standard -------
...
*     check for a zero calculated mean -----------------------------------------
      if ( XM .lt. small .or. XS .lt. small ) then	
